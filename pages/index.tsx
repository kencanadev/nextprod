...
import { encryptData } from '@/pages/api/dataSessionStorage';
import { saveSession, SessionData } from '../lib/indexedDB';
...
const LoginCover = () => {
    ...
    const [modalFailedFnb, setModalFailedFnb] = useState(false);
    const [modalFailedDist, setModalFailedDist] = useState(false);
    ...
    const handleLogin = async () => {
        try {
            if (entitas === 'Wa') {
                ...
                const allowedEntitasFnb = ['800', '801', '802', '899','803'];
                console.log('entitas = ', entitas);

                // Validasi tipe dan entitas
                if (tipe === 'FNB') {
                    if (!allowedEntitasFnb.includes(entitas)) {
                        console.log('qwerty FNB');
                        setModalFailedDist(true);
                        return;
                    }
                } else if (tipe === 'ERP') {
                    if (allowedEntitasFnb.includes(entitas)) {
                        console.log('qwerty FNB');
                        setModalFailedFnb(true);
                        return;
                    }
                }
                ...
                if (response.data.data.length > 0) {
                    ...
                    localStorage.setItem('lastInputUser', user);
                    localStorage.setItem('lastInputTipe', tipe);
                    ...
                    if (tipe === 'ERP') {
                        ...
                        const encode = Buffer.from(JSON.stringify(sessionData)).toString('base64');
                        window.location.href = `http://10.10.1.19:3002?str=${encode}`;
    ...
    const handleCloseModals = () => {
        ...
        setModalFailedFnb(false);
        setModalFailedDist(false);
    };
    ...
    useEffect(() => {
        const lastInputUser = localStorage.getItem('lastInputUser');
        const lastInputTipe = localStorage.getItem('lastInputTipe');
        if (lastInputUser) {
            setUser(lastInputUser);
        }
        if (lastInputTipe) {
            setTipe(lastInputTipe);
        }
    }, []);
                            ...
                            >
                                <div>
                                    ...
                                    <div className="relative text-white-dark">
                                        ...
                                        >
                                            <option value="ERP">ERP DIST System</option>
                                            <option value="FNB">ERP FNB System</option>
                                ...
                                <p className="mt-4 text-center">Untuk pengalaman terbaik dan performa optimal, gunakan browser Google Chrome atau Microsoft Edge.</p>
                                <Transition appear show={modalSuccess || modalFailed || modalTipeSystem || modalFailedFnb || modalFailedDist} as={Fragment}>
                                    <Dialog as="div" open={modalSuccess || modalFailed || modalTipeSystem || modalFailedFnb || modalFailedDist} onClose={handleCloseModals}>
                                                    ...
                                                    <div className="mt-7 flex flex-col items-center justify-center">
                                                        <div className="text-lg font-bold">
                                                            {modalSuccess
                                                                ...
                                                                : modalFailedFnb
                                                                ? `Silahkan untuk entitas ${entitas} gunakan tipe ERP FNB System!`
                                                                : modalFailedDist
                                                                ? `Silahkan untuk entitas ${entitas} gunakan tipe ERP DIST System!`
                                                                : ''}
                                                        </div>
                                                    </div>
                                                    <div className="p-5">
                                                        <div className="mt-1 flex items-center justify-end">
                                                            {modalTipeSystem || modalFailed || modalFailedFnb || modalFailedDist ? (
