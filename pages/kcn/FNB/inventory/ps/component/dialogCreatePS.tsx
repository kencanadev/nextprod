...
const DialogPSList: React.FC<DialogPSList> = ({
    userid,
    kode_entitas,
    isOpen,
    onClose,
    ...
    const [jenisAkunTransaksi, setJenisAkunTransaksi] = useState<any>(false);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                if (statusPage === 'CREATE') {
                    setJenisAkunTransaksi(jenisTransaksi);
                    ...
                    const response = await axios.get(`${apiUrl}/erp/list_barang_filter?`, {
                        params: {
                            ...
                            limit: '25',
                            tipe: jenisTransaksi === 'nonPersediaan' ? 'Non Persediaan' : 'Persediaan',
                        },
                    });
                    const result1 = response.data;

                    if (result1.status) {
                        setDaftarBarang(result1.data);
                    }
                    ...
                    console.log('master[0] = ', master[0].kode_akun_ps);
                    const responseAkun = await axios.get(`${apiUrl}/erp/akun_jurnal_by_id?`, {
                        params: {
                            entitas: kode_entitas,
                            param1: master[0].kode_akun_ps,
                            param2: 'all',
                            param3: 'all',
                        },
                    });
                    ...
                    //list barang filter
                    const response1 = await axios.get(`${apiUrl}/erp/list_barang_filter?`, {
                        params: {
                            entitas: kode_entitas,
                            kode: searchNoItem ? searchNoItem : 'all',
                            nama: searchNamaItem ? searchNamaItem : 'all',
                            limit: '25',
                            tipe: responseAkun.data.data[0].no_akun === '1-10702' || responseAkun.data.data[0].no_akun === '6-10316' ? 'Non Persediaan' : 'Persediaan',
                        },
                    });
                    const result = response1.data;
                    setJenisAkunTransaksi(responseAkun.data.data[0].no_akun === '1-10702' || responseAkun.data.data[0].no_akun === '6-10316' ? 'nonPersediaan' : 'Persediaan');
        ...
        fetchData();
    }, [apiUrl, statusPage, listGudangforDetail, kode_entitas, kode_user, kode_ps, vRefreshData]);
    //***// END DETAIL PS //***//
    ...
    //         //     },
    //         // });
    //         const responseData = await axios.get(`${apiUrl}/erp/master_detail_jurnal_ps?entitas=${kode_entitas}&param1=${kode_ps}`);
    //         const { master, detail, jurnal } = responseData.data.data;
    ...
    //         //     },
    //         // });
    //         // console.log('responseAkun = ', responseAkun);
    //         // if (responseAkun.data.data[0].no_akun === '1-10702' || responseAkun.data.data[0].no_akun === '6-10316') {
    //         //     // jenisTransaksi = jenisTransaksi;
    //         //     setNoAkunFilterNonPersediaan(jenisTransaksi);
    //         // } else {
    //         //     jenisTransaksi = 'Persediaan';
    //         //     setNoAkunFilterNonPersediaan('Persediaan');
    //         // }
    ...
    //         console.log('result aaaaaaaaaaaaa= ', result);
    ...
    // useEffect(() => {
    //     refreshDaftarBarang();
    // }, [searchNoItem, searchNamaItem, kode_entitas, vRefreshData]);
    ...
    const autoJurnal = async () => {
        console.log('jenisAkunTransaksi = ', jenisAkunTransaksi);
        ...
        try {
            ...
            const [resAkunPersediaanBarangDagang, resAkunBebanPenyesuaianPersediaan] = await Promise.all([
                getAkunJurnalById(kode_entitas, jenisAkunTransaksi === 'nonPersediaan' ? result[0].kode_akun_non_persediaan : result[0].kode_akun_persediaan),
                getAkunJurnalById(kode_entitas, jenisAkunTransaksi === 'nonPersediaan' ? result[0].kode_akun_non_beban_perlengkapan : result[0].kode_akun_kerusakan_barang),
            ]);
