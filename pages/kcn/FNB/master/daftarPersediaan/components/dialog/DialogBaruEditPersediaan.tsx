...
const DialogBaruEditPersediaan = ({ isOpen, onClose, masterState, token, entitas, itemId, userid }: IDialogProps) => {
  ...
  const { formState, selectedItems, selectedAlternatifItem, setSelectedItems, setSelectedAlternatifItem, updateFormState, handleSave } = usePersediaanForm({
    entitas,
    token,
    itemId,
    masterState,
  ...
  return (
    ...
    >
      <div className="h-full overflow-auto p-4 ">
        {/* Upper Form */}
        <div className="grid w-full grid-cols-5 gap-x-4 gap-y-1">
          ...
          <div className="flex items-center gap-x-2">
            <label htmlFor="aktivasi" className="block text-xs font-medium text-gray-900 dark:text-white">
              Aktivasi
            </label>
            <div className="w-full rounded border border-gray-300 px-2">
              <ComboBoxComponent
          ...
          <div className="flex items-center gap-x-2">
            <label htmlFor="no-barang" className="block text-xs font-medium text-gray-900 dark:text-white">
              No. Barang
            </label>
            <div className="flex w-full rounded border border-gray-300 px-2">
              <TextBoxComponent name="no-barang" placeholder="No. Barang" floatLabelType="Never" value={formState.no_item} onChange={(e: any) => updateFormState('no_item', e.target.value)} />
              <div>
                <TooltipComponent
          ...
          <div className="flex items-center gap-x-0">
            <label htmlFor="nama-barang" className="block text-xs font-medium text-gray-900 dark:text-white">
              Nama Barang
            </label>
            <div className="w-full rounded border border-gray-300 px-2">
              <TextBoxComponent name="nama-barang" placeholder="Nama Barang" floatLabelType="Never" value={formState.nama_item} onChange={(e: any) => updateFormState('nama_item', e.target.value)} />
            </div>
          </div>
        </div>
        {/* Separator */}
        ...
        {formState.tipe === 'Persediaan' && (
          <div className="mb-4 grid w-full grid-cols-3 gap-x-5 gap-y-3">
            <div className="flex items-center gap-x-0">
              <label htmlFor="no-barang-barcode" className="block text-xs font-medium text-gray-900 dark:text-white">
                No. Barang (Barcode)
              </label>
              <div className="w-full rounded border border-gray-300 px-2">
                <TextBoxComponent
            ...
            <div className="flex items-center gap-x-0">
              ...
              <div className="flex w-full items-center rounded border border-gray-300 bg-[#DCDCDC] px-2" onClick={() => setShowSupplier(true)}>
                <TextBoxComponent
                  name="grup-barang"
                  ...
                  readOnly
                />
                <button id="grupBarang" className="flex items-center justify-center bg-[#DCDCDC] font-semibold" onClick={() => setShowSupplier(true)}>
                  <FontAwesomeIcon icon={faSearch} width="18" height="18" />
                </button>
              </div>
            </div>
            <div className="flex items-center gap-x-0">
              <label htmlFor="nama-barang-2" className="block text-xs font-medium text-gray-900 dark:text-white">
                Nama Barang
              </label>
              <div className="w-full rounded border border-gray-300 px-2">
                <TextBoxComponent name="nama-barang-2" placeholder="Nama Barang" floatLabelType="Never" value={formState.kustom3} onChange={(e: any) => updateFormState('kustom3', e.target.value)} />
              </div>
            </div>
            <div className="flex items-center gap-x-2">
              <input
        ...
        <Tab.Group defaultIndex={0}>
          ...
          <Tab.Panels className="w-full flex-1 border border-t-0 border-white-light p-2  text-sm dark:border-[#191e3a]">
            ...
            {selectedTab === 'Catatan' && <CatatanPersediaan formState={formState} updateState={updateFormState} />}
            ...
            {selectedTab === 'Alternatif Produk' && <AlternatifProdukPersediaan selectedItems={selectedAlternatifItem} setSelectedItems={setSelectedAlternatifItem} entitas={entitas} token={token} />}
      ...
      <DialogListSupplier isOpen={showSupplier} onClose={() => setShowSupplier(false)} updateState={updateFormState} entitas={entitas} token={token} />
    </DialogComponent>
  );
};
