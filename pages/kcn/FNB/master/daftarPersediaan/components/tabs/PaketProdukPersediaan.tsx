...
const PaketProdukPersediaan = ({ entitas, token, selectedItems, setSelectedItems }: { entitas: any; token: any; selectedItems: any; setSelectedItems: any }) => {
  ...
  const gridProduk = useRef<GridComponent | any>(null);
  const gridDaftarBarang = useRef<GridComponent | any>(null);

  const handleDataJurnal = async (jenis: any) => {
    const totalLine = gridProduk.current!.dataSource.length;
    ...
    if (jenis === 'new') {
      ...
      gridProduk.current!.addRecord(detailBarangBaru);
      gridProduk.current!.refresh();
    }
  };
      ...
      <div>
        <TooltipComponent content="Pilih data barang" opensOn="Hover" openDelay={1000} position="TopRight" target="#buNoItem">
          <div className="col-xs-6 col-sm-6 col-lg-6 col-md-6" style={{ paddingRight: '20px' }}>
            <TextBoxComponent value={args.kode_itempak} readOnly={true} showClearButton={false} />
            <span>
              <ButtonComponent
                id="buNoItem1"
                type="button"
  ...
  const handlePilihBarang = () => {
    const detailBarangBaru = {
      id_prod: gridProduk.current.dataSource.length,
      id_item: gridProduk.current.dataSource.length,
      kode_itempak: selectedItem.kode_item,
      nama_item: selectedItem.nama_item,
      satuan: selectedItem.satuan,
      qty: 1,
    };

    gridProduk.current.dataSource[selectedRowIndex] = detailBarangBaru;
    gridProduk.current.refresh();
    setShowDialog(false);
  };
  ...
  const handleDeleteDataJurnal = () => {
    swal
      .fire({
        title: `Hapus data di baris ${selectedRowIndex + 1}?`,
      ...
      .then((result) => {
        if (result.isConfirmed) {
          gridProduk.current.dataSource.splice(selectedRowIndex, 1);
          gridProduk.current.dataSource.forEach((item: any, index: any) => {
            item.id_prod = index + 1;
          });
          gridProduk.current.refresh();
        }
      });
  ...
  const handleActionComplete = (args: any) => {
    if (args.requestType === 'save') {
      const data = gridProduk.current.dataSource;
    ...
    <div className="p-2">
      ...
      >
        <ColumnsDirective>
          ...
          <ColumnDirective field="satuan" headerTextAlign="Center" textAlign="Left" headerText="Satuan" width="100" />
          <ColumnDirective field="qty" headerTextAlign="Center" textAlign="Right" headerText="Kuantitas" width="100" allowEditing />
        </ColumnsDirective>
      ...
      {showDialog && (
        <DialogComponent
          ...
          height="580"
        >
          <GridComponent ref={gridDaftarBarang} dataSource={data} recordClick={(args: any) => setSelectedItem(args.rowData)} recordDoubleClick={handlePilihBarang} allowFiltering allowPaging>
            <ColumnsDirective>
...
export default PaketProdukPersediaan;
