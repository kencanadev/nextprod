...
dialogPembatalanWarkatProps) => {
        ...
        if (kode_entitas !== null || kode_entitas !== '') {
            try {
                ...
                if (masterDataState == 'BARU') {
                    ...
                    if (modalJenisPenerimaan === 'batal cair') {
                    ...
                    } else if (modalJenisPenerimaan === 'batal tolak') {
                        const resultListEditPPIJurnal: any[] = await ListEditPPIJurnal(kode_entitas, masterKodeDokumen, '200', '299', token);
                        const jurnal = await resultListEditPPIJurnal.map((item: any) => {
                            return {
                                id: item.id_dokumen,
                                kode_akun: item.kode_akun,
    ...
    const saveDoc = async () => {
        ...
        if ((stateDataHeader?.kodeAkun === '' || stateDataHeader?.kodeAkun === null || stateDataHeader?.kodeAkun === undefined) && modalJenisPenerimaan === 'batal cair') {
            withReactContent(swalToast).fire({
                icon: 'warning',
        ...
        if (dataJurnal.nodes.length <= 0 && dataBarang.nodes.length > 0 && modalJenisPenerimaan === 'batal cair') {
            await Data();
            setIsDataFetched(true);
        } else {
            const hasAkunDivisiNodes = dataJurnal.nodes.filter(
                (node: any) =>
                    (node.tipe === 'Pendapatan' || node.tipe === 'Pendapatan Lain-Lain' || node.tipe === 'Beban' || node.tipe === 'Beban Lain-Lain') &&
                    (node.kode_jual === '' || node.kode_jual === null)
            );
            ...
            await ReCalcDataNodes(dataBarang, dataJurnal, paramObject)
                .then((result) => {
                    jsonData = {
                        ...
                        api_id: stateDataHeader?.apiId,
                        api_catatan: stateDataHeader?.apiCatatan,
                        api_pending: stateDataHeader?.apiPending,
                        api_norek: stateDataHeader?.apiNorek,
                        kode_um: null,
                        no_kontrak_um: null,
        ...
        <div>
            <DialogComponent
                ...
                header={() => {
                    ...
                    } else if (masterDataState == 'Pembatalan') {
                        header = (
                            <div>
                                <div className="header-title">
            ...
            >
                ...
                >
                    {(masterDataState == 'BARU' || masterDataState == 'Pembatalan') && (
                        <ButtonComponent
                            id="buBatalDokumen1"
                            content="Batal"
                    ...
                    {(masterDataState == 'BARU' || modalJenisPenerimaan == 'batal cair' || modalJenisPenerimaan === 'batal tolak') && (
                        <ButtonComponent
                            id="buSimpanDokumen1"
                            content="Simpan"
                            cssClass="e-primary e-small"
                            ...
                            onClick={() => {
                                ...
                                withReactContent(swalDialog)
                                    .fire({
                                        title: `<p style="font-size:12px">Anda yakin ingin membatalkan ${tipe} BG ini?</p>`,
                                        width: '18%',
                                        target: '#dialogPhuList',
                                        confirmButtonText: 'OK',
