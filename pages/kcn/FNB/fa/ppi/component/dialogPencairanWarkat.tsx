...
dialogPencairanWarkatProps) => {
                ...
                if (masterDataState == 'BARU') {
                    ...
                    await resultListEditPPIMaster.map((item: any) => {
                        setStateDataHeader((prevState: any) => ({
                            ...
                            kodeAkun: modalJenisPenerimaan === 'Edit Pencairan' || masterDataState === 'UpdateFilePendukung' ? item.kode_akun_debet : '',
                            noAkunValue: modalJenisPenerimaan === 'Edit Pencairan' || masterDataState === 'UpdateFilePendukung' ? item.no_akun : '',
                            namaAkunValue: modalJenisPenerimaan === 'Edit Pencairan' || masterDataState === 'UpdateFilePendukung' ? item.nama_akun : '',
                            ...
                            tglEfektif: masterDataState === 'UpdateFilePendukung' ? moment(item.tgl_pengakuan) : moment(),
                            apiId: item.api_id,
                    ...
                    if (modalJenisPenerimaan === 'Edit Pencairan' || masterDataState == 'UpdateFilePendukung') {
    ...
    const handleUploadZip = async (kode_dokumen: any) => {
        ...
        for (let index = 0; index < selectedFiles.length; index++) {
            ...
            const tabIdx = modalJenisPenerimaan === 'Cair' && masterDataState === 'UpdateFilePendukung' && fileWithTabIdx.tabIndex === 2 ? fileWithTabIdx.tabIndex + 1 : fileWithTabIdx.tabIndex;
            const fileExtension = file.name.split('.').pop();
            console.log('tabIdx = ', tabIdx, selectedFiles, selectedNamaFiles);
            ...
            const namaFileObj = selectedNamaFiles.find((item: any) => item.id === fileWithTabIdx.tabIndex);
            const formattedName = moment().format('YYMMDDHHmmss');
            // Jika ditemukan, gunakan nama dari object
            const namaFile = namaFileObj ? namaFileObj.name : formattedName;
    ...
    return (
        <div>
            ...
            >
                ...
                >
                    {(masterDataState == 'BARU' || masterDataState == 'Pencairan' || masterDataState === 'UpdateFilePendukung') && (
                        <ButtonComponent
                            id="buBatalDokumen1"
                            content="Batal"
