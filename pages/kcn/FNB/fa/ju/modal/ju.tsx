...
const ModalJurnalUmum: React.FC<dialogJurnalUmum> = ({ userid, kode_entitas, isOpen, onClose, kode_user, status_edit, kode_ju_edit, onRefresh, token, plag }) => {
    ...
    const saveDoc = async () => {
        ...
        try {
            ...
            if (gridDataJurnal.dataSource.length === 0) {
                ...
                //PEDIODE /* AKUN */TANSI
            } else if (moment(tglDokumen).format('YYYY-MM-DD') < formatPeriode) {
                ...
                const JSON = {
                    ...
                    tolak_catatan: null,
                    kode_kry: null,
                    tgl_trxdokumen: moment(tglJU).format('YYYY-MM-DD HH:mm:ss'), //tgl_trxdokumen edit
                    api_id: null,
                    api_pending: null,
                    api_catatan: null,
                    api_norek: null,
                    kode_aktiva: null,
                    kode_um: selectedNoKontrakUM.kode_um ?? null,
                    no_kontrak_um: selectedNoKontrakUM.no_kontrak ?? null,
                    kode_rps: null,
                ...
                if (responseAPI.data.status === true) {
                    ...
                    if (responseAPI.data.message.toLowerCase().startsWith(`Duplicate entry 'JU`.toLocaleLowerCase())) {
                        ...
                        saveDoc();
                        return;
                    } else if (responseAPI.data.error === 'Error: Server sedang sibuk, mohon coba lagi beberapa saat.') {
    ...
    // ======================END IMAGE UPLOAD====================================
     ...
     useEffect(() => {
            const dialogElement = document.getElementById('dialogJUList');
            if (dialogElement) {
        ...
        >
            ...
            <div className='w-full h-[calc(100%-100px)] flex flex-col'>
                <div className='w-full h-full '>
                    <div className='w-full h-full  '>
                        {/* ===============  Master Header Data ========================   */}
                        <div className="mb-1  h-[70px]" style={{ padding: 10, marginTop: -5 }}>
                        ...
                        {/* ===============  Detail Data ========================   */}
                        <div className="panel-tab" style={{ background: '#f0f0f0', width: '100%', height: '100%', marginTop: -5, borderRadius: 10 }}>
                            <TabComponent
                                // ref={(t) => (tabTtbList = t)}
                                selectedItem={0}
                            ...
                            >
                                ...
                                <div className="e-content !h-[calc(100%-50px)]">
                                    ...
                                    <div  tabIndex={1} style={{ width: '100%', height: '100%', marginTop: '5px', padding: 10,  }}>
                                        ...
                                        >
                                            <div className="e-tab-header  w-full flex !bg-black" style={{ display: 'flex' }}>
                                                <div
                                                    tabIndex={0}
            ...
            <div className='w-full  h-[30px]  bottom-4 absolute'
                >
                   <ButtonComponent
                        disabled={isSaving}
                        id="buBatalDokumen1"
