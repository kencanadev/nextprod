...
import Swal from 'sweetalert2';
import { FirstDayInPeriod, myAlertGlobal, myAlertGlobal2, usersMenu, viewPeriode } from '@/utils/routines';
...
const FrmPraBkkList = () => {
    ...
    const [editApproved, setEditApproved] = useState<string>('');
    ...
    const chbBatal = [
        { id_jenis: 'N', label: 'Tidak (N)' },
        { id_jenis: 'Y', label: 'Batal (Y)' },
    ];

    const [chbBatalState, setChtransaksi] = useState<Item<string>[]>(chbBatal);
    const [batalChecked, setBatalChecked] = useState<boolean[]>(Array(chbBatalState.length).fill(true));
    ...
    const paramList = {
        ...
        param8: formDataState.chbAkun ? formDataState.edKodeAkun : 'all',
        ...
        param14: formDataState.jenisBatal,
    };
    ...
    const onKlikTabList = async () => {
        ...
        await refreshDataList(paramList, setRecordsDataDetailList, gridListData, token, setIsLoadingProgress, setProgressValue, setDisplayedProgress, setLoadingMessage);
    };

    const toggleCheckboxes = (checkedState: boolean[]) => {
        return checkedState.map(() => !checkedState.every((checked) => checked)); // Toggle all
    };

    const HandleBatalList = (index: number, batalChecked: boolean[], setBatalChecked: Function) => {
        const newBatalChecked = [...batalChecked];
        newBatalChecked[index] = !newBatalChecked[index];
        setBatalChecked(newBatalChecked);
    };

    const getCheckedItems = (items: any[], checked: boolean[], isBatal: boolean = false) => {
        ...
        const joinedItems = items
            ...
            .map((item) => item.id_jenis)
            .join(',');

        return joinedItems || 'all';
    };

    const jenisApprovalChecked = getCheckedItems(chbBatal, batalChecked, true);
    ...
    const queryCellInfoListData = (args: any) => {
        ...
        if (field === 'jumlah_mu') {
            args.cell.style.background = '#e6e600';
            args.cell.style.color = '#F70000';
        }

        // if (field === 'batal') {
        if (data.batal === 'Y') {
            args.cell.style.background = '#ff0000';
            args.cell.style.color = '#fff';
        }
        // }

        if (field === 'status') {
            if (getValue('status', data) == 'Tertutup') {
                args.cell.style.color = 'red';
            } else if (getValue('status', data) == 'Proses') {
                args.cell.style.color = 'maroon';
            }
        }
        if (field === 'status_app') {
            if (getValue('status_app', data) == 'Disetujui') {
                args.cell.style.color = 'green';
            } else if (getValue('status_app', data) == 'Koreksi') {
                args.cell.style.color = 'maroon';
            } else if (getValue('status_app', data) == 'Ditolak') {
                args.cell.style.color = 'red';
            } else {
    ...
    const handleDateValidation = async () => {
        try {
            const result = await Swal.fire({
                title: `Tanggal Transaksi lebih kecil dari periode yang sekarang, tidak bisa dibatalkan.`,
    ...
    const handleBatalValidation = async (dokumen: string, tanggal: string) => {
        try {
            return await Swal.fire({
                title: `<p style="font-size:12px; color:black;">
                  Batalkan Pengeluaran Lain \n
                  No. Dokumen : ${dokumen} \n
    ...
    const getEndOfPeriod = async (formatPeriode: any) => {
        // Mengambil tahun dan bulan dari formatPeriode
        const year = parseInt(formatPeriode.split('-')[0]);
        const month = parseInt(formatPeriode.split('-')[1]);

        // Membuat objek tanggal berdasarkan tahun dan bulan
        const date = new Date(year, month, 0); // Bulan 0 berarti hari terakhir di bulan sebelumnya

        const endOfPeriod = date.toISOString().split('T')[0];
        // Mengembalikan tanggal akhir bulan
        return endOfPeriod;
    };

    const batal = async () => {
        let reqBody: any;
        let responseAPIBatal: any;
        const selectedRecord = gridListData.getSelectedRecords()?.[0];

        if (!selectedRecord?.kode_dokumen) {
        ...
        if (selectedRecord?.batal === 'Y') {
            withReactContent(swalToast).fire({
                icon: 'warning',
                title: '<p style="font-size:12px; color:white;">Data sudah dibatalkan</p>',
                width: '100%',
                customClass: {
        ...
        const periodeRef = await viewPeriode(kode_entitas == '99999' ? '999' : kode_entitas);
        const periode = periodeRef;

        // const tglDokumen = moment(selectedRecord.tgl_dokumen).format('YYYY-MM-DD');
        // const minAllowedDate = moment().subtract(1, 'day').format('YYYY-MM-DD');
        // const periode = await FirstDayInPeriod(stateDokumen?.kode_entitas);
        // const formatPeriode = moment(periode).format('YYYY-MM-DD');
        // const endOfPeriod = await getEndOfPeriod(formatPeriode);

        // if (tglDokumen < minAllowedDate) {
        // console.log('tglDokumen', tglDokumen);
        // console.log('formatPeriode', formatPeriode);
        // console.log('tglDokumen >= endOfPeriod', tglDokumen >= endOfPeriod);

        // throw exitCode;

        const isCekPeriode = cekPeriode(periode, moment(selectedRecord.tgl_dokumen).format('YYYY-MM-DD'));
        if (isCekPeriode === false) {
            return;
        }
        // if (tglDokumen >= endOfPeriod) {
        //     // myAlertGlobal('Tanggal Transaksi lebih kecil dari periode yang sekarang, tidak bisa dibatalkan.', 'main-target');
        //     // return;
        // } else {
        //     myAlertGlobal('Tanggal Transaksi lebih kecil dari periode yang sekarang, tidak bisa dibatalkan.', 'main-target');
        //     return;
        ...
        if (selectedRecord.status !== 'Terbuka') {
            myAlertGlobal(`Status dok. ${selectedRecord.status} tidak boleh dibatalkan.`, 'main-target');
            return;
        }

        if (jenisTab === 'Baru') {
            const shouldContinue = await handleBatalValidation(selectedRecord.no_dokumen, selectedRecord.tgl_dokumen);

            if (!shouldContinue) return;
            const reqBody = {
                entitas: stateDokumen.kode_entitas,
                kode_dokumen: selectedRecord.kode_dokumen,
                userid: stateDokumen.userid,
            };

            try {
                await axios
                    .post(`${apiUrl}/erp/pembatalan_bk_bm`, reqBody, {
                        headers: {
                            Authorization: `Bearer ${stateDokumen.token}`,
                        },
                    })
                    .then((result) => {
                        responseAPIBatal = result.data;
                        //  setProgressValue(50);
                        // console.log('progressValue 50', progressValue);
                        // console.log('displayedProgress 50', displayedProgress);
                    })
                    .catch((e: any) => {
                        responseAPIBatal = e.response.data;
                    });

                // const response = await axios.post(
                //     `${apiUrl}/erp/pembatalan_bk_bm`,
                //     reqBody, // Pastikan reqBody sudah didefinisikan sebelumnya
                //     {
                //         headers: {
                //             Authorization: `Bearer ${stateDokumen.token}`,
                //         },
                ...
                if (responseAPIBatal.status) {
                    withReactContent(swalToast).fire({
                        icon: 'success',
                        title: '<p style="font-size:12px; color:white;">Dokumen berhasil dibatalkan</p>',
                        width: '100%',
                        customClass: {
                            popup: styles['colored-popup'],
                        },
                ...
                // console.error('Error canceling document: ', error + 'responseAPIBatal ');
                myAlertGlobal2(`Gagal Batal Dokumen  ${error + responseAPIBatal.serverMessage}`, 'main-target');
                // Handle error case
                // Misalnya tampilkan pesan error ke user
            }
        }
    };

    const cekPeriode = (periode: any, dateNow: any) => {
    ...
    //     const periode = await FirstDayInPeriod(stateDokumen?.kode_entitas);
    //     const formatPeriode = moment(periode).format('YYYY-MM-DD');
    //     const vTglDok = moment(selectDataGrid[0]?.tgl_dokumen).format('YYYY-MM-DD');

    //     if (selectDataGrid[0]?.batal === 'Y') {
    //         withReactContent(swalToast).fire({
    //             icon: 'warning',
    //             title: '<p style="font-size:12px; color:white;">Data sudah dibatalkan</p>',
    //             width: '100%',
    ...
    //         withReactContent(swalToast).fire({
    //             icon: 'warning',
    //             title: '<p style="font-size:12px; color:white;">Tanggal Transaksi lebih kecil dari periode yang sekarang, data tidak bisa dikoreksi.</p>',
    //             width: '100%',
    //             customClass: {
    //                 popup: styles['colored-popup'], // Custom class untuk sweetalert
    //             },
    ...
    const showEditRecord = async (jenisEdit: any) => {
        ...
        // console.log('vTglDok ', vTglDok);
        // console.log('endOfPeriod ', endOfPeriod);
        // console.log('vTglDok >= endOfPeriod ', vTglDok >= endOfPeriod);

        // if (selectedRowKodeDok === '') {
        if (vTglDok >= formatPeriode || jenisEdit === 'Edit File Pendukung') {
            if (selectDataGrid[0]?.kode_dokumen === '' || selectDataGrid[0]?.kode_dokumen === undefined) {
                withReactContent(swalToast).fire({
                    icon: 'warning',
                    title: '<p style="font-size:12px; color:white;">Silahkan pilih data terlebih dahulu</p>',
    ...
    useEffect(() => {
        const fetchDataUseEffectTombol = async () => {
            try {
                ...
                const allowedEntities = new Set(['899', '698', '999']);
                // console.log('allowedEntities.has(cekEntitasUser.kode_entitas)', allowedEntities.has(cekEntitasUser.kode_entitas));
                if (allowedEntities.has(cekEntitasUser.kode_entitas)) {
    ...
    useEffect(() => {
        setFormDataState((prevState: any) => ({
            ...prevState,
            jenisBatal: jenisApprovalChecked || 'all',
        }));
    }, [jenisApprovalChecked]);
    ...
    // const HandlePilihSemuaBatalList = (batalChecked: boolean[], setBatalChecked: Function) => {
    //     const newBatalChecked = batalChecked.map(() => !batalChecked.every((checked) => checked));
    //     setBatalChecked(newBatalChecked);
    // };
                                                                ...
                                                                <div className="flex space-x-2">
                                                                    <div className="relative">
                                                                        ...
                                                                        <ButtonComponent
                                                                            ...
                                                                            disabled={userMenu.hapus === 'Y' || userid === 'administrator' ? false : true}
                                                                            onClick={async () => {
                                                                                await batal();
                                                                            }}
                                                                            content="Batal"
                                                ...
                                                <div className="mb-2 mt-2 border p-3" style={{ backgroundColor: 'white', borderRadius: 6, fontSize: 11 }}>
                                                    <div className="mb-2" style={{ marginBottom: 4 }}>
                                                        <input
                                                            ...
                                                            checked={batalChecked.every((checked: any) => checked)}
                                                            onChange={() => setBatalChecked(toggleCheckboxes(batalChecked))} // Use the new function
                                                        />
                                                        <span style={{ fontWeight: 'bold' }}>Status Pembatalan (Pilih Semua)</span>
                                                    </div>
                                                    <div className="ml-4">
                                                        {chbBatal.map((item, index) => (
                                                            <div key={index} className="mb-2" style={{ marginBottom: -2 }}>
                                                                <label className="mt-1 inline-flex">
                                                                    <input
                                                                        ...
                                                                        checked={batalChecked[index]} // Atur checked berdasarkan defaultAlasanChecked atau alasanChecked
                                                                        onChange={() => HandleBatalList(index, batalChecked, setBatalChecked)}
                                                                    />
                                                    ...
                                                    <Tab.Panel key={item.kategori}>
                                                        <div className="overflow-hidden" style={{ background: '#dedede', width: '100%' }}>
                                                            ...
                                                            >
                                                                ...
                                                                <Inject
                                                                    services={[Page, Selection, Aggregate, Edit, Toolbar, Sort, Group, Filter, Resize, Reorder, /*Freeze,*/ ExcelExport, PdfExport]}
                                                                />
                                                            </GridComponent>
                                                        </div>
                                                    </Tab.Panel>
