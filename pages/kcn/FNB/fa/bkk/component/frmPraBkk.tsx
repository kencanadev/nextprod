...
const FrmPraBkk: React.FC<FrmPraBkkProps> = ({ stateDokumen, isOpen, onClose, onRefresh, isFilePendukungBk, dataListMutasibank, onRefreshTipe }) => {
    ...
    useEffect(() => {
        const generateNoDok = async () => {
            const result = await generateNU(
                stateDokumen?.kode_entitas,
                '',
                '16',
                dataListMutasibank?.tipeApi === 'API' ? moment(dataListMutasibank?.tglTransaksiMutasi).format('YYYYMM') : moment(headerState?.tgl_dokumen).format('YYYYMM')
            );
            if (result) {
                // setHeaderState(result);
                setHeaderState((prevFormData: any) => ({
    ...
    const getTokenKey = async () => {
        const response = await axios.get(`${apiUrl}/erp/token_uuid`);
    ...
    const cekSaldoBukuBesar = async (data: any) => {
    ...
    const validasiJurnal = async (data: any) => {
    ...
    const cekSaldoValid = async (kode_akun: any, kode_subledger: any, tgl: any, jumlah_rp: any, kode_entitas: any, token: any) => {
        ...
        if (data.length > 0) {
            ...
            // console.log('validasi.lengt ', validasi[0]);
                ...
                if (validasi[0].valid === 'N') {
                    swal.fire({
                        ...
                        html: `
                                    ...
                                    Saldo Akhir: ${frmNumber(validasi[0].saldo)}<br>
                                    Saldo Setelah Transaksi: ${frmNumber(validasi[0].saldo_akhir)}<br>
    ...
    const blokingan = async () => {
        ...
        //     headerState.kode_akun_kredit,
        //     '%%',
        //     moment(headerState.tgl_dokumen).format('YYYY-MM-DD HH:mm:ss'),
        //     headerState.jumlah_mu * -1, //Number(formatNumber(headerState.jumlah_mu)),
        //     stateDokumen.kode_entitas,
        //     stateDokumen.token
        // );

        // if (cekValidasiHeader === false) {
        //     return;
        // }
    ...
    //     console.log('cekValidasiHeader ', cekValidasiHeader);
    ...
    const saveDoc = async () => {
        ...
        if (vTglDok >= formatPeriode) {
            try {
                ...
                // const isSaldoNormal = await(async()=>{
                //             const cekValidasiHeader = await cekSaldoValid(
                //                 headerState.kode_akun_kredit,
                //                 '%%',
                //                 moment(headerState.tgl_dokumen).format('YYYY-MM-DD HH:mm:ss'),
                ...
                if (!cekBlokingan) {
                    ...
                    if (stateDokumen?.CON_BKK !== 'PREVIEW_IMAGE') {
                        if (stateDokumen?.CON_BKK !== 'PREVIEW_IMAGE_ORI') {
                            const cekValidasiHeader = await cekSaldoValid(
                                headerState.kode_akun_kredit,
                                '%%',
                                moment(headerState.tgl_dokumen).format('YYYY-MM-DD HH:mm:ss'),
                    ...
                    try {
                        ...
                        // console.log('responseAPI', responseAPI);
                        // console.log('responseAPI', responseAPI?.data.status);
                        ...
                        if (stateDokumen?.masterDataState === 'BARU' && responseAPI?.data.status === true) {
                            ...
                            resultUpdateImage = await handleUpload(
                                ...
                                stateDokumen?.kode_entitas,
                                stateDokumen?.masterDataState,
                                responseAPI.data.data.kode_dokumen, //stateDokumen?.masterKodeDokumen,
                                upload1,
                                upload2,
                                upload3,
                                upload4,
                        ...
                        try {
                            if (responseAPI?.data.status === true) {
                                if (resultUpdateImage.status === true) {
                                    myAlertGlobal(`Dokumen transaksi berhasil tersimpan`, 'FrmPraBkk');
                                } else {
                                    myAlertGlobal(`Dokumen transaksi berhasil tersimpan - Silahkan update file pendukung manual`, 'FrmPraBkk');
                                    await generateNU(
                                        stateDokumen?.kode_entitas,
                                        responseAPI?.data.data.no_dokumen,
                                        '16',
                                ...
                                myAlertGlobal(`Gagal Simpan - ${error.response.data.error}`, 'FrmPraBkk');
                                clearInterval(interval);
                                setIsLoadingProgress(false);
                                setProgressValue(0);
                            } else {
            ...
            withReactContent(swalToast).fire({
                icon: 'warning',
                title: '<p style="font-size:12px; color:white;">Tanggal Transaksi lebih kecil dari periode yang sekarang, data tidak bisa disimpan.</p>',
                width: '100%',
                customClass: {
            ...
            // await myAlertGlobal('Tanggal Transaksi lebih kecil dari periode yang sekarang, data tidak bisa disimpan.', 'FrmPraBkk');
                    ...
                    <div>
                        ...
                        >
                            <div className="flex justify-between">
                                        ...
                                        <div className={styles['grid-rightNote']}>
                                            <div style={{ float: 'right' }}>
                                                <table className="mt-0 pt-0">
                                                    <tbody>
                                                        <tr>
                                                            <td style={{ paddingRight: '10px', paddingTop: '0px', fontSize: 11 }}>
                                                                <b>Total Db/Kr :</b>
                                                            </td>
                                                            {/* selisih = TotDebet - TotKredit; */}
                                                            <td style={{ paddingTop: '0px', fontSize: 11 }}>
                                ...
                                {/* <div className="w-[40%]"> */}
                                <div className={`w-[40%] p-1 ${isLoadingProgress ? 'pointer-events-none' : ''}`}>
                                    <ButtonComponent
                                        id="buBatalDokumen1"
                                        content="Batal"
