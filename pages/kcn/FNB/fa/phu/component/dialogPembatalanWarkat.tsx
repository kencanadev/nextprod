...
interface dialogPembatalanWarkatProps {
    userid: any;
    kode_entitas: any;
...
const DialogPembatalanWarkat: React.FC<dialogPembatalanWarkatProps> = ({
    userid,
    kode_entitas,
    masterKodeDokumen,
    masterDataState,
    ...
    const refreshDatasource = async () => {
        ...
        if (kode_entitas !== null || kode_entitas !== '') {
            try {
                ...
                const resultListEditPHUMaster: any[] = await ListEditPHUMaster(kode_entitas, 'IDR', masterKodeDokumen, selectedKodeSupp, modalJenisPembayaran === 'batal cair' ? 'C' : 'T');
                const resultListEditPHUDetail: any[] = await ListEditPHUDetail(kode_entitas, 'IDR', masterKodeDokumen, selectedKodeSupp, modalJenisPembayaran === 'batal cair' ? 'C' : 'T');
                const resultListEditPHUJurnal: any[] = await ListEditPHUJurnal(kode_entitas, 'IDR', masterKodeDokumen, selectedKodeSupp, modalJenisPembayaran === 'batal cair' ? 'C' : 'T');
                ...
                // Menyaring data dengan ID yang berawalan 3
                let jurnalFiltered;
                if (modalJenisPembayaran === 'batal cair') {
                    jurnalFiltered = jurnal.filter((item: any) => item.id.toString().startsWith('3'));
                } else {
                    jurnalFiltered = jurnal.filter((item: any) => item.id.toString().startsWith('2'));
                }

                await setDataJurnal({ nodes: jurnalFiltered });
                await setEditDataJurnal(jurnalFiltered);
    ...
    const saveDoc = async () => {
        ...
        await ReCalcDataNodes(dataBarang, dataSourceJurnal, paramObject, stateDataHeader)
            .then((result) => {
                jsonData = {
                    ...
                    tgl_approved: null,
                    tgl_pengakuan: moment(stateDataHeader?.tglEfektif).format('YYYY-MM-DD HH:mm:ss'),
        ...
        console.log('data Jurnal = ', dataJurnal);
        ...
        try {
            ...
            if (status === true) {
                if (modalJenisPembayaran === 'batal tolak') {
                ...
                const auditResponse = await axios.post(`${apiUrl}/erp/simpan_audit`, {
                    ...
                    proses: 'CANCEL',
                    diskripsi: `Pembayaran Hutang ${modalJenisPembayaran === 'Batal Cair' ? 'pencairan' : 'penolakan'} warkat nilai transaksi = ${frmNumber(stateDataHeader?.jumlahBayar)}`,
                        ...
                        header = (
                            <div>
                                <div className="header-title">
                                    Pembayaran Hutang <span style={{ color: '#00ab55', fontWeight: 'bold', textTransform: 'uppercase', marginLeft: 10 }}>[ {modalJenisPembayaran} ]</span>
                                </div>
                            </div>
                        );
                    }
                    ...
                    >
                        {(masterDataState == 'BARU' || masterDataState == 'EDIT') && (
                            <ButtonComponent
                                ...
                                style={{ float: 'right', width: '100px', marginTop: 1 + 'em', marginRight: 3.2 + 'em', backgroundColor: '#3b3f5c' }}
...
export default DialogPembatalanWarkat;
