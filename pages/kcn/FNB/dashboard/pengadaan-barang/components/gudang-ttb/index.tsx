...
import { fetchDetailAktualStok, fetchDetailStokGudang, fetchGudangTtb, fetchSettings, getSettingTelegram, sendTelegramMessage, sendWhatsappMessage } from '../../api';
...
import { handleKirimPesanTtb } from '../../functions/handleKirimPesan';

type InfoGudangTtbProps = {
  kode_entitas: string;
  token: string;
};

const InfoGudangTTB = ({ kode_entitas, token }: InfoGudangTtbProps) => {
  ...
  const fetchData = async () => {
    ...
    const res = await fetchGudangTtb(params, token);
    setData(res);
  };
  ...
  const handleRecordDoubleClick = async (args: any) => {
    ...
    if (field === 'nama_item') {
    ...
    } else if (field === 'stok_utama' || field === 'stok_transit') {
      ...
      await fetchDetailAktualStok({ params, token }).then(async (res: any) => {
        ...
        if (field === 'stok_utama') {
          ...
          filteredData = res.filter((item: any) => item.nama_gudang.toLowerCase().includes('ttb'));
        }
  ...
  const handleKirimPesanClick = async () => {
    ...
    if (checkedHari.ch30 === false && checkedHari.ch60 === false && checkedHari.chUp === false) {
    ...
    const { wa_pesan, tele_pesan }: any = await handleKirimPesanTtb(checkedHari, data, kode_entitas, token);
    ...
    try {
      const res = await sendTelegramMessage({ params: bodyOM, token });
      if (res.data.ok) {
        withReactContent(swalToast).fire({
          icon: 'success',
    ...
    try {
      await sendWhatsappMessage({ body: bodyOM2, token });
    } catch (error) {
    ...
    <div className="Main h-full">
      <div className="relative flex h-[calc(100vh-180px)]">
            ...
            <div className="flex h-full flex-col gap-6 overflow-y-auto overflow-x-hidden">
              ...
              <div className="flex flex-col space-y-2">
                ...
                <div className="w-fit bg-yellow-400 p-1 pr-3.5 text-black">
                  <CheckBoxComponent
                    ...
                    change={(args: ChangeEventArgsButton) => {
                      // const value: any = args.checked;
                      // updateStateFilter('isMotifProductChecked', value);
                      updateStateHari('ch60', args.checked);
                    }}
                ...
                <div className="mt-4 flex justify-start">
                  <button onClick={handleKirimPesanClick} className="mt-4 rounded-md bg-blue-800 px-3 py-1 text-white hover:bg-blue-900 disabled:cursor-not-allowed disabled:bg-gray-300">
                    Kirim Pesan
                  </button>
                </div>
              </div>
        ...
        <div className="h-full flex-1 overflow-auto">
          <GridComponent {...gridOptions} width="100%" queryCellInfo={setQueryCellInfo} rowSelected={handleRowSelected} recordDoubleClick={handleRecordDoubleClick} dataSource={data}>
            <ColumnsDirective>
              ...
              <ColumnDirective field="stok_minimal" format="N" headerText="Stok Minimal Internal" width="140" headerTextAlign="Center" textAlign="Right" clipMode="EllipsisWithTooltip" />
              <ColumnDirective field="stok_utama" format="N" headerText="Stok Gudang Utama" width="140" headerTextAlign="Center" textAlign="Right" clipMode="EllipsisWithTooltip" />
              <ColumnDirective field="stok_transit" format="N" headerText="Jml. Stok TTB" width="140" headerTextAlign="Center" textAlign="Right" clipMode="EllipsisWithTooltip" />
              <ColumnDirective field="umur" format="N" headerText="Umur Stok" width="140" headerTextAlign="Center" textAlign="Right" clipMode="EllipsisWithTooltip" />
            </ColumnsDirective>
            <AggregatesDirective>
              <AggregateDirective>
                <AggregateColumnsDirective>
