...
import styles from './fpacNonKontrak.module.css';
...
import DialogFrmFpacNonKontrak from './components/DialogFrmFpacNonKontrak';
import { usePembatalanPusat } from './hooks/usePembatalanPusat';
import DialogPembatalanFPAC from './components/dialog/DialogPembatalanFPAC';
import ReportDlgCabang from '../fpacKontrak/component/frmReportDlg';
...
const FPACList = () => {
  ...
  const [showModalCreateFpac, setShowModalCreateFpac] = useState(false);
  ...
  const handleNavigateLink = (jenis: any) => {
    setShowModalCreateFpac(false);
    if (jenis === 'edit') {
      if (selectedRow) {
        setStatusPage('EDIT');
        setIsApprovalCabang(false);
        setIsApprovalPusat(false);
        setShowModalCreateFpac(true);
      } else {
        swal.fire({
          title: 'Warning',
      ...
      if (selectedRow) {
        ...
        if (selectedData.status === 'Terbuka') {
          setStatusPage('EDIT');
          setIsApprovalCabang(false);
          ...
          swal.fire({
            text: 'Status approval Cabang Non Kontrak Baru, tidak dapat diapprove.',
            icon: 'warning',
          });
        }
      } else {
  ...
  const refreshData = async () => {
    const paramObject = {
      ...
      param11: isNamaCustomer ? namaCustomerValue : 'all', // Nama Customer
      param12: isStatusDokumen ? selectedOptionStatusDokumen : 'all', // Status Dokumen
      param13: isStatusApproval ? selectedOptionStatusApproval : 'all', // Status
      param14: isEntitas ? entitasValue : 'all', // entitas
      param15: isPembatalanPesananan ? 'Y' : 'all',
    };

    try {
      const response = await axios.get(`${apiUrl}/erp/list_fpac_non_kontrak`, {
        params: paramObject,
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      setrecordsDataFPAC(formattedData);
    } catch (error) {
  ...
  const { handleBatalClick, setMasterData, isLoading: isLoadingPembatalan, status } = usePembatalanPusat(data?.data);
  ...
  const handlePembatalanFPACSubmit = () => {
    handleBatalClick();
  ...
  return (
    <div className={`main ${showModalCreateFpac ? 'h-[100vh] min-h-[100vh]' : ''}`} id="main-target">
      <div style={{ minHeight: '40px', marginTop: '-3px', marginBottom: '11px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div className="mt-2 flex items-center md:mt-0">
          <span style={{ fontSize: 16 }} className="font-serif text-xl">
            Form Pembelian Antar Barang (FPAC) - NON KONTRAK
          </span>
        </div>
      </div>

      <div id="main-content" style={{ position: 'sticky', overflow: 'hidden' }} className="flex !h-[75vh] !gap-6">
        {/* ==============================FILTER============================================== */}
        <SidebarComponent
          id="default-sidebar"
          target={'#main-content'}
              ...
              <div className="my-5 w-full">
                <TooltipComponent content="Refresh data yang ditampilkan" opensOn="Hover" openDelay={1000} position="BottomCenter">
                  <ButtonComponent
                    ...
                    style={{ backgroundColor: '#3b3f5c', width: '100%', padding: '10px, 0px' }}
                    onClick={refreshData}
                  />
                </TooltipComponent>
              </div>
        ...
        {/* Form Grid Layouts */}
        <div
          className="panel h-[75vh] border-l-[5px] border-white"
              ...
              <div>
                <TooltipComponent ref={(t) => (tooltipListData = t)} target=".e-headertext">
                  <TabComponent selectedItem={0} animation={{ previous: { effect: 'None' }, next: { effect: 'None' } }} height="100%">
                    <div className="e-content">
                      <div style={{ marginTop: -5, fontSize: '12px', fontWeight: 'bold', padding: '10px 12px', cursor: 'pointer', borderBottom: '3px solid transparent', height: '73vh' }}>
                        <div className="h-[70vh]">
                          ...
                          >
                            <ColumnsDirective>
                              ...
                              <ColumnDirective field="no_reff" headerText="No FJ/Reff" width="100" textAlign="Left" headerTextAlign="Center" />
      ...
      {/* =============== Dialog ======================= */}
      {showModalCreateFpac && (
        <DialogFrmFpacNonKontrak
          open={showModalCreateFpac}
          onClose={() => setShowModalCreateFpac(false)}
          token={token}
