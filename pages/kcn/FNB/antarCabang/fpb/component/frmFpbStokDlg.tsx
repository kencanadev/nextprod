...
import { kodeJual } from '../../../sales/fj/component/fungsiFormFJList';
import Swal from 'sweetalert2';
import { exitCode } from 'process';
L10n.load(idIDLocalization);

interface FrmFpbStokDlgProps {
    isOpen: boolean;
    onClose: any;
    onBatal: any;
    selectedData: any;
...
const FrmFpbStokDlg = ({ isOpen, onClose, onBatal, selectedData, target, stateDokumen, kode_entitas_pusat, kode_item, diskripsi }: FrmFpbStokDlgProps) => {
    ...
    const SearchDataNo = (keyword: any, listItemBarang: any[]) => {
        if (keyword === '') {
            ...
            const filteredData = listItemBarang.filter((item) => item.no_supp.toLowerCase().startsWith(keyword.toLowerCase()));
            return filteredData;
        }
    };
    ...
    const fetchDataDialog = async (kode_entitas_pusat: any, kode_item: any) => {
        try {
            ...
            // console.log(resultFetchDataDialog.length);

            // if (resultFetchDataDialog.length <= 0) {
            //     console.log('Masuk Sini stok gudang', diskripsi);
            //     await Swal.fire({
    ...
    useEffect(() => {
        fetchDataDialog(kode_entitas_pusat, kode_item);
    }, [isOpen]);
