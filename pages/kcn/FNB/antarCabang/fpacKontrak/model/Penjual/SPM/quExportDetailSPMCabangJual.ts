...
export const quExportDetailSPMCabangJual = {
  kode_do: 'oto',
  id_do: 1,
  kode_so: 'oto',
  id_so: 1,
  kode_item: "DataCbg.quItemCabangJual.FieldValues['kode_item']",
  diskripsi: "DataCbg.quItemCabangJual.FieldValues['nama_item']",
  ...
  harga_mu: 'qudfpacharga_jual_mu.AsFloat',
  ...
  jumlah_mu: 'qudfpacqty_std.AsFloat * qudfpacharga_jual_mu.AsFloat',
  jumlah_rp: 'qudfpacqty_std.AsFloat * qudfpacharga_jual_mu.AsFloat',
  berat: 'qudfpacbrt.AsFloat',
  kode_dept: 'skodedept',
  kode_kerja: null, // Mengosongkan nilai kode_kerja
  qty_batal: 0,
};

export const createDetaiSPMPenjualPayload = async (editData: any, hargaJualMu: number, kode_entitas: string, token: string) => {
  const detail = editData.detail[0];
  const cust = await fetchCustomerMap(kode_entitas, editData.entitas_cabang_jual, token);

  const payload = {
    ...quExportDetailSPMCabangJual,
