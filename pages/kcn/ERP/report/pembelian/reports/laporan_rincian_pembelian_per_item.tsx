...
const ReportPage = () => {
    ...
    const [param18, setNoSjSupp] = useState('');
    const [param19, setNoLpbSupp] = useState('');
    ...
    useEffect(() => {
        ...
        const noSjSupp = params.get('noSjSupp');
        const noLpbSupp = params.get('noLpbSupp');
        ...
        if (noSjSupp !== null) {
            setNoSjSupp(noSjSupp);
        }
        if (noLpbSupp !== null) {
            setNoLpbSupp(noLpbSupp);
        }
    ...
    useEffect(() => {
        dispatch(setPageTitle('Daftar Beli Per Jenis Barang'));
    });
    ...
    return (
        <div style={{ width: '100%', height: '100vh' }}>
            <Viewer reportUri="/report/laporan//pembelian/laporan_rincian_pembelian_per_item.rdlx-json" reportParam={parameter} hiddenPrint={allowPrint} />
        </div>
    );
};
