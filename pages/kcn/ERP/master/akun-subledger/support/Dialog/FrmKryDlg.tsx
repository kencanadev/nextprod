...
import { ColumnDirective, ColumnsDirective, GridComponent, Inject, Resize, Selection, Sort } from '@syncfusion/ej2-react-grids';
import { fetchKaryawan } from '..';
import { TextBoxComponent } from '@syncfusion/ej2-react-inputs';

interface Props {
    isOpen: boolean;
    onClose: (KaryawanSelected: any) => void;
    entitas: string;
    token: string;
}
const FrmKryDlg = ({ isOpen, onClose, entitas, token }: Props) => {
    // Definition
    const gridKaryawan = useRef<GridComponent>(null);
    const [selectedKaryawan, setSelectedKaryawan] = useState<any>(null);
    const [loading, setLoading] = useState(false);
    const [dsKaryawan, setDsKaryawan] = useState<any>([]);
    ...
    const refreshData = async () => {
        setLoading(true);
        const karyawanRes = await fetchKaryawan(entitas, token);
        setDsKaryawan(karyawanRes);
    };
    const closeForm = () => {
        onClose(selectedKaryawan);
    };
    // useEffect
    useEffect(() => {
        refreshData().then(() => {
                ...
                <div className="relative block">
                    <div className={`${loading && 'opacity-20'}`}>
                        ...
                        <div>
                            ...
                            >
                                <ColumnsDirective>
                                    <ColumnDirective field="emp_no" headerText="NIP" headerTextAlign="Center" width={130} />
                                    <ColumnDirective field="Full_Name" headerText="Nama Karyawan" headerTextAlign="Center" width={200} />
                                    <ColumnDirective field="pos_name_en" headerText="Jabatan" headerTextAlign="Center" width={200} />
                                    <ColumnDirective field="worklocation_name" headerText="Divisi" headerTextAlign="Center" width={200} />
                                </ColumnsDirective>
