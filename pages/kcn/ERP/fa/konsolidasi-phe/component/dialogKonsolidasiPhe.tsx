...
import { GetEditPhe, GetListEkspedisi, GetListPph } from '@/lib/fa/konsolidasi-phe/api/api';
import { ReCalc } from '@/lib/fa/konsolidasi-phe/functional/reCalc';
import { CekSubledger, ListSubledger } from '../../ppi/model/apiPpi';
import DialogSubledger from './dialogSubledger';

enableRipple(true);

interface dialogKonsolidasiPheProps {
    userid: any;
    kode_entitas: any;
    kodeEntitas: any;
    entitas: any;
...
const DialogKonsolidasiPhe: React.FC<dialogKonsolidasiPheProps> = ({
    userid,
    kode_entitas,
    kodeEntitas,
    entitas,
    ...
    const [loadFilePendukung, setLoadFilePendukung] = useState<ImageData[] | any>([]);
    const tabs = [
    ...
    const refreshDatasource = async () => {
        ...
        if (kode_entitas !== null || kode_entitas !== '') {
            const paramObject = {
                kode_entitas: kodeEntitas === '' ? kode_entitas : kodeEntitas,
                token: token,
            ...
            try {
                if (masterDataState == 'BARU' && kodeEntitas !== '') {
                    const responseListEkspedisi = await GetListEkspedisi(paramObject);
                    const responseListPph = await GetListPph(paramObject);

                    const genearteNu = await generateNU(kodeEntitas, '', '27', stateDataHeader?.dateGenerateNu.format('YYYYMM'));
                    ...
                    const mergerObject = {
                        ...handleParamsObject,
                        valueObject: stateDataParams?.via,
                    };
                    ...
                    const respJurnalEditPhe = respDataEditPhe.detailJurnal;

                    const kodePph23 = responseListPph.filter((item: any) => item.kode_pajak === respMasterEditPhe.pph23);

                    const resultreferensi = await fetchPreferensi(kodeEntitas, apiUrl);
                    const resultDepartement = await FillFromSQL(kodeEntitas, 'departemen');
                    ...
                    if (masterDataState === 'BAYAR') {
                        setStateDataHeader((prevState: any) => ({
                            ...
                            kodeJual: kodeEntitas === '300' ? 'DS' : resultreferensi[0].kode_jual,
                    ...
                    const totalPembayaran = respListAlokasiPembayaranPheFix.reduce((sum: any, item: any) => sum + (parseFloat(item.bayar_muFix) || 0), 0);

                    setStateDataFooter((prevState: any) => ({
                        ...prevState,
                        totalHutang: totalNettoMu + totalPembayaran,
                        totalPembayaran: totalPembayaran,
                        sisaHutang: totalNettoMu,
                    ...
                    const respJurnalEditPheFix = respJurnalEditPhe.map((item: any, index: number) => ({
                        ...item,
                        ...
                        persen: parseFloat(item.persen),
                        mu: 'IDR',
                        departemen: stateDataHeader?.noDept + ' - ' + stateDataHeader?.namaDept,
                    }));

                    gridPheJurnalListRef.current?.setProperties({ dataSource: respJurnalEditPheFix });
                    gridPheJurnalListRef.current?.refresh();
    ...
    const handleActionBaginRows = async (args: any) => {
        if (args.requestType === 'save') {
            if (gridPheListRef.current && Array.isArray(gridPheListRef.current.dataSource)) {
                ...
                const sisa = parseFloat(args.rowData.sisaFix) - parseFloat(args.data.bayar_mu);
                const editedData = {
                    ...args.data,
                    sisa_hutang: sisa,
                    ...
                    bayar_muFix: args.data.bayar_mu,
                };
                ...
                // Menghitung total netto_mu
                const totalNettoMu = dataSource.reduce((sum: any, item: any) => sum + (item.sisaFix || 0), 0);
                const totalNettoMuEdit = dataSource.reduce((sum: any, item: any) => sum + (item.netto_mu || 0), 0);

                // Menghitung Total Pembayaran
                const totalPembayaran = dataSource.reduce((sum: any, item: any) => sum + (parseFloat(item.bayar_mu) || 0), 0);
                const totalPembayaranEdit = dataSource.reduce((sum: any, item: any) => sum + (parseFloat(item.bayarNetto) || parseFloat(item.bayar_muFix)), 0);
                const KalkulasiTotalHutang = masterDataState === 'BARU' ? totalNettoMu : totalNettoMuEdit + totalPembayaranEdit;
                const KalkulasiSisaHutang = masterDataState === 'BARU' ? totalNettoMu - totalPembayaran : KalkulasiTotalHutang - totalPembayaran;
                ...
                setStateDataFooter((prevState: any) => ({
                    ...prevState,
                    totalHutang: KalkulasiTotalHutang,
                    totalPembayaran: totalPembayaran,
                    sisaHutang: KalkulasiSisaHutang,
                    selisihAlokasiDana:
    ...
    const handleActionBaginRowsJurnal = async (args: any) => {
        if (args.requestType === 'save') {
            if (gridPheJurnalListRef.current && Array.isArray(gridPheJurnalListRef.current.dataSource)) {
                ...
                // Set dataSource baru
                gridPheJurnalListRef.current.dataSource = dataSource;
                gridPheJurnalListRef.current.refresh(); // Perbarui tampilan grid
            } else {
            }
        }
    };
    ...
    const prosesBloking = async (tipe: any) => {
        ...
        const periode = await FirstDayInPeriod(kodeEntitas);
        const formatPeriode = moment(periode).format('YYYY-MM-DD');
        const periode_date = await viewPeriode(kodeEntitas == '99999' ? '999' : kodeEntitas);
    ...
    const saveDoc = async (tipe: any) => {
        ...
        const paramObject = {
            kode_entitas: kodeEntitas,
            token: token,
            tglDokumen: moment(stateDataHeader?.tanggalBayar).isValid() ? moment(stateDataHeader?.tanggalBayar).format('YYYY-MM-DD HH:mm:ss') : moment().format('YYYY-MM-DD HH:mm:ss'),
        ...
        if (masterDataState === 'BARU') {
            const cekDataTtb = await cekDataDiDatabase(kodeEntitas, 'tb_m_phe', 'no_phe', jsonData?.no_phe, token);
            if (cekDataTtb) {
                // jsonData.no_ttb = defaultNoTtb;
                const generateCounter = await generateNU(kodeEntitas, defaultNoPhe, '27', stateDataHeader?.dateGenerateNu.format('YYYYMM'));
                const generateNoDok = await generateNU(kodeEntitas, '', '27', stateDataHeader?.dateGenerateNu.format('YYYYMM'));
            ...
            if (status === true) {
                await generateNU(kodeEntitas, defaultNoPhe, '27', stateDataHeader?.dateGenerateNu.format('YYYYMM'));
                // await generateNUDivisi(kodeEntitas, noTtbValue, divisiPenjualan, '08', moment(date1).format('YYYYMM'), moment(date1).format('YYMM') + `${divisiPenjualan}`);
                endProgress();
    ...
    const closeDialog = async () => {
        ...
        // console.log('gridPheJurnalListRef.current?.dataSource = ', gridPheJurnalListRef.current?.dataSource);
    };
    ...
    const onClickSubsidiaryLedger = async (args: any) => {
        if (args.no_akun === undefined) {
            ...
            const resultCekSubledger: any[] = await CekSubledger(kodeEntitas, args.kode_akun);
            if (resultCekSubledger[0].subledger === 'Y') {
                const resultListSubledger: any[] = await ListSubledger(kodeEntitas, args.kode_akun);
                                            ...
                                            <div className="flex" style={{ alignItems: 'center' }}>
                                                ...
                                                <div className="mt-[-105px] flex w-[53%] items-center">
                                                    ...
                                                    {masterDataState === 'BAYAR' || masterDataState === 'BATAL BAYAR' || masterDataState === 'EDIT RELEASE' || masterDataState === 'FilePendukung' ? (
                                                        ...
                                                        // </select>
                                                        <input
                                                            className="container form-input ml-[10px] mt-[117px] w-[41%] rounded-[2px] border border-[#bfc9d4] bg-[#eeeeee] text-[11px]"
                                                            disabled={true}
                                            ...
                                            <div className="e-content">
                                                ...
                                                <div style={{ width: '100%', height: '100%', marginTop: '5px' }} tabIndex={1}>
                                                    <GridComponent
                                                        ...
                                                        editSettings={
                                                            masterDataState === 'BATAL BAYAR' || masterDataState === 'FilePendukung'
                                                                ? { newRowPosition: 'Bottom' }
                                                                : { allowAdding: true, allowEditing: true, allowDeleting: true, newRowPosition: 'Bottom' }
                                                        }
                                                        selectionSettings={{ mode: 'Row', type: 'Single' }}
                                                        ...
                                                        rowSelected={selectedRowsData}
                                                        load={load}
                                                        actionBegin={handleActionBaginRowsJurnal}
                                                        // actionComplete={handleActionComplate}
                                                    >
                                                        <ColumnsDirective>
                                                            {/* <ColumnDirective field="id" type="number" isPrimaryKey={true} headerText="No." headerTextAlign="Center" textAlign="Center" width="30" /> */}
...
export default DialogKonsolidasiPhe;
