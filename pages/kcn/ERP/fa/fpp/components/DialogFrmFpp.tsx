...
const DialogFrmFpp: React.FC<DialogFrmFppProps> = ({ isOpen, onClose, token, kode_entitas, statusPage, userid, kode_dokumen, kode_user, onRefresh, statusFpp, approval, entitas_user }) => {
    ...
    const disabledInput = (() => {
        ...
        if ((approval == '1' || approval == '2') && entitas_user !== '898') {
            return true;
        }
    ...
    const disabledBtn = (() => {
        if (statusPage === 'CREATE') return false;
        if (statusPage === 'EDIT' && statusFpp === 'Lunas' && entitas_user !== '898') return true;
        if (statusPage === 'EDIT' && approval == '0') return false;
        if (statusPage === 'UPDATE-FILE' && entitas_user === '898') return false;
        if (statusPage === 'APPROVAL') return false;
        if ((approval == '1' || approval == '2') && entitas_user === '898') return false;
        return true;
    })();
    ...
    const saveDoc = async (flag: string) => {
        ...
        const reqBody = {
            ...
            keterangan: masterData.keterangan.toUpperCase(),
    ...
    useEffect(() => {
        const fetchSatuan = async () => {
            await FillFromSQL(kode_entitas, 'satuan')
                .then((result: any) => {
                    setListSatuan([...result, { satuan: 'LTR' }]);
                })
                .catch((error) => {
                    console.error('Error:', error);
                });
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <div className="mt-1 flex items-center gap-2">
                                    ...
                                    <div className="flex">
                                        ...
                                        {entitas_user === '898' && !disabledInput ? (
                                            <button
                                                className="ml-1 flex items-center justify-center border border-white-light bg-[#eee] pr-1 font-semibold dark:border-[#17263c] dark:bg-[#1b2e4b] ltr:rounded-r-md ltr:border-l-0 rtl:rounded-l-md rtl:border-r-0"
                                                style={{ height: 25, marginLeft: 0, borderRadius: 2 }}
                                                onClick={() => {
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        cols={50}
                                        rows={5}
                                        className="border border-gray-400 p-1 text-[11px] uppercase !text-black"
                    ...
                    <TabComponent selectedItem={statusPage === 'UPDATE-FILE' ? 1 : 0} animation={{ previous: { effect: 'None' }, next: { effect: 'None' } }} height="100%">
                        ...
                        <div className="e-tab-header custom-tab-wrap !max-h-[25px]">
                            ...
                            <div
                                tabIndex={1}
                                onClick={() => {
                                    setSelectedHead(`pengajuan-${selectedPengajuan + 1}`);
                                    setSelectedPengajuan(selectedPengajuan);
                                }}
                                style={{
                                    marginTop: 5,
                                    fontSize: '10px',
                            ...
                            <div
                                tabIndex={2}
                                onClick={() => {
                                    setSelectedHead(`pembayaran-${selectedPembayaran + 1}`);
                                    setSelectedPembayaran(selectedPembayaran);
                                }}
                                style={{
                                    marginTop: 5,
                                    fontSize: '10px',
