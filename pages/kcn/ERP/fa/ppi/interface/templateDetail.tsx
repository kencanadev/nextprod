...
const TemplateDetail: React.FC<templateDetailProps> = ({
    ...
    if (modalJenisPenerimaan === 'Tunai' || (masterDataState === 'UpdateFilePendukung' && modalJenisPenerimaan === 'Tunai')) {
        tabsFilePendukung = tabs;
    } else if (modalJenisPenerimaan === 'Transfer' || (masterDataState === 'UpdateFilePendukung' && modalJenisPenerimaan === 'Transfer')) {
    ...
    // JUrnal
    const OpenPreviewInsert = (index: any, tab : any) => {
        // yang di isi tab.id itu awalnya index
        setIndexPreview(tab.id);
        ...
        if (!images[tab.id]) {
            const foundItem = loadFilePendukung.find((item: any) => item.id_dokumen === tab.id);
            ...
            setImageDataUrl(images[tab.id][0]);
        }
    };
                ...
                const newNodes = await dataSource.map((node: any) => {
                    if (node.no_fj === args.rowData.no_fj) {
                        ...
                        if (args.data.bayar_mu < 0) {
                            ...
                            if (args.data.bayar_mu > node.owing) {
                                // sisaFaktur = args.data.bayar_mu - node.sisa_faktur2;
                                setStateDataDetail((prevState: any) => ({
                                    ...prevState,
                                    isChecboxPilih: true,
                                }));
                                // console.log('sisaFaktur = ', sisaFaktur);
                                ...
                                sisaFaktur = parseFloat(node.owing) - args.data.bayar_mu;
            ...
            for (let i = 0; i < items.length; i++) {
                if (items[i].kind === 'file' && items[i].type.startsWith('image/')) {
                    ...
                    if (stateDataDetail?.selectedFile === 'update') {
                        ...
                        setNamaFiles((prevNamaFiles: any) => prevNamaFiles.map((file: any) => (file.id === tabIndex ? { ...file, name: formattedName } : file)));
                    }
                    if (stateDataDetail?.selectedFile === 'baru') {
                        setSelectedFiles((prevFiles: any) => [...prevFiles, { file, tabIndex }]);
                        // setNamaFiles((prevNamaFiles: any) => [...prevNamaFiles, formattedName]);
                        setNamaFiles((prevNamaFiles: any) => [...prevNamaFiles, { id: tabIndex, name: formattedName }]);
                    }
                }
            }
        }
                                        ...
                                        <TooltipComponent content="Bersihkan" opensOn="Hover" openDelay={1000} target="#buDeleteAll1">
                                            <TooltipComponent content="Simpan" opensOn="Hover" openDelay={1000} target="#buPost1">
                                                <TooltipComponent content="Batal" opensOn="Hover" openDelay={1000} target="#buCancel1">
                                                    <div className="mt-1 flex">
                                                        <div style={{ width: '75%' }}>
                                                            {masterDataState === 'UpdateFilePendukung' || modalJenisPenerimaan === 'batal tolak' ? null : (
                                                                <>
                                                                    <ButtonComponent
                                                                        id="buAdd1"
                                                    ...
                                                    {masterDataState === 'BARU' ? (
                                                        ...
                                                        images[index].length > 0 && (
                                                            <ImageWithDeleteButton
                                                                ...
                                                                images={images}
                                                                OpenPreviewInsert={(index: any) => OpenPreviewInsert(index, tab)}
                                                            />
                                                        )
                                                            ...
                                                            <>
                                                                {(() => {
                                                                    const foundItem = loadFilePendukung.find(
                                                                        (item: any) =>
                                                                            item.id_dokumen ===
                                                                            ((modalJenisPenerimaan === 'Warkat' || masterDataState === 'UpdateFilePendukung') && tab.id === 3 ? index + 1 : index)
                                                                    );
                                                                    const filegambar = foundItem?.filegambar;
                                                ...
                                                modalJenisPenerimaan === 'Edit Penolakan' ? (
                                                    <>
                                                        <button
                                                            ...
                                                            onClick={() => OpenPreviewInsert(index, tab)}
                                                        >
