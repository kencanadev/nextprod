...
import { DiskonByCalc, FillFromSQL, GetInfo, entitaspajak, frmNumber, generateNU, generateNUDivisi, tanpaKoma } from '@/utils/routines';
//========================================================
...
import { cekNoDokTerakhir, fetchPreferensi } from '@/utils/global/fungsi';
import { GridComponent } from '@syncfusion/ej2-react-grids';
import GlobalProgressBar from '@/components/GlobalProgressBar';
import { useProgress } from '@/context/ProgressContext';
import { CekPeriodeAkutansi } from '../../phu/component/fungsiFormPhuList';

enableRipple(true);

interface dialogPpiListTransferProps {
    userid: any;
    kode_entitas: any;
...
const DialogPpiListTransfer: React.FC<dialogPpiListTransferProps> = ({
    userid,
    kode_entitas,
    masterKodeDokumen,
    masterDataState,
...
dialogPpiListTransferProps) => {
    ...
    const refreshDatasource = async () => {
        ...
        if (kode_entitas !== null || kode_entitas !== '') {
            try {
                ...
                // Ambil Token Redis perubahan 2025-0707
                ...
                if (masterDataState == 'BARU') {
                    ...
                    if (masterBarangProduksi === 'Y') {
                        ...
                        await generateNU(kode_entitas, '', '19', moment(masterBarangProduksi?.tglTransaksiMutasi).format('YYYYMM'))
                            .then((result) => {
                                setStateDataHeader((prevState: any) => ({
                                    ...prevState,
                                    noDokumenValue: result,
                    ...
                    await fetchPreferensi(kode_entitas, apiUrl)
                        .then((result) => {
                            setStateDataHeader((prevState: any) => ({
                                ...prevState,
                                noHutang: result[0].no_piutang,
                                namaHutang: result[0].nama_piutang,
                                kodeAkunHutang: result[0].kode_akun_piutang,
                                tipeHutang: result[0].tipe_piutang,

                                kodeAkunHutangBg: result[0].kode_akun_piutang_bg,
                                noAkunHutangBg: result[0].no_piutang_bg,
                                namaAkunHutangBg: result[0].nama_piutang_bg,
                                tipeAkunHutangBg: result[0].tipe_piutang_bg,
                            }));
                        })
                        .catch((error) => {
                            console.error('Error:', error);
                    ...
                    await resultListEditPPIMaster.map((item: any) => {
                        setStateDataHeader((prevState: any) => ({
                            ...
                            tglValuta: item.tgl_valuta === null ? null : moment(item.tgl_valuta),
    ...
    const [isDataFetchedReloadAPI, setIsDataFetchedReloadAPI] = useState(false);
    ...
    const handleUploadZip = async (kode_dokumen: any) => {
        ...
        for (let index = 0; index < selectedFiles.length; index++) {
            ...
            // Cari nama berdasarkan tabIdx di selectedNamaFiles
            const namaFileObj = selectedNamaFiles.find((item: any) => item.id === tabIdx);

            // Jika ditemukan, gunakan nama dari object
            const namaFile = namaFileObj ? namaFileObj.name : `Unknown_${index}`;

            const fileNameWithExtension = masterDataState !== 'BARU' ? `PU${namaFile}.${fileExtension}` : `PU${namaFile}.${fileExtension}`;
    ...
    const prosesBloking = async () => {
        // const hasTwoSpecificNodes = dataJurnal.nodes.filter(
        //     (node: any) =>
        //         (node.tipe === 'Hutang' && (node.subledger === '' || node.subledger === null || node.subledger === undefined)) ||
        //         (node.tipe === 'Piutang' && (node.subledger === '' || node.subledger === null || node.subledger === undefined)) ||
        //         node.subledger === 'Y'
        // );
        // const cekDataAkunTipe = dataJurnal.nodes.some((node: any) => node.tipe === '' || node.tipe === null);
        // // Mengambil nilai no_akun
        // const noAkunList = hasTwoSpecificNodes.map((item: any) => item.no_akun);

        // // Mengubah array menjadi string
        // const noAkunString = noAkunList.join(', ');
        ...
        // const today = moment();
        // const chosenDate = moment(stateDataHeader.tglDokumen);
        ...
        //     return;
        // }
        // ambil semua yang bayar_mu > 0
                const bayarLebih = dataBarang.nodes.filter((d) => d.bayar_mu > 0);
                const tanggalDokumen = moment(stateDataHeader.tglDokumen);
        
                for (let item of bayarLebih) {
                    const tglFj = moment(item.tgl_fj);
                    if (tglFj > tanggalDokumen) {
                        console.log('masuk sini = ', item.no_fj);
                        withReactContent(swalToast).fire({
                            icon: 'warning',
                            title: `<p style="font-size:12px;color:white;">Terdapat tanggal faktur yang direferensi lebih besar dari tgl dokumen ini, data tidak bisa di simpan.</p>`,
                            width: '100%',
                            target: '#dialogPhuList',
                            customClass: {
                                popup: styles['colored-popup'], // Custom class untuk sweetalert
        ...
        const chosenDate = moment(stateDataHeader.tglDokumen).startOf('day'); // Tanggal yang dipilih / dari input
        ...
        if (cekEntitasPajak === 'false' && !isAdmin && !allowBackdate) {
            ...
            if (diffDays < -14) {
                withReactContent(swalToast).fire({
                    icon: 'warning',
                    title: `<p style="font-size:12px;color:white;">Tanggal tidak boleh lebih dari 14 hari sebelum tanggal hari ini.</p>`,
                    width: '100%',
                    target: '#dialogPhuList',
                    customClass: {
                        popup: styles['colored-popup'], // Custom class untuk sweetalert
            ...
            if (diffDays > 3) {
                withReactContent(swalToast).fire({
                    icon: 'warning',
                    title: `<p style="font-size:12px;color:white;">Tanggal tidak boleh lebih dari 3 hari setelah tanggal hari ini.</p>`,
                    width: '100%',
                    target: '#dialogPhuList',
                    customClass: {
                        popup: styles['colored-popup'], // Custom class untuk sweetalert
        ...
        //         withReactContent(swalToast).fire({
        //             icon: 'warning',
        //             title: `<p style="font-size:12px;color:white;">No. Akun mempunyai subsidiary ledger ${noAkunString}.</p>`,
        //             width: '100%',
        //             target: '#dialogPhuList',
        //             customClass: {
        //                 popup: styles['colored-popup'], // Custom class untuk sweetalert
        ...
        // const respGetNamaBankCust = await GetNamaBankCust(kode_entitas, token, stateDataHeader?.kodeCustomerValue);
        // if(respGetNamaBankCust.length){
        ...
        if (!isValidTabIndex) {
            ...
            await withReactContent(swalDialog).fire({
                title: `<p style="font-size:12px">File Pendukung bukti transfer belum dimasukan</span></p>`,
                width: '23.4%',
                target: '#dialogPhuList',
                confirmButtonText: 'OK',
                // cancelButtonText: 'No', // Menambahkan teks tombol cancel
        ...
        if (dataJurnal.nodes.length > 0) {
            ...
            // const TotAlokasiDana = stateDataFooter.totalPenerimaan; // Misalkan state lain yang ingin kamu bandingkan
            // if (parseFloat(totalDebet.toFixed(2)) !== TotAlokasiDana && parseFloat(totalKredit.toFixed(2)) !== TotAlokasiDana) {
            //     withReactContent(swalToast).fire({
            //         icon: 'warning',
            //         title: '<p style="font-size:12px;color:white">Nilai jurnal berbeda dengan nilai dokumen, periksa kembali</p>',
            //         width: '100%',
        ...
        if (stateDataHeader?.noReferensi === '') {
            withReactContent(swalToast).fire({
                icon: 'warning',
                title: '<p style="font-size:12px;color:white;">No. Tanda Terima Pembayaran belum diisi.</p>',
                width: '100%',
                target: '#dialogPhuList',
                customClass: {
                    popup: styles['colored-popup'], // Custom class untuk sweetalert
        ...
        if (stateDataHeader?.kode_salesValue === '' || stateDataHeader?.kode_salesValue === null || stateDataHeader?.kode_salesValue === undefined) {
            withReactContent(swalToast).fire({
                icon: 'warning',
                title: `<p style="font-size:12px;color:white;">Kolektor belum diisi.</p>`,
                width: '100%',
                target: '#dialogPhuList',
                customClass: {
                    popup: styles['colored-popup'], // Custom class untuk sweetalert
        ...
        if (stateDataHeader?.kodeCustomerValue === '' || stateDataHeader?.kodeCustomerValue === null || stateDataHeader?.kodeCustomerValue === undefined) {
            withReactContent(swalToast).fire({
                icon: 'warning',
                title: `<p style="font-size:12px;color:white;">Customer belum diisi.</p>`,
                width: '100%',
                target: '#dialogPhuList',
                customClass: {
                    popup: styles['colored-popup'], // Custom class untuk sweetalert
        ...
        if (totalJumlahPembayaran === 0) {
            withReactContent(swalToast).fire({
                icon: 'warning',
                title: '<p style="font-size:12px;color:white;">Data alokasi dana belum diisi.</p>',
                width: '16%',
                target: '#dialogPhuList',
                customClass: {
                    popup: styles['colored-popup'], // Custom class untuk sweetalert
        ...
        if (stateDataHeader?.jumlahBayar === '' || stateDataHeader?.jumlahBayar === '0' || stateDataHeader?.jumlahBayar === '0.00') {
            withReactContent(swalToast).fire({
                icon: 'warning',
                title: '<p style="font-size:12px;color:white;">Jumlah Pembayaran Belum Di Isi</p>',
                width: '18%',
                target: '#dialogPhuList',
                customClass: {
                    popup: styles['colored-popup'], // Custom class untuk sweetalert
        ...
        const paramObject = {
            ...
            tgl_rekonsil:
                masterBarangProduksi === 'Y'
                    ...
                    ? updatedTglJamDokumen
                    : stateDataHeader?.tglDokumen.format('YYYY-MM-DD HH:mm:ss'),
            tgl_valuta: moment(new Date()).format('YYYY-MM-DD HH:mm:ss'),
            tipeApi: masterBarangProduksi === 'Y' ? '' : masterBarangProduksi?.tipeApi,
            rekonsiliasi: 'Y',
        };
        if (masterDataState === 'BARU') {
            await generateNU(kode_entitas, '', '19', masterBarangProduksi === 'Y' ? dateGenerateNu.format('YYYYMM') : moment(masterBarangProduksi?.tglTransaksiMutasi).format('YYYYMM'))
                .then((result) => {
        ...
        if (dataJurnal.nodes.length <= 0 && dataBarang.nodes.length > 0) {
                    ...
                    jsonData = {
                        ...
                        tgl_dokumen:
                            masterBarangProduksi === 'Y'
                                ...
                                ? updatedTglJamDokumen
                                : stateDataHeader?.tglDokumen.format('YYYY-MM-DD HH:mm:ss'),
                        // tgl_dokumen: moment(masterBarangProduksi?.tglTransaksiMutasi).utc().format('YYYY-MM-DD HH:mm:ss'),
                        no_warkat: stateDataHeader?.noBuktiTransfer,
                        // tgl_valuta: stateDataHeader?.tglBuat.format('YYYY-MM-DD HH:mm:ss'),
                        tgl_valuta:
                            masterDataState === 'BARU'
                                ? stateDataHeader?.tglBuat.format('YYYY-MM-DD HH:mm:ss')
                                : stateDataHeader?.tglValuta === null
                                ? stateDataHeader?.tglValuta
                                : stateDataHeader?.tglValuta.format('YYYY-MM-DD HH:mm:ss'),
                        kode_cust: stateDataHeader?.kodeCustomerValue,
                        kode_akun_debet: stateDataHeader?.kodeAkun,
                        kode_supp: null,
                        ...
                        faktur: 'N',
                        barcode: null,
                        komplit: masterBarangProduksi === 'Y' ? 'N' : masterBarangProduksi?.tipeApi === 'API' ? 'N' : 'N',
                        validasi1: 'N',
                        validasi2: 'N',
                        validasi3: 'N',
                        validasi_ho2: 'N',
                        ...
                        api_id: masterBarangProduksi === 'Y' ? stateDataHeader.apiId : masterBarangProduksi?.tipeApi === 'API' ? masterBarangProduksi?.apiId : stateDataHeader.apiId,
                        api_catatan:
                            masterBarangProduksi === 'Y' ? stateDataHeader.apiCatatan : masterBarangProduksi?.tipeApi === 'API' ? masterBarangProduksi?.description : stateDataHeader.apiCatatan,
                        api_pending:
                            masterBarangProduksi === 'Y' ? stateDataHeader.apiPending : masterBarangProduksi?.tipeApi === 'API' ? masterBarangProduksi?.apiPending : stateDataHeader.apiPending,
                        api_norek: masterBarangProduksi === 'Y' ? stateDataHeader.apiNorek : masterBarangProduksi?.tipeApi === 'API' ? masterBarangProduksi?.noRekeningApi : stateDataHeader.apiNorek,
                        kode_um: null,
                        no_kontrak_um: null,
                        kode_aktiva: null,
                        token: tokenRedis,
            ...
            if (masterDataState === 'BARU') {
                try {
                    ...
                    if (masterBarangProduksi === 'Y') {
                        responseSimpan = await axios.post(`${apiUrl}/erp/simpan_ppi`, jsonData, {
                            headers: {
                                Authorization: `Bearer ${token}`,
                            },
                        });
                    } else {
                        responseSimpan = await axios.post(`${apiUrl}/erp/update_warkat_mutasi_bank`, jsonData, {
                            headers: {
                                Authorization: `Bearer ${token}`,
                            },
                        });
                    ...
                    if (status === true) {
                        ...
                        const auditResponse = await axios.post(`${apiUrl}/erp/simpan_audit`, {
                            ...
                            proses: 'NEW',
                            diskripsi: `Penerimaan Piutang transfer nilai transaksi = ${frmNumber(stateDataHeader?.jumlahBayar)}`,
                        ...
                        endProgress();
                        if (resultSimpan.error === 'Server sedang sibuk, mohon coba lagi beberapa saat.') {
                            await withReactContent(swalPopUp).fire({
                                icon: 'error',
                                title: `${resultSimpan.error}`,
                                width: '50%', // Atur lebar popup sesuai kebutuhan
                                target: '#dialogPhuList',
                                heightAuto: true,
                                timer: 3500,
                ...
                // await handleUploadZip('PU0000001111');
    ...
    return (
        <div>
                        ...
                        header = (
                            <div>
                                <div className="header-title">
                                    Penerimaan Piutang Baru{' '}
                                    <span style={{ color: '#00ab55', fontWeight: 'bold', textTransform: 'uppercase', marginLeft: 10 }}>
                                        [ {modalJenisPenerimaan} ] {masterBarangProduksi?.tipeApi === 'API' ? '[ API ID : ' + masterBarangProduksi?.apiId + ' ]' : null}
                                    </span>
                                </div>
                            </div>
                        );
            ...
            >
                    ...
                    <div>
                        ...
                        <div>
                            ...
                            <TemplateFooter
                                ...
                                setStateDataFooter={setStateDataFooter}
                                modalJenisPenerimaan={masterDataState}
                            />
                        </div>
                    </div>
                </div>
                ...
                >
                    {(masterDataState == 'BARU' || masterDataState == 'EDIT' || masterDataState === 'UpdateFilePendukung') && (
                        <ButtonComponent
                            id="buBatalDokumen1"
                            content="Batal"
                    ...
                    {(masterDataState == 'BARU' || masterDataState == 'EDIT' || masterDataState === 'UpdateFilePendukung') && (
                        <ButtonComponent
                            ...
                            onClick={masterDataState === 'UpdateFilePendukung' ? updateFilePendukung : prosesBlokingTerbaru}
                            disabled={isLoading}
                        />
                    )}
                    {isOpenPreview && (
...
export default DialogPpiListTransfer;
