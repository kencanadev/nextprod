...
const DialogFrmDp: React.FC<DialogFrmFppProps> = ({ isOpen, onClose, token, kode_entitas, kode_dokumen, statusPage, userid, onRefresh }) => {
    ...
    const saveDoc = async (flag: string) => {
        ...
        const payloadKeuangan = {
            dokumen: 'JU',
            no_dokumen: noJu,
            tgl_dokumen: moment(masterData.tgl_bayar).format('YYYY-MM-DD HH:mm:ss'),
            no_warkat: null,
            tgl_valuta: moment(masterData.tgl_bayar).format('YYYY-MM-DD HH:mm:ss'),
            kode_cust: null,
            ...
            tolak_catatan: null,
            kode_kry: null,
            tgl_trxdokumen: moment(masterData.tgl_bayar).format('YYYY-MM-DD HH:mm:ss'),
            api_id: null,
            api_pending: null,
            api_catatan: null,
            api_norek: null,
        ...
        const payloadJurnal = [
            {
                ...
                kode_akun: statusPage === 'PEMBAYARAN' ? 'AK0000000080' : 'AK0000000041',
                // kode_subledger: kode_subledger?.data?.data?.[0]?.kode_subledger ?? null,
                kode_subledger: statusPage === 'PEMBAYARAN' ? 'SS0000001943' : kode_subledger?.data?.data?.[0]?.kode_subledger,
                kurs: 1,
                debet_rp: statusPage === 'APPROVAL' ? reqBody.total_rp : masterData.bayar_rp,
                kredit_rp: 0,
                jumlah_rp: statusPage === 'APPROVAL' ? reqBody.total_rp : masterData.bayar_rp,
            ...
            },
            {
                ...
                kode_akun: statusPage === 'PEMBAYARAN' ? 'AK0000000037' : 'AK0000000080',
                // kode_subledger: kode_subledger?.data?.data?.[0]?.kode_subledger ?? null,
                kode_subledger: statusPage === 'PEMBAYARAN' ? 'SS0000001410' : 'SS0000001943',
                kurs: 1,
                debet_rp: 0,
                kredit_rp: statusPage === 'APPROVAL' ? reqBody.total_rp : masterData.bayar_rp,
            ...
            <div className="mb-1">
                ...
                <div className="panel-tab my-2" style={{ background: '#F7F7F7', width: '100%', height: 'auto' }}>
                    <TabComponent selectedItem={statusPage === 'UPDATE-FILE' ? 1 : 0} animation={{ previous: { effect: 'None' }, next: { effect: 'None' } }} height="100%">
                        {/* Header */}
                        <div className="e-tab-header custom-tab-wrap">
