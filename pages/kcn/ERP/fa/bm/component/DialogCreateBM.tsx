...
import { frmNumber, generateNU, FillFromSQL, FirstDayInPeriod } from '@/utils/routines';
...
import { cekDataDiDatabase, cekNoDokTerakhir, entitaspajak, isEntitasPajak } from '@/utils/global/fungsi';
...
const DialogBMList: React.FC<DialogBMList> = ({
    ...
    function stringToNumber(value: unknown): number {
        if (typeof value === 'string') {
            const cleanedValue = value.replace(/,/g, '');
            const number = parseFloat(cleanedValue);
            return isNaN(number) ? 0 : number;
        } else if (typeof value === 'number') {
            return value; // Jika sudah berupa angka, langsung kembalikan
        }
    ...
    const handlePilihJurnal = async () => {
        if (selectedJurnalHeader.header === 'Y' && modalAkunSource === 'header') {
            ...
            if (modalAkunSource === 'header') {
                ...
                const totalLines = gridAlokasiDana.dataSource.length;
                const id_dokumen = gridAlokasiDana.dataSource[selectedRowIndex].id_dokumen;
    ...
    buttonDaftarAkunKasDetail = [
        {
            ...
            isFlat: false,
            click: () => {
                currentDaftarAkunKasDetail = (gridDaftarJurnalDetailList.current?.getSelectedRecords() as any[]) || [];
    ...
    const handleDataJurnal = async (jenis: any) => {
        const totalLine = gridAlokasiDana.dataSource.length + 1;
    ...
    const validate = async () => {
        ...
        // const isSubledgerNull = gridAlokasiDana.dataSource.some(
        //   (item: any) => (item.tipe === 'Hutang' || item.tipe === 'Piutang' || cek_subledger.data.data[0].subledger === 'Y') && item.kode_subledger === null
        // );
    ...
    const saveDoc = async () => {
        ...
        const modifiedDataJurnalHeader = {
            ...
            kode_subledger: selectedSubledgerAllDataHeader.kode_subledger ? selectedSubledgerAllDataHeader.kode_subledger : null,
            userid: userid.toUpperCase(),
        };
        ...
        const reqBody = {
            plagMutasi: dataListMutasibank?.tipeApi,
            entitas: kode_entitas,
            bm_pos: 'N',
        ...
        // if (dataListMutasibank?.tipeApi === 'API') {
        //     console.log('dataListMutasibank 1 = ', dataListMutasibank);
        // } else {
        //     console.log('dataListMutasibank 2 = ', dataListMutasibank);
        // }
            ...
            if (responseAPI && responseAPI.data.status === true) {
                if (statusPage === 'CREATE' && isApprovedData === 'N') {
                    ...
                    const auditReqBody = {
                        ...
                        kode_audit: null,
                        dokumen: 'BM',
                        kode_dokumen: responseAPI.data.data.kode_dokumen,
                        no_dokumen: mNoBM,
                                ...
                                <div className="e-content">
                                    ...
                                    <div tabIndex={0} style={{ width: '100%', height: '280px', marginTop: '5px', padding: 10 }}>
                                        <TooltipComponent openDelay={1000} target=".e-headertext">
                                            ...
                                            >
                                                <ColumnsDirective>
                                                    <ColumnDirective field="id_dokumen" visible={false} isPrimaryKey />
                                                    <ColumnDirective
                                                        field="no_akun"
                                                        headerText="No. Akun"
