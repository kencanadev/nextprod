...
import { frmNumber, generateNU, FillFromSQL, fetchPreferensi, FirstDayInPeriod, entitaspajak } from '@/utils/routines';
...
import { faSearch, faListAlt, faList } from '@fortawesome/free-solid-svg-icons';
import { resWord, resExcel, resZip, resUnknow, resPdf } from './resource';
import JSZip from 'jszip';
import { GetListTabNoRek } from '@/lib/fa/mutasi-bank/api/api';
import { CekUserApp, DaftarAkunDebet } from '../../ppi/model/apiPpi';
import { useProgress } from '@/context/ProgressContext';
import GlobalProgressBar from '@/components/GlobalProgressBar';

L10n.load(idIDLocalization);

interface DialogBMPOSList {
    userid: string;
    kode_entitas: any;
    isOpen: boolean;
    onClose: () => void;
...
let gridAlokasiFakturPOS: Grid | any;
let gridDaftarKaryawan: Grid | any;
let gridDaftarDepartmen: Grid | any;

const DialogBMPOSList: React.FC<DialogBMPOSList> = ({
    userid,
    kode_entitas,
    isOpen,
    onClose,
    ...
    const [noAkunAPi, setNoAkunApi] = React.useState('1');
    ...
    const handleCreate = async () => {
        ...
        try {
            const response = await fetch(`${apiUrl}/erp/master_detail_jurnal_debet_pemasukkan_lain_bm?entitas=${kode_entitas}&param1=&param2=baru`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            ...
            if (data.status) {
                const formattedData = data.data.alokasi.map((item: any, index: number) => {
                    const tglFj = item.tgl_fj ? moment(item.tgl_fj).format('YYYY-MM-DD') : null;
                    const tglTrxFj = item.tgl_trxfj ? moment(item.tgl_trxfj).format('YYYY-MM-DD') : null;
                    const hariIni = moment().format('YYYY-MM-DD');
                    const JT = item.JT ? moment(item.JT) : null;
                    const hari = JT ? moment(JT).diff(hariIni, 'days') : null;

                    return {
                        ...item,
                        pUcode: '',
                        id_dokumen: (index + 1).toString(),
                        bayar_mu: 0,
                        pay: false,
                        tgl_fj: tglFj,
                        tgl_trxfj: tglTrxFj,
                        sisa: parseFloat(item.owing),
                ...
                gridAlokasiFakturPOS.dataSource = formattedData;
            } else {
                console.error('Failed to fetch alokasi data');
            }
        } catch (error) {
            console.error('Error fetching alokasi data:', error);
        }
    };
    ...
    const handleEdit = async () => {
        ...
        const { master, detail, debet, alokasi } = response.data.data;
        ...
        if (master.length > 0) {
            ...
            setDate1(moment(dataMaster.tgl_trxdokumen, 'YYYY-MM-DD'));
        }
        if (debet.length > 0) {
            const mappedAlokasiDanaHeader = debet.map((item: any) => ({
                ...
                // yang ditampilkan di table alokasi dana
        ...
        if (alokasi.length > 0) {
            const mappedAlokasiPOS = alokasi.map((item: any) => ({
                ...
                id_dokumen: item.id_dokumen,
                pajak: item.pajak,
                bayar_mu: parseInt(item.bayar_mu),
                Lunas_awal: item.Lunas_awal,
                pay: item.pay,
                owing: item.owing,
                tgl_dokumen: item.tgl_dokumen,
                hari: item.hari2,
                sisa: item.sisaPiutangFaktur2,
            }));
            const totalBayarMu = mappedAlokasiPOS.reduce((total: any, item: any) => total + item.bayar_mu, 0);
            setTotalNilaiPOS(totalBayarMu);
            gridAlokasiFakturPOS.dataSource = mappedAlokasiPOS;
        } else {
            console.log('gagal');
        }
    };
    ...
    const refreshDaftarAkunHeader = async () => {
        try {
            ...
            gridJurnalHeaderList.current?.setProperties({
    ...
    const refreshDaftarAkunDetail = async () => {
        try {
            ...
            gridDaftarJurnalDetailList.current?.setProperties({
                dataSource: responAkunDetail,
            });
        ...
        if (selectedJurnalHeader.header === 'Y' && modalAkunSource === 'header') {
            ...
            if (modalAkunSource === 'header') {
                ...
                const detailBarangBaru = {
                    ...
                    // nilai_mu: nilaiMU != 0 ? nilaiMU : totalDebet === totalKredit ? 0 : selisih === 0 ? Number(formatNumber(inputValueJumlah_mu)) : selisih,
    ...
    const gridIndukHeaderJurnal = (props: any) => {
        return (
            <div
                style={{
                    fontWeight: props.header === 'Y' ? 'bold' : 'normal',
                    marginLeft: props.header === 'N' ? '0.5rem' : '0',
                }}
            >
                {props[props.column.field]}
            </div>
        );
    };
    ...
    const refreshDaftaSubledgerCustomer = async (kode_mu: any) => {
        ...
        try {
            ...
            gridDaftarSubledgerCustomerJurnal.current?.setProperties({
    ...
    const [selectedRowIndexPOS, setSelectedRowIndexPOS] = useState(0);

    const rowSelectingDetailJurnalDetail = (args: any) => {
        setSelectedRowIndex(args.rowIndex);
    };
    const rowSelectingDetailPOS = (args: any) => {
        setSelectedRowIndexPOS(args.rowIndex);
    };
    ...
    if (selectedJurnalHeader && selectedJurnalHeader.balance !== undefined) {
        formattedBalance = new Intl.NumberFormat('en-US', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
        }).format(selectedJurnalHeader.balance);

        resultBalance = isNaN(Number(selectedJurnalHeader.balance)) ? '' : formattedBalance;
    }
    ...
    useEffect(() => {
        if (statusPage === 'EDIT') {
            if (inputValueJumlah_mu) {
                const number = parseFloat(formatNumber(inputValueJumlah_mu));
                const words = terbilang(number);
                setOutputWordsJumlah_mu(`** ${words} **`);
            } else {
                setOutputWordsJumlah_mu('Masukkan angka yang valid');
    ...
    const handleInputBlur = () => {
        if (!isNaN(Number(inputValueJumlah_mu.replace(/,/g, ''))) && inputValueJumlah_mu !== '') {
            const formattedValue = parseFloat(inputValueJumlah_mu.replace(/,/g, '')).toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2,
            });
    ...
    const handleDataJurnal = async (jenis: any) => {
        ...
        if (Number(inputValueJumlah_mu) == 0) {
            withReactContent(swalToast).fire({
                icon: 'error',
    ...
    const validate = async () => {
        ...
        const someKreditNull = gridAlokasiDana.dataSource.some((item: any) => item.kredit_rp === 0);
        ...
        const isSubledgerNull = gridAlokasiDana.dataSource.some((item: any) => {
            const itemTipeLower = item.tipe.toLowerCase();

            const needsSubledger =
                item.tipe === 'Hutang' ||
                item.tipe === 'Piutang' ||
                cek_subledger.data.data.some((subledgerRule: any) => subledgerRule.subledger === 'Y' && subledgerRule.tipe.toLowerCase() === itemTipeLower);

            return needsSubledger && item.nama_relasi === null;
        });
        ...
        // if (!Array.isArray(gridAlokasiFakturPOS.dataSource)) {
        //     console.error('gridAlokasiFakturPOS.dataSource is not an array');
        //     return false;
        // }

        const allPayFalse = gridAlokasiFakturPOS.dataSource.every((item: any) => item.pay === false);
        if (allPayFalse) {
            withReactContent(swalToast).fire({
                icon: 'error',
                title: '<p style="font-size:12px">Data alokasi faktur pos tidak diisi</p>',
                width: '100%',
                target: '#dialogBMList',
            });
            return false;
        ...
        if (totalNilaiPOS - Number(inputValueJumlah_mu.replace(/,/g, '')) !== 0) {
            withReactContent(swalToast).fire({
                icon: 'error',
                title: '<p style="font-size:12px">Masih ada selisih antara debet dan alokasi faktur POS</p>',
                width: '100%',
                target: '#dialogBMList',
            });
            return false;
    ...
    const saveDoc = async () => {
        ...
        const today = moment();
        const chosenDate = moment(date2);
        const differenceInDays = chosenDate.diff(today, 'days');

        const periode = await FirstDayInPeriod(kode_entitas);
        const formatPeriod = moment(periode);
        ...
        const text = differenceInDays < -14 ? 'sebelum' : 'sesudah';
        ...
        if (moment(date2).format('YYYY-MM-DD') < moment(periode).format('YYYY-MM-DD')) {
            swal.fire({
                icon: 'error',
                title: 'Tanggal lebih kecil lebih dari periode yang ditentukan.',
                showConfirmButton: false,
                timer: 3000,
                target: '#dialogBMList',
            });
        ...
        if (cekEntitasPajak === 'false' && (differenceInDays < -14 || differenceInDays > 14) && resultCekUserApp[0].app_backdate === 'N' && userid !== 'administrator') {
            swal.fire({
                icon: 'error',
                title: `<p style="font-size:12px;color:white;">Tanggal tidak boleh lebih dari 14 hari ${text} tanggal hari ini.</p>`,
                showConfirmButton: false,
                timer: 3000,
                target: '#dialogBMList',
            });
        ...
        const noBm = await generateNU(kode_entitas, '', '17', moment().format('YYYYMM'));
        ...
        const modifiedDataJurnalHeader = {
            ...
            catatan: keterangan,
            kurs: kursHeader,
            kode_akun: dataJurnalHeader.kode_akun,
            kode_subledger: selectedkode_Subledger === '' ? null : selectedkode_Subledger,
        ...
        const reqBody = {
            ...
            isEntitasPajak: 'N',
            alokasi: gridAlokasiFakturPOS.dataSource.map((item: any) => ({
                pUcode: null,
                kode_fj: item.kode_fj,
                kode_dokumen: statusPage === 'EDIT' ? kode_bm : '',
                id_dokumen: item.id_dokumen,
                bayar_mu: item.bayar_mu,
                pay: item.pay == true ? 'Y' : 'N',
            })),
        };
        ...
        try {
            ...
            if (responseAPI.data.status === true) {
                if (statusPage === 'CREATE') {
                    await generateNU(kode_entitas, responseAPI.data.data.no_dokumen, '17', moment().format('YYYYMM'));
                    handleUpload(responseAPI.data.data.kode_dokumen);
            ...
            if (responseAPI && responseAPI.data.status === true) {
                withReactContent(swalToast).fire({
                    icon: 'error',
                    title: `<p style="font-size:12px">Data Transaksi berhasil disimpan, silakan cek kembali data file pendukung.</p>`,
                    width: '100%',
                    target: '#dialogBMList',
                });
            } else if (error?.response?.data?.error === 'Error: Server sedang sibuk, mohon coba lagi beberapa saat.') {
                withReactContent(swalToast).fire({
                    icon: 'error',
                    title: `<p style="font-size:12px">Gagal Simpan Data: Server sedang sibuk, mohon coba lagi beberapa saat.</p>`,
                    width: '100%',
                    target: '#dialogBMList',
                });
            } else {
                withReactContent(swalToast).fire({
                    icon: 'error',
                    title: `<p style="font-size:12px">${error?.response?.data?.message}</p>`,
                    width: '100%',
                    target: '#dialogBMList',
                });
            }
        ...
        if (uploaderRef.current && uploaderRef2.current && uploaderRef3.current && uploaderRef4.current) {
            const filesArray = [
                preview
                    ? [
                          {
                              rawFile: await fetch(preview).then((res) => res.blob()),
                              fileName: nameImage,
                              originalName: uploaderRef.current.getFilesData()[0]?.name,
                          },
                      ]
                ...
                preview2
                    ? [
                          {
                              rawFile: await fetch(preview2).then((res) => res.blob()),
                              fileName: nameImage2,
                              originalName: uploaderRef2.current.getFilesData()[0]?.name,
                          },
                      ]
                ...
                preview3
                    ? [
                          {
                              rawFile: await fetch(preview3).then((res) => res.blob()),
                              fileName: nameImage3,
                              originalName: uploaderRef3.current.getFilesData()[0]?.name,
                          },
                      ]
                ...
                preview4
                    ? [
                          {
                              rawFile: await fetch(preview4).then((res) => res.blob()),
                              fileName: nameImage4,
                              originalName: uploaderRef4.current.getFilesData()[0]?.name,
                          },
                      ]
    ...
    //     const totalKredit = gridAlokasiDana.dataSource.reduce((total: number, item: any) => {
    //       // Konversi kredit_rp ke number jika tidak null dan dapat dikonversi ke angka, jika tidak gunakan 0
    //       const kreditRp = item.kredit_rp !== null && !isNaN(Number(item.kredit_rp)) ? Number(item.kredit_rp) : 0;
    ...
    // Checkbox Alokasi //
    const [originalSisa, setOriginalSisa] = useState<any>({});

    const handleCheckboxChange = (record: any, event: any) => {
        if (gridAlokasiFakturPOS.dataSource.some((item: any) => item.kode_fj === record.kode_fj && item.pay)) {
            // Uncheck
            gridAlokasiFakturPOS.dataSource = gridAlokasiFakturPOS.dataSource.map((item: any) => {
                if (item.kode_fj === record.kode_fj) {
                    return {
                        ...item,
                        bayar_mu: 0,
                        sisa: originalSisa[item.kode_fj],
                        pay: false,
                    };
                }
                return item;
            ...
            const newOriginalSisa = {
                ...originalSisa,
                [record.kode_fj]: record.sisa,
            };
            setOriginalSisa(newOriginalSisa);
        }
        const total = gridAlokasiFakturPOS.dataSource.reduce((total: any, item: any) => {
            const bayar_mu = typeof item.bayar_mu === 'string' ? parseFloat(item.bayar_mu) : item.bayar_mu;
            return total + (bayar_mu || 0);
        }, 0);
        setTotalNilaiPOS(total);
    };

    const handleCheckboxChange2 = (record: any, checked: any) => {
        const dataSource = gridAlokasiFakturPOS.dataSource;
        ...
        if (record.pay) {
            editedData = {
                ...dataSource[idx],
                bayar_mu: 0,
                sisa: dataSource[idx].owing,
                pay: false,
            };
        } else {
            editedData = {
                ...dataSource[idx],
                bayar_mu: dataSource[idx].sisa,
                sisa: 0,
                pay: true,
            };
        }

        dataSource[idx] = editedData;
        gridAlokasiFakturPOS.refresh();
    };

    const checkboxTemplate = (props: any) => <input type="checkbox" checked={props.pay} onChange={(event: any) => handleCheckboxChange2(props, event.target.checked)} />;

    const [totalNilaiPOS, setTotalNilaiPOS] = useState<any>(0);

    const kalkulasiALOKASI = async () => {
        Promise.all(
            gridAlokasiFakturPOS.dataSource.map(async (item: any) => {
                item.sisa = item.owing - item.bayar_mu;
                if (item.bayar_mu > 0) {
                    item.pay = true;
                } else if (item.bayar_mu == 0) {
                    item.pay = false;
                }
            })
        ).then(() => {
    ...
    const actionCompletePOS = async (args: any) => {
        switch (args.requestType) {
            case 'save':
                if (tambah === false) {
                    const editedData = args.data;
                    gridAlokasiFakturPOS.dataSource[args.rowIndex] = editedData;
                    gridAlokasiFakturPOS.refresh();
                    kalkulasiALOKASI();
                } else if (edit) {
                    kalkulasiALOKASI();
            ...
            case 'beginEdit':
                ...
                // kalkulasiALOKASI();
                // console.log('beginEdit');
                // console.log(gridAlokasiFakturPOS.dataSource, 'gridAlokasiFakturPOS');
                break;
            case 'delete':
                gridAlokasiFakturPOS.dataSource.forEach((item: any, index: any) => {
                    item.id = index + 1;
                });
                gridAlokasiFakturPOS.refresh();
                break;
    ...
    return (
        <DialogComponent
            ...
            header={statusPage === 'CREATE' ? 'Pemasukan Lain ' : `Pemasukan Lain (EDIT) : ${kode_bm} `}
            showCloseIcon={true}
            target="#main-target"
            closeOnEscape={false}
            allowDragging={true}
            ...
            <div style={{ minWidth: '70%', overflow: 'auto' }}>
                            ...
                            >
                                ...
                                <div className="e-content">
                                    ...
                                    >
                                        <TooltipComponent openDelay={1000} target=".e-headertext">
                                            <GridComponent
                                                id="gridAlokasiFakturPOS"
                                                name="gridAlokasiFakturPOS"
                                                className="gridAlokasiFakturPOS"
                                                locale="id"
                                                ref={(g) => (gridAlokasiFakturPOS = g)}
                                                editSettings={{
                                                    allowEditing: !isButtonDisabled,
                                                }}
                                                selectionSettings={{ mode: 'Row', type: 'Single' }}
                                                ...
                                                height={215}
                                                gridLines={'Both'}
                                                rowSelecting={rowSelectingDetailPOS}
                                                actionComplete={actionCompletePOS}
                                                allowKeyboard={false}
                                            >
                                                <ColumnsDirective>
                                                    <ColumnDirective
                                                    ...
                                                    <ColumnDirective
                                                        ...
                                                        clipMode="EllipsisWithTooltip"
                                                        template={(props: any) => {
                                                            const hariValue = props.hari;
                                                            const formattedHari = hariValue < 0 ? `(${Math.abs(hariValue)})` : hariValue;
                                                            return <span>{formattedHari}</span>;
                                                        }}
                                                    />
                                                    <ColumnDirective
                                                        field="netto_mu"
                                                        ...
                                                        format="N2"
                                                        // template={(props: any) => {
                                                        //   return <span>{props.netto_mu ? parseFloat(props.netto_mu).toLocaleString('en-US', { minimumFractionDigits: 2 }) : 0}</span>;
                                                        // }}
                                                    />
                                                    <ColumnDirective
                                                        ...
                                                        //   return (
                                                        //     <span>
                                                        //       {props.total_pajak_rp
                                                        //         ? parseFloat(props.total_pajak_rp).toLocaleString('en-US', {
                                                        //             minimumFractionDigits: 2,
                                                        //           })
                                                        //         : 0}
                                                        //     </span>
                                                    ...
                                                    <ColumnDirective
                                                        field="owing"
                                                        allowEditing={false}
                                                        headerText="Sisa Nilai yang harus dibayar"
                                                        width="155"
                                                        textAlign="Right"
                                                        headerTextAlign="Center"
                                                        clipMode="EllipsisWithTooltip"
                                                        format="N2"
                                                        // template={(props: any) => {
                                                        //   return <span>{props.owing ? parseFloat(props.owing).toLocaleString('en-US', { minimumFractionDigits: 2 }) : 0}</span>;
                                                        // }}
                                                    />
                                                    <ColumnDirective
                                                        allowEditing={false}
                                                    ...
                                                    <ColumnDirective
                                                        ...
                                                        format="N2"
                                                        // template={(props: any) => {
                                                        //   return <span>{props.bayar_mu ? parseFloat(props.bayar_mu).toLocaleString('en-US', { minimumFractionDigits: 2 }) : 0}</span>;
                                                        // }}
                                                    />
                                                    <ColumnDirective
                                                        field="sisa"
                                                        ...
                                                        format="N2"
                                                        // template={(props: any) => {
                                                        //   return <span>{props.sisa ? parseFloat(props.sisa).toLocaleString('en-US', { minimumFractionDigits: 2 }) : 0}</span>;
                                                        // }}
                                                    />
                                                </ColumnsDirective>
                                                <Inject services={[Page, Selection, Edit, CommandColumn, Toolbar, Resize]} />
                                        ...
                                        <div style={{ float: 'right', marginTop: 8 }}>
                                            <div style={{ marginBottom: '10px' }}>
                                                ...
                                                >
                                                    <b>{frmNumber(totalNilaiPOS)}</b>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    {/* END ALOKASI FAKTUR POS */}
                                </div>
                            </TabComponent>
                        </div>
                    </div>
                ...
                {modalDaftarAkunHeader && (
                    ...
                    >
                        ...
                        >
                            <ColumnsDirective>
                                <ColumnDirective field="no_akun" headerText="Kode Akun" headerTextAlign="Center" textAlign="Left" width="95" clipMode="EllipsisWithTooltip" />
                                <ColumnDirective field="nama_akun" headerText="Nama Akun" headerTextAlign="Center" textAlign="Left" width="280" clipMode="EllipsisWithTooltip" />
                            </ColumnsDirective>
...
export default DialogBMPOSList;
