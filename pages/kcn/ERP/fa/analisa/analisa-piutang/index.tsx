...
import { SpreadNumber } from '../../pembayaran-uang-muka/utils';
...
function PivotToolbar() {
    ...
    const userKodeJabatan = sessionData?.kode_jabatan ?? '';
    const toolbarOptions: any = ['New', 'Save', 'SaveAs', 'Rename', 'Remove', 'Load', 'Grid', 'Chart', 'SubTotal', 'GrandTotal', userKodeJabatan === "898052050006" || userKodeJabatan === "89805040003" || userid.toLowerCase() == "administrator" ? 'Export' : ''];
    ...
    //         if (!isNaN(colIndex)) {
    //             pivotValues = pivotObj.current!.pivotValues[args.data[colIndex].rowIndex][args.data[colIndex].colIndex];
    //         }
    ...
    const [onMemberFilter, setOnMemberFilter] = useState(false);

    const divRef = useRef<any>(null); // Buat referensi ke div
    const [divWidth, setDivWidth] = useState(0); // State untuk menyimpan lebar div
    ...
    let dataSourceSettings: any  = {
        ...
        dataSource: listData,
        expandAll: false,
        filterSettings: [{ name: 'jumlah', type: 'Number', condition: 'GreaterThan', value1: '0' }],
        fieldMapping: [
            ...
            { name: 'tgl_faktur', caption: 'Tgl Faktur' },
            { name: 'jatuh_tempo', caption: 'Jatuh Tempo', dataType: 'date',format: 'dd-MM-yyyy'  },
            { name: 'tgl_tempo', caption: 'Tgl Tempo' },
        ],
    ...
    const refreshListData = async () => {
        if (kode_entitas !== null && kode_entitas !== '') {
            try {
                ...
                const formatSalesName = (name: string) => {
                ...
                const modData = responseData.map((item: any) => {
                    return {
                        ...item,
                        jumlah: SpreadNumber(item.jumlah), // Pastikan tanpaKoma mengubah string ke number
                        nama_sales: formatSalesName(item.nama_sales),
                    };
                });
                setListData(modData);
    ...
    function queryCellInfoFunction(args: any) {

        if (args.cell) {
            // Cek apakah row ini header atau data
            let colIndex = Number(args.cell.getAttribute('aria-colindex'));
            if (args.cell.cellType !== "RowHeader" && args.cell.cellType !== "ColumnHeader" && args.data[0].hasChild == true ) {


                let maxIndex = Object.keys(args.data).length
                console.log('args.data[maxIndex]',args               );
                
                if(colIndex !== 0 && colIndex != maxIndex && args.data[maxIndex - 1].columnHeaders === "") {
                    const rowIndex = args.rowIndex;
                    args.cell.style.backgroundColor = rowIndex % 2 === 0 ? "" : "#f8f4d6";
                    args.cell.style.fontWeight = 'bold';
                }
                // if(args.data[maxIndex - 1].columnHeaders === "" ) {
                //     args.cell.style.backgroundColor = ""
                // }
            //   args.cell.style.backgroundColor = rowIndex % 2 === 0 ? '' : "#f8f4d6"; // Warna selang-seling
        ...
        if (args.report && args.reportName && args.reportName !== '') {
            ...
            reports.map(function (item: any) {
                if (args.reportName === item.reportName) {
                    ...
                    // localStorage.setItem('currentSave',pivotObj.current.getPersistData())
                    isSaved = true;
                }
            });
            if (!isSaved) {
    ...
    function memberFiltering(args: any) {
    ...
    useEffect(() => {
        if(activeTab === 'grid_tab') {
            setTimeout(() => {

               pivotObj.current!.gridSettings.allowAutoResizing = false;
    ...
    return (
        <div className="control-pane h-full w-full">
            ...
            <div className="-mt-5 flex justify-between border-b border-gray-300">
                    <div>
                        <button
                    ...
                    <h2 className="text-xl font-bold">Analisa Piutang (AR)</h2>
                    {/* {activeTab === 'grid_tab' && (
                    <button
                        onClick={() => {
                            ...
                            if(currentSave) {
                                ...
                                pivotObj.current.loadPersistData(currentSave);
                            }
                        }}
                        className={`px-3 py-2 text-xs font-semibold `}
                    >
                        Load Kondisi Terakhir
                    </button>
                    )} */}
                </div>
                <div className={`flex h-[calc(100vh-230px)] w-full flex-col rounded border border-black-light bg-[#dedede] p-3 ${activeTab === 'data_tab' ? 'block' : 'hidden'}`}>
                    ...
                    <div className="flex">
                        <div className="kiri w-[35%]  p-2">
                            ...
                            <p className='mt-5'>{`* Untuk banyak pilihan gunakan tanda koma.`}</p>
                ...
                {activeTab === 'grid_tab' && (
                    <div className={`flex h-[calc(100vh-180px)] w-full flex-col rounded border border-black-light ${activeTab === 'grid_tab' ? 'block' : 'hidden'}`}>
                        <div className="control-section h-full w-full" id="pivot-table-section" style={{ overflow: 'initial', overflowX: 'auto' }} ref={divRef}>
                            <style>
                                {`
                                div.e-group-filters.e-filters.e-li b.e-droppable {
                                    height: 38.5px
                                }
                                ...
                                // .e-grid:not(.e-row-responsive, .e-left-shadow, .e-right-shadow) .e-gridcontent tr.e-row:not(.e-altrow)  {
                                //     background: #f8f4d6
                                // }
                            ...
                            <PivotViewComponent
                                ...
                                height={'100%'}
                                showFieldList={true}
                                exportAllPages={userKodeJabatan === "898052050006" || userKodeJabatan === "89805040003" || userid.toLowerCase() == "administrator"}
                                maxNodeLimitInMemberEditor={200}
                                ...
                                gridSettings={{
                                    ...
                                    width: '100%',                               
                                    queryCellInfo: queryCellInfoFunction, 
                                    // querycellinfo: excelQueryCellInfo,
                                }}
                                allowExcelExport={userKodeJabatan === "898052050006" || userKodeJabatan === "89805040003" || userid.toLowerCase() == "administrator"}
                                allowNumberFormatting={true}
                                allowConditionalFormatting={true}
                                // queryCellInfo={queryCellInfoFunction}
                                // queryCellInfo={queryCellInfoFunction}
                                allowPdfExport={userKodeJabatan === "898052050006" || userKodeJabatan === "89805040003" || userid.toLowerCase() == "administrator"}
                                showToolbar={true}
