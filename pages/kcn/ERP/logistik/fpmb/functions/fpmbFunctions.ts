import { Grid } from '@syncfusion/ej2-grids';
import { getValue } from '@syncfusion/ej2/base';
import moment from 'moment';
import Swal from 'sweetalert2';
import withReactContent from 'sweetalert2-react-content';
import JSZip from 'jszip';
import axios from 'axios';
import { myAlertGlobal2 } from '@/utils/routines';
import router from 'next/dist/client/router';

const apiUrl = process.env.NEXT_PUBLIC_LOGIN_API || '';

export const HandleTransaksiList = (index: number, transaksiChecked: boolean[], setTransaksiChecked: Function) => {
    const newTransaksiChecked = [...transaksiChecked];
    newTransaksiChecked[index] = !newTransaksiChecked[index];
    setTransaksiChecked(newTransaksiChecked);
};

export const HandlePilihSemuaTransaksiList = (transaksiChecked: boolean[], setTransaksiChecked: Function) => {
    const newTransaksiChecked = transaksiChecked.map(() => !transaksiChecked.every((checked) => checked));
    setTransaksiChecked(newTransaksiChecked);
};

export const HandlePengajuanList = (index: number, pengajuanChecked: boolean[], setPengajuanChecked: Function) => {
    const newPengajuanChecked = [...pengajuanChecked];
    newPengajuanChecked[index] = !newPengajuanChecked[index];
    setPengajuanChecked(newPengajuanChecked);
};

export const HandlePilihSemuaPengajuanList = (pengajuanChecked: boolean[], setPengajuanChecked: Function) => {
    const newTransaksiChecked = pengajuanChecked.map(() => !pengajuanChecked.every((checked) => checked));
    setPengajuanChecked(newTransaksiChecked);
};

export const HandleApprovalList = (index: number, approvalChecked: boolean[], setApprovalChecked: Function) => {
    const newApprovalChecked = [...approvalChecked];
    newApprovalChecked[index] = !newApprovalChecked[index];
    setApprovalChecked(newApprovalChecked);
};

export const HandlePilihSemuaApprovalList = (approvalChecked: boolean[], setApprovalChecked: Function) => {
    const newApprovalChecked = approvalChecked.map(() => !approvalChecked.every((checked) => checked));
    setApprovalChecked(newApprovalChecked);
};

// export const getCheckedItems = (items: any[], checked: boolean[]) =>
//     items
//         .filter((_, index) => checked[index])
//         .map((item) => item.id_jenis)
//         .join(',');

export const getCheckedItems = (items: any[], checked: boolean[], isApproval: boolean = false) => {
    const joinedItems = items.filter((_, index) => checked[index]).map((item) => item.id_jenis);

    // return isApproval ? `"${joinedItems.join('","')}"` : joinedItems.join(',');
    return isApproval ? joinedItems.join(',') : joinedItems.join(',');
};

export interface Item<T> {
    id_jenis?: T;
    label: string;
}

export const chTransaksi = [
    { id_jenis: 0, label: 'Ekspedisi' },
    { id_jenis: 1, label: 'Mobil Sendiri Kirim Gudang' },
    { id_jenis: 2, label: 'Mobil Sendiri Kirim Langsung' },
    { id_jenis: 3, label: 'GV Customer' },
    { id_jenis: 4, label: 'GV TTB' },
    { id_jenis: 5, label: 'Mutasi Full' },
];

export const chPengajuan = [
    { id_jenis: 1, label: 'Baru (Y)' },
    { id_jenis: 2, label: 'Koreksi' },
];

export const chApproval = [
    { id_jenis: 'N', label: 'Belum Approved (N)' },
    { id_jenis: 'Y', label: 'Approved (Y)' },
];

export const toggleCheckboxes = (checkedState: boolean[]) => {
    return checkedState.map(() => !checkedState.every((checked) => checked)); // Toggle all
};

export const handleCheckboxChange = (name: any, value: any, setFormListDataState: any) => {
    // console.log('handleCheckboxChange name', name);
    // console.log('handleCheckboxChange value', value);

    setFormListDataState((prevFormData: any) => ({
        ...prevFormData,
        [name]: value,
    }));
};

export const handleInputChange = (name: any, value: any, cekBoxname: any, setFormListDataState: any) => {
    // console.log('name', name);
    // console.log('value', value);
    setFormListDataState((prevFormData: any) => ({
        ...prevFormData,
        [name]: value,
        [cekBoxname]: value !== '',
        // [`checked_${name}`]: value !== '',
    }));
};

// next-dev/pages/kcn/ERP/logistik/fpmb/functions/fpmbFunctions.ts

// export const calculateCellColors = (dataRow: any) => {
//     let cellStyles = {
//         backgroundColor: 'white',
//         color: 'black',
//     };

//     if (dataRow.approval === 'Y') {
//         cellStyles.backgroundColor = '#feffd5'; //'#85FF59'; //'#D5FFFF';
//         cellStyles.color = 'green';
//     }

//     if (dataRow.pengajuan === 'Baru') {
//         cellStyles.color = 'blue';
//     } else if (dataRow.pengajuan === 'Koreksi') {
//         cellStyles.color = '#0080FF';
//     } else if (dataRow.pengajuan === 'Batal') {
//         cellStyles.color = 'red';
//     }

//     return cellStyles;
// };

// export const rowDataBound = (args: any) => {
//     const styles = calculateCellColors(args.data);
//     // args.row.style.backgroundColor = styles.backgroundColor;
//     args.row.style.color = styles.color;
// };

export const QueryCellInfoListData = (args: any) => {
    // console.log('args:', args);
    // Set default colors
    args.cell.style.color = 'black'; // Default font color
    // args.cell.style.backgroundColor = '#feffd5'; // Default background color

    const pengajuan = getValue('pengajuan', args.data);

    // console.log('approvalStatus', approvalStatus);

    if (pengajuan === 'Baru') {
        args.cell.style.color = 'blue';
    } else if (pengajuan === 'Koreksi') {
        args.cell.style.color = 'orange';
    } else if (pengajuan === 'Batal') {
        args.cell.style.color = 'red'; // Custom color
    }

    if (getValue('approval', args.data) === 'Y') {
        // args.cell.style.backgroundColor = '#feffd5'; // Default background color
        args.cell.style.color = 'green';
    }
};

export const handleSearchList = (value: any, dsGridList: Grid) => {
    if (dsGridList) {
        dsGridList.search(value);
    }
};

export const resImage =
    'data:@file/png;base64,';
export const resAudio =
    'data:@file/png;base64,';
export const resVideo =
    'data:@file/png;base64,';
export const resExcel =
    'data:@file/png;base64,';
export const resWord =
    'data:@file/png;base64,';
export const resPowerPoint =
    'data:@file/png;base64,';
export const resZip =
    'data:@file/png;base64,';
export const resUnknow =
    'data:@file/png;base64,';
export const resPdf =
    'data:@file/png;base64,';

export const handleFileImageSelect = async (args: any, jenis: any, objekImage: any, setObjekImage: Function) => {
    // const handleFileSelect = async (args: any, jenis: any) => {
    const file = args.filesData[0].rawFile;
    const fileName = file.name.toLowerCase(); // Ubah nama file ke huruf kecil
    const fileExtension = fileName.split('.').pop()?.toLowerCase(); // Ambil ekstensi file dalam huruf kecil

    if (jenis === '1') {
        setObjekImage((prevData: any) => ({
            ...prevData,
            nameImage: 'RC' + moment().format('YYMMDDHHmmss') + '.' + fileExtension,
        }));
    } else if (jenis === '2') {
        setObjekImage((prevData: any) => ({
            ...prevData,
            nameImage2: 'RC' + moment().format('YYMMDDHHmmss') + '.' + fileExtension,
        }));
    } else if (jenis === '3') {
        setObjekImage((prevData: any) => ({
            ...prevData,
            nameImage3: 'RC' + moment().format('YYMMDDHHmmss') + '.' + fileExtension,
        }));
    } else if (jenis === '4') {
        setObjekImage((prevData: any) => ({
            ...prevData,
            nameImage4: 'RC' + moment().format('YYMMDDHHmmss') + '.' + fileExtension,
        }));
    }

    const reader = new FileReader();
    // const cekImage = imageState;

    reader.onload = (e: any) => {
        if (fileExtension === 'jpg' || fileExtension === 'jpeg' || fileExtension === 'png') {
            const IDjenisGambar = document.getElementsByClassName('e-upload-files');

            if (jenis === '1') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview: e.target.result as string,
                }));
                IDjenisGambar[0]?.setAttribute('id', jenis);
            } else if (jenis === '2') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview2: e.target.result as string,
                }));
                IDjenisGambar[1]?.setAttribute('id', jenis);
            } else if (jenis === '3') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview3: e.target.result as string,
                }));
                IDjenisGambar[2]?.setAttribute('id', jenis);
            } else if (jenis === '4') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview4: e.target.result as string,
                }));
                IDjenisGambar[3]?.setAttribute('id', jenis);
            }
        } else if (fileExtension === 'xls' || fileExtension === 'xlsx') {
            if (jenis === '1') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview: resExcel,
                }));
            } else if (jenis === '2') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview2: resExcel,
                }));
            } else if (jenis === '3') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview3: resExcel,
                }));
            } else if (jenis === '4') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview4: resExcel,
                }));
            }
        } else if (fileExtension === 'doc' || fileExtension === 'docx') {
            if (jenis === '1') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview: resWord,
                }));
            } else if (jenis === '2') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview2: resWord,
                }));
            } else if (jenis === '3') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview3: resWord,
                }));
            } else if (jenis === '4') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview4: resWord,
                }));
            }
        } else if (fileExtension === 'zip' || fileExtension === 'rar') {
            if (jenis === '1') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview1: resZip,
                }));
            } else if (jenis === '2') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview2: resZip,
                }));
            } else if (jenis === '3') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview3: resZip,
                }));
            } else if (jenis === '4') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview4: resZip,
                }));
            }
        } else if (fileExtension === 'pdf') {
            if (jenis === '1') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview: resPdf,
                }));
            } else if (jenis === '2') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview2: resPdf,
                }));
            } else if (jenis === '3') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview3: resPdf,
                }));
            } else if (jenis === '4') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview4: resPdf,
                }));
            }
        } else {
            if (jenis === '1') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview: resUnknow,
                }));
            } else if (jenis === '2') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview2: resUnknow,
                }));
            } else if (jenis === '3') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview3: resUnknow,
                }));
            } else if (jenis === '4') {
                setObjekImage((prevData: any) => ({
                    ...prevData,
                    preview4: resUnknow,
                }));
            }
        }
    };

    if (file) {
        reader.readAsDataURL(file);
    }
};

export const handleRemoveImage = (jenis: any, objekImage: any, setObjekImage: Function) => {
    if (jenis === '1') {
        setObjekImage((prevData: any) => ({
            ...prevData,
            preview: null,
        }));
        const element: any = document.getElementById(jenis);
        // element.parentNode.removeChild(element);
        if (element !== null) {
            element.parentNode.removeChild(element);
        }
        // const element = document.getElementById(jenis);
        // element.querySelectorAll(`:scope > #${jenis}`);
    } else if (jenis === '2') {
        setObjekImage((prevData: any) => ({
            ...prevData,
            preview2: null,
        }));
        // const element: any = document.querySelector('.e-upload-file-list');
        const element: any = document.getElementById(jenis);
        if (element !== null) {
            element.parentNode.removeChild(element);
        }
    } else if (jenis === '3') {
        setObjekImage((prevData: any) => ({
            ...prevData,
            preview3: null,
        }));
        // const element: any = document.querySelector('.e-upload-file-list');
        const element: any = document.getElementById(jenis);
        if (element !== null) {
            element.parentNode.removeChild(element);
        }
    } else if (jenis === '4') {
        // setPreview4(null);
        // setObjekImage({
        //     ...imageState,
        //     preview4: null,
        // });
        setObjekImage((prevData: any) => ({
            ...prevData,
            preview4: null,
        }));
        // const element: any = document.querySelector('.e-upload-file-list');
        const element: any = document.getElementById(jenis);
        if (element !== null) {
            element.parentNode.removeChild(element);
        }
    } else if (jenis === 'all') {
        withReactContent(Swal)
            .fire({
                html: 'Apakah anda akan membersihkan semua gambar?',
                width: '20%',
                target: '#FrmPraBkk',
                showCancelButton: true,
                confirmButtonText: '<p style="font-size:10px">Ya</p>',
                cancelButtonText: '<p style="font-size:10px">Tidak</p>',
            })
            .then((result: any) => {
                if (result.isConfirmed) {
                    setObjekImage((prevData: any) => ({
                        ...prevData,
                        preview: null,
                        preview2: null,
                        preview3: null,
                        preview4: null,
                    }));
                    const elements: any = document.getElementsByClassName('e-upload-file-list');
                    while (elements.length > 0) {
                        elements[0].parentNode.removeChild(elements[0]);
                    }
                } else {
                    console.log('cancel');
                }
            });
    }
};

export const handleUpload = async (
    kode_dokumen: any,
    imageState: any,
    kode_entitas: any,
    masterDataState: any,
    masterKodeDokumen: any,
    uploaderRef: any,
    uploaderRef2: any,
    uploaderRef3: any,
    uploaderRef4: any,
    jsonImageEdit: any,
    dokumen: any
) => {
    // console.log({
    //     kode_dokumen,
    //     imageState,
    //     kode_entitas,
    //     masterDataState,
    //     masterKodeDokumen,
    //     uploaderRef,
    //     uploaderRef2,
    //     uploaderRef3,
    //     uploaderRef4,
    //     jsonImageEdit,
    //     dokumen,
    // });
    if (uploaderRef.current && uploaderRef2.current && uploaderRef3.current && uploaderRef4.current) {
        const filesArray = [
            imageState.preview ? [{ rawFile: await fetch(imageState.preview).then((res) => res.blob()), fileName: imageState.nameImage }] : uploaderRef.current.getFilesData(), // ubah menjadi fileuploader
            imageState.preview2 ? [{ rawFile: await fetch(imageState.preview2).then((res) => res.blob()), fileName: imageState.nameImage2 }] : uploaderRef2.current.getFilesData(), // ubah menjadi fileuploader
            imageState.preview3 ? [{ rawFile: await fetch(imageState.preview3).then((res) => res.blob()), fileName: imageState.nameImage3 }] : uploaderRef3.current.getFilesData(), // ubah menjadi fileuploader
            imageState.preview4 ? [{ rawFile: await fetch(imageState.preview4).then((res) => res.blob()), fileName: imageState.nameImage4 }] : uploaderRef4.current.getFilesData(), // ubah menjadi fileuploader
        ];
        const nameImages = [imageState.nameImage, imageState.nameImage2, imageState.nameImage3, imageState.nameImage4];
        const zip = new JSZip();
        let filesAdded = false;

        filesArray.forEach((files, index) => {
            if (files.length > 0) {
                const file = files[0].rawFile;
                zip.file(nameImages[index], file);
                filesAdded = true;
            }
        });
        // console.log(filesAdded);
        if (filesAdded) {
            try {
                const zipBlob = await zip.generateAsync({ type: 'blob' });
                const formData = new FormData();
                formData.append('myimage', zipBlob, `IMG_${kode_dokumen}.zip`);
                formData.append('ets', kode_entitas);
                formData.append('nama_file_image', `IMG_${kode_dokumen}.zip`); // dari response simpan ju

                const response = await axios.post(`${apiUrl}/upload`, formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                });

                if (response.status === 200) {
                    const result = response.data;
                    // console.log('Upload successful:', result);
                    // myAlert('Berhasil Upload');

                    const createJson = (id: any, nameImage: any) => ({
                        entitas: kode_entitas,
                        kode_dokumen: kode_dokumen, // dari response simpan
                        id_dokumen: id,
                        dokumen: dokumen,
                        filegambar: nameImage,
                        fileoriginal: 'defaulf-file',
                    });

                    const jsonZip = {
                        entitas: kode_entitas,
                        kode_dokumen: kode_dokumen, // dari response simpan
                        id_dokumen: '999', //default
                        dokumen: dokumen,
                        filegambar: `IMG_${kode_dokumen}.zip`, // dari response simpan
                        fileoriginal: 'defaulf - file.zip',
                    };

                    const combinedArray = filesArray.map((files, index) => (files.length > 0 ? createJson(index + 1, nameImages[index]) : null)).filter(Boolean);

                    combinedArray.push(jsonZip);
                    // jika edit
                    // cari id yang tidak ada disini untuk dihapus
                    if (masterDataState === 'EDIT') {
                        const combinedIds = combinedArray.map((item: any) => item.id_dokumen.toString());
                        const editIds = jsonImageEdit.map((item: any) => item.id_dokumen.toString());
                        const missingIds = editIds.filter((id: any) => !combinedIds.includes(id));
                        const paramsArrayDelete = missingIds.join(',');

                        if (paramsArrayDelete !== '') {
                            //DELETE TB_IMAGES
                            try {
                                const response = await axios.delete(`${apiUrl}/erp/hapus_file_pendukung`, {
                                    params: {
                                        entitas: kode_entitas,
                                        param1: masterKodeDokumen,
                                        param2: paramsArrayDelete, // Sesuaikan dengan data yang diperlukan untuk menghapus
                                    },
                                });
                                // console.log('Response dari penghapusan file pendukung:', response.data);
                            } catch (error) {
                                console.error('Error saat menghapus file pendukung:', error);
                            }
                        } else {
                            console.log('tidak ada yg perlu dihapus');
                        }
                    }

                    // INSERT
                    try {
                        // Simpan data ke database
                        const insertResponse = await axios.post(`${apiUrl}/erp/simpan_tbimages`, combinedArray);
                        // console.log('simpan fungsi ke database image');
                        return insertResponse.data;
                    } catch (error) {
                        console.error('Error saat menyimpan data baru:', error);
                    }
                } else {
                    console.error('Upload failed:', response.statusText);
                }
            } catch (error) {
                console.error('Error uploading file:', error);
            }
        } else {
            // console.log('Tanpa Gambar // tidak ada perubahan');

            //kondisi jika gambar dihapus semua

            //hapus semua
            if (jsonImageEdit.length > 0) {
                try {
                    const response = await axios.delete(`${apiUrl}/erp/hapus_file_pendukung`, {
                        params: {
                            entitas: kode_entitas,
                            param1: masterKodeDokumen,
                            param2: '1,2,3,4,999', // Sesuaikan dengan data yang diperlukan untuk menghapus
                        },
                    });
                    // console.log('Response dari penghapusan file pendukung:', response.data);
                } catch (error) {
                    console.error('Error saat menghapus file pendukung:', error);
                }
            }
        }
    } else {
        console.log('Uploader refs are not defined');
    }
};

// export const handleFPMBSubmit = async ({ statusPengajuan, pAppFPMBMBLKL, kodeMB, noFPMB, files }: FPMBSubmitParams) => {
export const handleFPMBSubmit = (statusPengajuan: any, pAppFPMB: any, noFPMB: any, files: any, objectImage: any) => {
    try {
        // Check if status is 'Koreksi'
        if (statusPengajuan === 'Koreksi') {
            myAlertGlobal2('Status FPMB sudah koreksi. Mohon lakukan koreksi hasil rencek di FASTD.', '#fpMb');
            // throw new Error('Status FPMB sudah koreksi. Mohon lakukan koreksi hasil rencek di FASTD.');
        }

        // Validate required files
        if (objectImage.preview === null) {
            myAlertGlobal2('File pendukung Surat Jalan belum diisi.', '#frmFpmb');
            // throw new Error('File pendukung Surat Jalan belum diisi.');
        }

        if (objectImage.preview2 === null) {
            myAlertGlobal2('File pendukung Foto Barang belum diisi.', '#frmFpmb');
            // throw new Error('File pendukung Barang belum diisi.');
        }

        // Check existing FPMB
        // const existingFPMB = await checkExistingFPMB(kodeMB);
        // if (existingFPMB && pAppFPMBMBLKL !== 'Approval' && pAppFPMBMBLKL !== 'UnApproval' && statusPengajuan !== '2') {
        //     throw new Error(`Proses Pengajuan FPMB tidak dapat dilakukan. FPMB sudah dibuat dengan No. Pengajuan: ${existingFPMB.noFPMB}`);
        // }

        // // Handle Approval
        // if (pAppFPMBMBLKL === 'Approval') {
        //     const confirmed = await confirmDialog('Apakah data pengajuan mutasi barang ini akan disetujui?');
        //     if (confirmed) {
        //         await updateFPMBApproval({
        //             kodeMB,
        //             approval: 'Y',
        //             userId: getCurrentUserId(),
        //             date: new Date(),
        //         });
        //     }
        // }

        // Handle file uploads
        // await uploadFPMBFiles(files, noFPMB);

        // // Update MB status
        // await updateMBStatus({
        //     kodeMB,
        //     pengajuan: '1',
        //     userId: getCurrentUserId(),
        // });

        return true;
    } catch (error) {
        console.error('FPMB submission error:', error);
        throw error;
    }
};
