...
const SupplierModal: React.FC<SupplierModalProps> = ({ isOpen = false, onClose, onSelectData, userid, kode_entitas, handleNamaSupp, nilaiTotalId, jenisBarang, vRefreshData }) => {
    type ListSupplier = {
        kode_supp: string;
        kode_mu: string;
        no_supp: string;
    ...
    console.log('vRefreshData.current += 1;', vRefreshData);
        ...
        const fetchDataUseEffect = async () => {
            const response = await axios.get(`${apiUrl}/erp/m_supplier_filter`, {
                params: {
                    ...
                    param1: jenisBarang === 'nonPersediaan' ? 'E' : 'all',
                },
            });
    ...
    const handleSelectData = (
        selectedData: string,
        selectedNamaTermin: string,
        selectedKodeSupp: string,
        selectedKodeTermin: string,
    ...
    const searchDataNama = (keyword: any) => {
        if (keyword === '') {
            ...
            const filteredData = recordListSupp.filter((item) => item.nama_relasi.toLowerCase().startsWith(keyword.toLowerCase()));
            return filteredData;
        }
    };
    ...
    const searchDataNoSupp = (keyword: any) => {
        ...
        if (keyword === '') {
            ...
            const filteredData = recordListSupp.filter((item) => item.no_supp.toLowerCase().startsWith(keyword.toLowerCase()));
            return filteredData;
        }
    };
