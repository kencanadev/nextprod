...
const spp = () => {
    ...
    const fetchData = async (kode_sp: any, name: any, tipe: any) => {
        ...
        if (responsData.status === true) {
            ...
            if (tipe === 'batal') {
                Promise.all(
                    response.data.data
                        .filter((item: any) => item.qty_sisa > 0)
                        .map((item: any) => {
                            return {
                                id: item.id_sp,
    ...
    const handleBatal = () => {
        const tipe = router.query.tipe;
        const tabIdx1 = router.query.tabIdx1;
        const tabIdx2 = router.query.tabIdx2;
        const tabIdx3 = router.query.tabIdx3;
        const encode = Buffer.from(
        `vTanggalChecked=${routeTanggalChecked}&vTipeDokumen=${routeTipeDokumen}&tglAwal=${routeTglAwal}&tglAkhir=${routeTglAkhir}&tglBerlaku1=${routeTglBerlaku1}&tglBerlaku2=${routeTglBerlaku2}&tglKirim1=${routeTglKirim1}&tglKirim2=${routeTglKirim2}&noPo=${routeNoPo}&namaSupp=${routeNamaSupp}&namaBarang=${routeNamaBarang}&isPoKontrakChecked=${routeIsPoKontrakChecked}&isPoNonKontrakChecked=${routeIsPoNonKontrakChecked}&isPoBarangProduksiChecked=${routeIsPoBarangProduksiChecked}&isPoDenganPajakChecked=${routeIsPoDenganPajakChecked}&isKirimanLangsungChecked=${routeIsKirimanLangsungChecked}&isPembatalanOrderChecked=${routeIsPembatalanOrderChecked}&isBelumAccDireksiChecked=${routeIsBelumAccDireksiChecked}&isSudahAccDireksiChecked=${routeIsSudahAccDireksiChecked}&noPoChecked=${routeNoPoChecked}&namaSuppChecked=${routeNamaSuppChecked}&namaBarangChecked=${routeNamaBarangChecked}&statusDok=${routeStatusDok}&statusApp=${routeStatusApp}`
        ).toString('base64');
        if (tipe === 'dashboard') {
        // router.back();
        if (tabIdx1 === 'outstanding_pekerjaan') {
            router.push({ pathname: '/kcn/ERP/dashboard/pengadaan-barang', query: { tabIdx1: tabIdx1, tabIdx2: tabIdx2, tabIdx3: tabIdx3 } });
        } else {
            router.push({ pathname: '/kcn/ERP/dashboard/pengadaan-barang', query: { tabIdx1: tabIdx1 } });
        }
        } else {
    ...
    const backPage = () => {
        ...
        // router.back();
        if (tabIdx1 === 'outstanding_pekerjaan') {
            router.push({ pathname: '/kcn/ERP/dashboard/pengadaan-barang', query: { tabIdx1: tabIdx1, tabIdx2: tabIdx2, tabIdx3: tabIdx3, noDokPO : noPOValue } });
        } else {
            router.push({ pathname: '/kcn/ERP/dashboard/pengadaan-barang', query: { tabIdx1: tabIdx1 } });
        }
        } else {
    ...
    return (
        <div style={{ maxWidth: '130.33vw', maxHeight: '40vw' }} className={`${stylesIndex.scale75Monitor}`}>
            {/* Form Grid Layouts */}
