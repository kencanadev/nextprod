...
const GridStok = ({
    ...
    const cellQuery = (args: any) => {
        if (args.column.field === 'order_penjualan') {
            if (args.data[args.column.field] < 0) {
                args.cell.style.color = 'red';
            }
        }
        if (args.column.field === 'jumlah_stok_tersedia') {
            if (args.data[args.column.field] < 0) {
                args.cell.style.color = 'red';
            }
        }
        if (args.column.field === 'order_pembelian') {
            if (args.data[args.column.field] < 0) {
                args.cell.style.color = 'red';
            }
        }
        if (args.column.field === 'harga1') {
            if (args.data[args.column.field] < 0) {
                args.cell.style.color = 'red';
            }
        }
        if (args.column.field === 'harga2') {
            if (args.data[args.column.field] < 0) {
                args.cell.style.color = 'red';
            }
        }
        if (args.column.field === 'harga3') {
            if (args.data[args.column.field] < 0) {
                args.cell.style.color = 'red';
            }
        }
        if (args.column.field === 'berat') {
            if (args.data[args.column.field] < 0) {
                args.cell.style.color = 'red';
            }
        }

        {
            gudangTerpilihData.map((item: any) => {
                if (args.column.field === formatKey(item.nama_gudang)) {
                    if (args.data[args.column.field] < 0) {
                        args.cell.style.color = 'red';
                    }
                }
    ...
    const customTotJumlahMu1 = (props: any) => {
        console.log('props', props);

        return <span style={{ fontWeight: 'bold' }}>{formatNumber(String(SpreadNumber(String(props.Sum))))}</span>;
    };

    const customTotJumlahMu2 = (props: any) => {
        console.log('props', props);

        return <span style={{ fontWeight: 'bold' }}>Jumlah Barang : {formatNumber(String(SpreadNumber(String(props.Count))))}</span>;
    };
    ...
    const sortingOptions: any = {
        columns: [{ field: 'no_item', direction: 'Ascending' }],
    };
    ...
    return (
        ...
        >
            <ColumnsDirective>
                ...
                <ColumnDirective field="stok_minimal" format={'N'} width="80" headerText={formatString('minimal')} headerTextAlign="Center" textAlign="Right" clipMode="EllipsisWithTooltip" />
            ...
            {gudangTerpilihData.length !== 0 && (
                <AggregatesDirective>
                    <AggregateDirective>
                        <AggregateColumnsDirective>
                            <AggregateColumnDirective field={'no_item'} type="Count" footerTemplate={customTotJumlahMu2} />
                            {gudangTerpilihData.map((item: any) => (
                                <AggregateColumnDirective key={item.nama_gudang} field={formatKey(item.nama_gudang)} type="Sum" footerTemplate={customTotJumlahMu1} />
                            ))}
                            <AggregateColumnDirective field={'order_penjualan'} type="Sum" footerTemplate={customTotJumlahMu1} />
                            ...
                            <AggregateColumnDirective field={'order_pembelian'} type="Sum" footerTemplate={customTotJumlahMu1} />
                            {/* <AggregateColumnDirective field="jumlah_stok_tersedia" type="Custom" customAggregate={totJumlahPabrik} footerTemplate={CustomSumJumlahpabrik} /> */}
                            {/* <AggregateColumnDirective field={'jumlah_ton'} type="Sum" format="N0" footerTemplate={customTotJumlahMu1} /> */}
                            {/* {/* <AggregateColumnDirective field="jumlah_stok_tersedia" type="Custom" customAggregate={totJumlahPabrik} footerTemplate={CustomSumJumlahpabrik} /> */}
                        </AggregateColumnsDirective>
                    </AggregateDirective>
                </AggregatesDirective>
