...
const KartuStockGudang = ({
  ...
  const [pilihGudang, setPilihGudang] = useState('');
  const getStockAll = async () => {
    ...
    const iframeSrc = `/kcn/ERP/inventory/informasi-stok-aktual/report/kartu_stock_gudang?entitas=${kode_entitas}&param1=${1}&param2=${moment(stateStokAll?.tanggal_awal_stok_all).format(
      'YYYY-MM-DD'
    )}&param3=${moment(stateStokAll?.tanggal_akhir_stok_all).format('YYYY-MM-DD')}&param4=${stateStokAll?.isNamaBarang ? 'Y' : 'N'}&param5=${pilihGudang}&param6=${
  ...
  const getDefaulGudang = async () => {
    const getDeffGudang = await axios.get(`${apiUrl}/erp/preferensi?entitas=${kode_entitas}`);
    setPilihGudang(getDeffGudang.data.data[0].kode_gudang);
  };
  useEffect(() => {
    getDefaulGudang();
  }, []);
        ...
        <div className="flex h-[75%] w-full flex-col gap-2">
          ...
          <div className="mb-1 flex w-full flex-col items-start">
            ...
            <select
              ...
              defaultValue={selectedRow?.kode_gudang}
              value={pilihGudang}
              onChange={(e) => setPilihGudang(e.target.value)}
            ...
            >
              ...
              {listGudang.map((item: any, index: number) => (
                <option value={item.kode_gudang} key={index}>
                  {item.nama_gudang}
                </option>
              ))}
            </select>
...
export default KartuStockGudang;
