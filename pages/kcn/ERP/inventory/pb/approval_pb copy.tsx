...
const ApprovalPB = () => {
    ...
    let routeViewValue: any, routeKodePBValue: any, routeStatusValue: any, routeFilePendukungValue: any;
    //encode url
    ...
    if (typeof str === 'string') {
        ...
        const { kode_lpb, view, status, updatefilependukung } = queryParams;
    ...
    const saveDoc = async (auth: any) => {
        ...
        // END CEK DETAIL PAJAK
        console.log('Masuk saveDoc Tidak');
        ...
        // console.log('Masuk saveDoc Tidak 2');
        ...
        const cekStokPB = async () => {
            ...
            //             },
            //         });
            //         console.log('item.kode_item');
            //         console.log(item.kode_item);

            //         console.log('responseCekStok.data.data');
            //         console.log(responseCekStok.data.data[0]);
        ...
        console.log('Masuk saveDoc Tidak 3 = ', S_blockingDetailPajak);
        console.log('modifiedDataJurnal.nodes.length = ', modifiedDataJurnal.nodes.length);
        console.log('modifiedDataJurnal.nodes[0].jumlah_mu = ', modifiedDataJurnal.nodes[0].jumlah_mu);
        console.log('blockingJurnal = ', blockingJurnal);
        console.log('fdo = ', fdo);
        ...
        } else if ((modifiedDataJurnal.nodes.length === 1 && modifiedDataJurnal.nodes[0].jumlah_mu === 0) || modifiedDataJurnal.nodes[0].jumlah_mu === 0) {
        ...
        } else if (fdo === 'default' || fdo === null || fdo === '') {
            console.log('Masuk saveDoc Tidak 7');
            Swal.fire({
                title: `Warning`,
                text: 'Barang untuk direalisasikan ke cabang?',
                showCancelButton: true,
                confirmButtonText: 'Yes',
                cancelButtonText: 'No',
            }).then(async (result) => {
                // console.log('Masuk saveDoc Tidak 8');
                if (result.isConfirmed) {
                    // console.log('Masuk saveDoc Tidak 9 = ', blockingGudang, blockingStok, auth);
                    // setFDO('P');
                    if (auth !== 'skipBlokAuth' && blockingGudang === true && blockingStok === true) {
                        // console.log('lanjut mang');
                        Swal.fire({
                            title: 'Warning',
                            ...
                            if (result.isConfirmed) {
                                // console.log('OK, Lanjutkan dengan stok opname');
                                // router.push('../../main/main');
                            } else if (result.dismiss === Swal.DismissReason.cancel) {
                                // console.log('Permintaan otorisasi user untuk proses');
                                setModalLogin(true);
                            }
                        });
                    } else {
                        ...
                        if (hasEmptyFields) {
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    let responseExportNonKontrakCabangApi;
                                    let responseExportKontrakPusatApi;
                                    if (response.data.status === true) {
                                        //     Await the validation approval
                                        await validasiApproval().then(async (result: any) => {
                                            console.log('result validasiApproval ', result);
                                            if (result === 'exportNonKontrak') {
                                                ...
                                                await ExportNonKontrakCabang(modifiedData)
                                                    .then(async (result: any) => {
                                                        ...
                                                        const responseExportNonKontrak = await axios
                                                                ...
                                                                if (result) {
                                                                    ...
                                                                    if (responseExportNonKontrakCabangApi.kodeDokumenPembeli?.kodeSPM !== '') {
                                                                        await generateNUDivisi(
                                                                            ...
                                                                            moment().format('YYMM') + sSoKodeJual
                                                                        );
                                                                    }
                                                            ...
                                                            .catch((e: any) => {
                                                                responseExportNonKontrakCabangApi = e.response.data;
                                                            });

                                                        console.log('responseExportNonKontrak', responseExportNonKontrak);
                                                    })
                                                    .catch((e: any) => {
                                                ...
                                                await ExportKontrakPusat(modifiedData)
                                                    .then(async (result: any) => {
                                                        ...
                                                        console.log('responseExportNonKontrak', responseExportKontrakPusat);
                                                    })
                                                    .catch((e: any) => {
                                                        console.log('error', e.response.data);
                                                    });
                                        ...
                                        //UPLOAD handleUpload NANTI SETELAH BERHASIL EXPORT
                                        ...
                                        Swal.fire({
                                            title: 'Warning',
                                            text: `Gagal Approval PB : ${response.data.serverMessage}`,
                                            icon: 'error',
                                        });
                                    }
        ...
        // if (fdo === 'P') {
        //     console.log('LANJUT PROSES OTOMATIS');
        // } else {
        //     console.log('BERHENTI PROSES OTOMATIS');
        // }
        // if (auth !== 'skipBlokAuth' && blockingGudang === true && blockingStok === true) {
        //     console.log('lanjut mang');
        //     Swal.fire({
        ...
        //         // detail_jurnal: modifiedDataJurnal.nodes,
        //     };
        //     // console.log('🚀 ~ saveDoc ~ modifiedData:', modifiedData);
        //     // console.log(routeStatusValue);
        //     // console.log(totalPotonganRP);
        ...
        //             // Await the validation approval
        //             await validasiApproval().then((result: any) => {
        //                 if (result === 'exportNonKontrak') {
        //                     console.log('masuk exportNonKontrak');
        //                     ExportNonKontrakCabang(modifiedData);
        //                 } else if (result === 'exportKontrakPusat') {
        //                     console.log('masuk exportKontrakPusat');
        //                 }
        //                 // Handle export logic based on the flags
        //                 // if (result.exportNonKontrak) {
        //                 //     console.log('masuk exportNonKontrak');
        //                 //     ExportNonKontrakCabang(modifiedData);
        //                 // } else if (exportKontrakPusat) {
        //                 //     console.log('masuk exportKontrakPusat');
        //                 //     // Add logic for exportKontrakPusat if needed
        //                 // }
        //             });
        //             // const response = await axios.patch(`${apiUrl}/erp/update_approval_pb`, modifiedData);
        //             // if (response.data.status === true) {
    ...
    const handleFileSelect = (index: any, file: any, imageserver: any, dokumen: any) => {
        // console.log(index);
        // console.log(file);
    ...
    const handleFileSelectPO = (index: any, file: any, imageserver: any, dokumen: any) => {
        if (dokumen === 'PO') {
            setSelectedRowsImageJenis('server');
    ...
    const ExportNonKontrakCabang = async (jsonApprovePb: any) => {
        ...
        const vPBCabangBeliJson = {
            pb: {
                ...
                audit: {
                    ...
                    tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                    proses: 'APPROVAL PB',
                    diskripsi: `Approval PB item = ${data.nodes.length} total_berat = ${totalBeratHeader} nilai transaksi ${totalNettoRPSet.toLocaleString('en-US', {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                    })}`,
                    userid: userid, // userid login web
            ...
            const objectFbCabangBeli = {
                fb: {
                    ...
                    audit: {
                        ...
                        tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                        proses: 'NEW',
                        diskripsi: `Auto FB Approval disetujui item =  ${itemFbCabangBeli}  nilai transaksi ${totalNettoRPSet.toLocaleString('en-US', {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                        })}`,
                        userid: userid, // userid login web
                ...
                const objectSpmCabangBeli = {
                    spm: {
                        ...
                        audit: {
                            ...
                            tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                            proses: 'NEW',
                            diskripsi: `Auto SPM item =  ${itemSPMCabangBeli}  total berat ${sTotalBeratSoCabang.toLocaleString('en-US', {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                            })}`,
                            userid: userid, // userid login web
        ...
        const SJCabangBeliJson = async () => {
            ...
            console.log('sDetailSJCabangBeli ccc', sDetailSJCabangBeli);

            if (dariFpb) {
                sNoSjCabangBeli = await generateNUDivisi(kode_entitas, '', sSoKodeJual, '12', moment().format('YYYYMM'), moment().format('YYMM') + `${sSoKodeJual}`);
                let sDetailSJCabangBeli = await vDetailSJCabangBeli();

                // console.log('sDetailSJCabangBeli', sDetailSJCabangBeli);
                // console.log('masuk pa eko dariFpb', dariFpb);

                const sTotalMUSoCabang = sDetailSJCabangBeli?.every((item: any) => item === null)
                    ? null
                    : sDetailSJCabangBeli?.reduce((total: any, item: any) => {
                          return total + parseFloat(item?.jumlah_m || 0);
                      }, 0);
                console.log('masuk pa eko dariFpb', dariFpb);

                const detailjsonApprovePb = await jsonApprovePb.detail;
                // console.log('detailjsonApprovePb', detailjsonApprovePb);
                const totalBerat = detailjsonApprovePb?.every((item: any) => item === null)
                    ? null
                    : detailjsonApprovePb?.reduce((total: any, item: any) => {
                ...
                if (getMasterSo.length > 0) {
                    ...
                    const objectSjCabangBeli = {
                        sj: {
                            ...
                            pengemudi: pengemudi,
                            nopol: nopol,
                            total_rp: sTotalMUSoCabang ?? 0, //sTotalMUSoCabang,
                            total_diskon_rp: 0,
                            total_pajak_rp: 0,
                            netto_rp: sTotalMUSoCabang ?? 0,
                            total_berat: sTotalBeratSoCabang, //sTotalBeratSoCabang,
                            ...
                            audit: {
                                ...
                                tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                                proses: 'NEW',
                                diskripsi: `Auto SJ item =  ${itemSJCabangBeli}   total berat = ${sTotalBeratSoCabang} nilai transaksi ${sTotalMUSoCabang.toLocaleString('en-US', {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                })}`,
                                userid: userid, // userid login web
                    ...
                    const objectFjCabangBeli = {
                        fj: {
                            ...
                            jurnal: [
                                {
                                    ...
                                    kurs: kurs,
                                    debet_rp: sTotalMUSoCabang * kurs - 0 * kurs,
                                    kredit_rp: 0,
                                    jumlah_rp: sTotalMUSoCabang * kurs - 0 * kurs,
                                    jumlah_mu: sTotalMUSoCabang * kurs - 0 * kurs,
                                ...
                                },
                                {
                                    ...
                                    kurs: kurs,
                                    debet_rp: 0,
                                    kredit_rp: sTotalMUSoCabang * kurs + potonganFJCabang * kurs,
                                    jumlah_rp: -(sTotalMUSoCabang * kurs) + potonganFJCabang * kurs,
                                    jumlah_mu: -(sTotalMUSoCabang * kurs) + potonganFJCabang * kurs,
                            ...
                            audit: {
                                ...
                                tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                                proses: 'NEW',
                                diskripsi: `Auto FJ item =  ${itemFJCabangBeli} nilai transaksi ${totalNettoRPSet.toLocaleString('en-US', {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                })}`,
                                userid: userid, // userid login web
                ...
                const objectPbPusat = {
                    pb: {
                        ...
                        audit: {
                            ...
                            tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                            proses: 'NEW',
                            diskripsi: `Auto PB Approval item =  ${itemPbPusat}  total berat ${sTotalBeratPOOS} nilai transaksi ${totalNettoRPSet.toLocaleString('en-US', {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                            })}`,
                            userid: userid, // userid login web
                ...
                const objectFbPusat = {
                    fb: {
                        ...
                        audit: {
                            ...
                            tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                            proses: 'NEW',
                            diskripsi: `Auto FB Approval disetujui item =  ${itemFbPusat} nilai transaksi ${totalNettoRPSet.toLocaleString('en-US', {
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                            })}`,
                            userid: userid, // userid login web
                    ...
                    const objectSpmPusat = {
                        spm: {
                            ...
                            audit: {
                                ...
                                tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                                proses: 'NEW',
                                diskripsi: `Auto SPM item =  ${itemSPMPusat}  total berat ${sTotalBeratSoPusat.toLocaleString('en-US', {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                })}`,
                                userid: userid, // userid login web
                    ...
                    const objectSjPusat = {
                        sj: {
                            ...
                            jurnal: [
                                {
                                    ...
                                    kode_subledger: null,
                                    kurs: kurs,
                                    debet_rp: totalNilaiHpp,
                                    kredit_rp: 0,
                                    jumlah_rp: totalNilaiHpp,
                                    jumlah_mu: totalNilaiHpp,
                            ...
                            audit: {
                                ...
                                tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                                proses: 'NEW',
                                diskripsi: `Auto SJ item =  ${itemSJPusat} total berat = ${sTotalBeratSoPusat} nilai transaksi ${totalNettoRPSet.toLocaleString('en-US', {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                })}`,
                                userid: userid, // userid login web
                    ...
                    const objectFjPusat = {
                        fj: {
                            ...
                            audit: {
                                ...
                                tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                                proses: 'NEW',
                                diskripsi: `Auto FJ item =  ${itemFJPusat} nilai transaksi ${totalNettoRPSet.toLocaleString('en-US', {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                })}`,
                                userid: userid, // userid login web
        ...
        console.log('jsonApprovalPb ', jsonApprovalPb);
    ...
    const ExportKontrakPusat = async (jsonApprovePb: any) => {
        ...
        const MBCabangBeliJson = async () => {
            ...
            if (sDetailMBCabangBeli.length >= 1) {
                const formatTgl = moment(TglPB).format('YYYY-MM-DD');
                tglBackTimeDokumen = await ResetTime2(kode_entitas, formatTgl);
                                                ...
                                                <table style={{ width: '100%' }}>
                                                    ...
                                                    <tbody>
                                                        {selectedFiles.map((file: any, index: any) => (
                                                            <tr
                                                                key={index}
                                                                onClick={() => handleFileSelect(index, file.name, file.decodeBase64_string, 'PB')}
                                                ...
                                                <table style={{ width: '100%' }}>
                                                    ...
                                                    <tbody>
                                                        {selectedFilesPO.map((file: any, index: any) => (
                                                            <tr
                                                                key={index}
                                                                onClick={() => handleFileSelectPO(index, file.name, file.decodeBase64_string, 'PO')}
