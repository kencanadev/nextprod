...
const AkunDlg: React.FC<AkunDlgProps> = ({ isOpen = false, onClose, userid, kode_entitas, searchtype, cariNo, cariNama, onSelectData, tipeValue }: AkunDlgProps) => {
    ...
    const handleOKClick = () => {
        if (selectedheader === 'Y') {
            // alert(`No. Akun ${selectedno_akun} adalah akun induk tidak bisa digunakan untuk transaksi`);
            Swal.fire({
                title: 'Warning',
                text: `No. Akun ${selectedno_akun} adalah akun induk tidak bisa digunakan untuk transaksi.`,
                icon: 'warning',
            });
        } else {
            const dataObject = {
