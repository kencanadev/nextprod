...
const DialogTtbList: React.FC<dialogTtbListProps> = ({
        ...
        if (kode_entitas !== null || kode_entitas !== '') {
            try {
                if (masterDataState == 'BARU') {
                    ...
                    setApiResponseGudang([...gudangTtb, customData]);
                        ...
                        Promise.all(
                            responsData.data.map((item: any) => {
                                return {
                                    ...
                                    satuan: item.satuan,
                                    // kuantitas: parseFloat(parseFloat(item.qty).toFixed(3)),
                                    kuantitas: SpreadNumber(item.qty),
                                    kodecabang: '',
                                    keterangan: item.ket,
                                    ...
                                    // jml_mak: item.jml_mak.toFixed(3),
                                    jml_mak: SpreadNumber(item.jml_mak),
                                    ...
                                    // qty_std: item.qty_std.toFixed(3),
    ...
    const DialogDaftarSj = async () => {
        ...
        // await RefreshDetailSj(kode_entitas, 'all', refKodeCust.current, setDataDetailSj, entitas);
        await RefreshDetailSj(kode_entitas, '', custSelectedKode === '' ? '' : custSelectedKode, setDataDetailSj, entitas);
        ...
        if (filterCondition === true) {
            ...
            await setDataBarang((state: any) => {
                ...
                const newItem = {
                    ...
                    // kuantitas: item[0].jml_mak.toFixed(3),
                    kuantitas: SpreadNumber(item[0].jml_mak),
                    kodecabang: '',
                    keterangan: '',
                    ...
                    // jml_mak: item[0].jml_mak.toFixed(3),
                    jml_mak: SpreadNumber(item[0].jml_mak),
                    ...
                    // qty: item[0].qty.toFixed(3),
                    // qty_std: item[0].qty_std.toFixed(3),
                    qty: SpreadNumber(item[0].qty),
                    qty_std: SpreadNumber(item[0].qty_std),
    ...
    const handleClickDaftarSj = async (dataItem: any) => {
        ...
        if (entitas === '898') {
            refKodeGudang.current = dataItem[0].kode_gudang;
            refNamaGudang.current = dataItem[0].nama_gudang;
        } else {
            if (dataItem[0].nama_gudang === 'GV. LANGSUNG' || dataItem[0].nama_gudang === 'GV. TTB') {
                refKodeGudang.current = dataItem[0].kode_gudang;
                refNamaGudang.current = dataItem[0].nama_gudang;
            } else {
                refKodeGudang.current = '';
        ...
        Promise.all(
            filteredData.map((item: any) => {
                ...
                return {
                    ...
                    kuantitas: SpreadNumber(item.jml_mak),
                    kodecabang: '',
                    keterangan: '',
            ...
            if (filteredData.length > 0) {
                ...
                setCatatanValue(!firstItem.keterangan ? dataItem[0].keterangan : firstItem.keterangan);
    ...
    function hasTabIndex0And1(selectedFiles: any[]): boolean {
        const tabIndexes = selectedNamaFiles.map((item: any, index: any) => index);
    ...
    const saveDoc = async (data: any) => {
        ...
        if (!refKodeGudang) {
            ...
            withReactContent(swalToast).fire({
                icon: 'warning',
                title: '<p style="font-size:12px;color:white">Data gudang harus diisi.</p>',
                width: '100%',
                target: '#dialogTtbList',
                customClass: {
                    popup: stylesTtb['colored-popup'], // Custom class untuk sweetalert
    ...
    const SpreadNumber = (number: any | number | string) => {
        const temp = parseFloat(parseFloat(number).toFixed(3));
        return temp;
    };
                                                ...
                                                <td>
                                                    <div className="flex">
                                                        <DropDownListComponent
                                                            ...
                                                            dataSource={apiResponseGudang}
                                                            fields={{ value: 'nama_gudang', text: 'nama_gudang' }}
