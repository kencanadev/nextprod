...
import { formatNumber, frmNumber, generateNU, FillFromSQL, FirstDayInPeriod, tanpaKoma, fetchPreferensi, overQTYAll, generateNUDivisi } from '@/utils/routines';
...
const DOPabrikPembatalanMKL: React.FC<dialogMutasiBarangAntarGudang> = ({ userid, kode_entitas, isOpen, onClose, kode_user, status_edit, kode_mb_edit, jenisDO, onRefresh }) => {
    ...
    const [mbEditAsli, setMbEditAsli] = useState<any>([]);
    ...
    const [NoMBPembatalan, setNoMBPembatalan] = useState('');
    ...
    const [tglMB, setTgl_MB] = useState<any>(moment().format('YYYY-MM-DD HH:mm:ss'));
    ...
    const [dataBarang, setDataBarang] = useState<any>({ nodes: [] }); //tidak digunakan
    const [dataJurnal, setDataJurnal] = useState<any>({ nodes: [] }); //tidak digunakan
    const [selectedRowIndex, setSelectedRowIndex] = useState(0);
    const [tambah, setTambah] = useState(false);
    const [edit, setEdit] = useState(false);
 const [tokenRedis, setTokenRedis] = useState<any>('');
    const [fbmNoGroup, setFBMnoGroup] = useState('');
    const [fbmKodeGroup, setFBMkodeGroup] = useState('');
...
const respToken = await axios.get(`${apiUrl}/erp/token_uuid`, {});
            ...
            if (status_edit) {
                ...
                if (listHeaderDetail.data.status === true) {
                    ...
                    setMbEditAsli(dataEdit);
                    console.log(dataEdit);
                    //HEADER
                    setNoMBPembatalan(dataEdit.no_mb);
                    //khusus pembatalan
                    ...
                    setKeterangan(`Pembatalan DO Pabrik No: ${dataEdit.no_mb} Tgl: ${moment(dataEdit.tgl_mb).format('DD-MM-YYYY')}`);
                    //dibalik khusus pembatalan
                    setSelectedGudang({ nama_gudang: dataEdit.gudang_tujuan, kode_gudang: dataEdit.kode_tujuan });
                    setSelectedGudang2({ nama_gudang: dataEdit.gudang_asal, kode_gudang: dataEdit.kode_gudang });
                    // if (jenisDO === 'KG' || jenisDO === 'MKG') {
                    //     setSelectedGudang2({ nama_gudang: dataEdit.gudang_asal, kode_gudang: dataEdit.kode_gudang });
                    //     setSelectedGudang({ nama_gudang: dataEdit.gudang_tujuan, kode_gudang: dataEdit.kode_tujuan });
                    // } else {
                    // }
                    ...
                    setFBMnoGroup(dataEdit.fbm_no_group);
                    setFBMkodeGroup(dataEdit.fbm_kode_group);
                    ...
                    const modifiedDetail = dataEdit.detailMB.map((item: any) => ({
                        ...
                        no_mbref: dataEdit.no_mb, // khusus pembatalan
                        id_pmb: 0, // khusus pembatalan
                        id_fpb: 0, // khusus pembatalan
                        fbm_id_so: 0, // khusus pembatalan
                        no_refkontrak: item.no_kontrak,
                    }));
                    ...
                    const modifiedDetailJurnal = detailJurnal.data.data.map((item: any, index: number) => ({
                        ...item,
                        ...
                        catatan: index === 0 ? `Mutasi barang dari gudang ${dataEdit.gudang_tujuan}` : `Mutasi barang ke gudang ${dataEdit.gudang_asal}`,
                    }));
    ...
    const handleDetail_Add = async (jenis: any) => {
        ...
        if (jenis === 'selected') {
            ...
            console.log(Stok);
            overQTYAll(kode_entitas, selectedGudang.kode_gudang, selectedBarang.kode_item, moment().format('YYYY-MM-DD HH:mm:ss'), '', 1, 'mb', 'Kuantitas barang').then(async (result) => {
                if (result === true) {
    ...
    }, [handleDetail_Add, DetailBarangDelete, DetailBarangDeleteAllJurnal]);
            ...
            if (Lskip_persediaan === 'N') {
                ...
                if (responseData[0]?.stok < parseFloat(qty)) {
                    const swalResult = await swal.fire({
                        title: 'Warning',
                        text: `${jenisWarning} (${qty}) melebihi stok yang ada (${responseData[0].stok}) di gudang asal, Lakukan TTB.`,
                        icon: 'warning',
                        target,
        ...
        try {
            ...
            if (selectedGudang.kode_gudang === selectedGudang2.kode_gudang) {
                ...
                const JSON = {
                    entitas: kode_entitas,
                    pembatalan: 'Y',
                    no_mb_pembatalan: NoMBPembatalan,
                    header: [
                        {
                            kode_mb: '',
                            no_mb: NoMB,
                            ...
                            no_reff: null, // noReff, // ikut FAS null
                            kode_supp: null, // selectedSupplier.kode_supp === undefined ? null : selectedSupplier.kode_supp, // ikut FAS null
                            kode_cust: null,
                            nopol: null, // noKendaraan, // ikut FAS null
                            via: null, // Via, // ikut FAS null
                            alamat_kirim: null, // alamatPengiriman, // ikut FAS null
                            kontrak: 'Y', // ubah dari state
                            status_export: null,
                            kode_gudang_cabang: kodeMB, // khusus PEMBATALAN
                            nama_gudang_cabang: 'BATAL DO PABRIK', // khusus PEMBATALAN
                            fdo: null,
                            ekspedisi: 'N', // ikut FAS
                            toleransi: 0.0,
                            harga_eks: 0.0,
                            harga_tambahan: 0.0,
                            ...
                            fbm: null,
                            fbm_no_so: null,
                            fbm_kode_group: '', // khusus pembatalan (urus dibackend ada kondisi soalnya)
                            fbm_no_group: 0, // khusus pembatalan
                            finalisasi: 'N',
                            pengajuan: 3,
                            approval: 'N',
                            tgl_finalisasi: null,
                ...
                if (status_edit == true) {
                    ...
                    if (JSON.header[0].jenis_mb === 'MKL') {
                        let bentukTTB = true;
                        for (let index = 0; index < modifiedDetailJson.length; index++) {
                            const hasilCek = await overQTYAllForMB(
                                kode_entitas,
                                selectedGudang.kode_gudang,
                                modifiedDetailJson[index].kode_item,
                            ...
                            if (hasilCek === true) {
                                bentukTTB = hasilCek
                                index = modifiedDetailJson.length
                                // if (confirm(`Item : ${modifiedDetailJson[index].kode_item} melebihi stok yang ada di gudang ${selectedGudang.nama_gudang} Apakah Akan Dibuatkan TTB?`)) {
                                //     bentukTTB = hasilCek;
                                // } else {
                                //     bentukTTB = false;
                                // }
                        ...
                        if (bentukTTB) {
                            const backtime = await axios.get(`${apiUrl}/erp/backtime?`, {
                                headers: {
                                    Authorization: `Bearer ${token}`,
                                },
                                params: {
                                    entitas: kode_entitas,
                                    param1: JSON.header[0].tgl_mb,
                                },
                            });
                            console.log('getTTB', mbEditAsli);
                            const getTTB = await axios.get(`${apiUrl}/erp/detail_ttb_utuh_langsung?`, {
                                headers: {
                                    Authorization: `Bearer ${token}`,
                                },
                                params: {
                                    entitas: kode_entitas,
                                    param1: mbEditAsli.do_kode_sj,
                                },
                            });

                            const backtimeData = backtime.data.data[0].pDate;
                            const quDataPusat: any = getTTB.data.data.master[0];
                            const quTempGudang: any = getTTB.data.data.tempGudang[0];
                            const quDataDetail: any = getTTB.data.data.detail;

                            const noTTB = await generateNUDivisi(
                                kode_entitas,
                                '',
                                quDataPusat.no_ttb.slice(0, 2),
                                '08',
                                moment(quDataPusat.tgl_ttb).format('YYYYMM'),
                                moment(quDataPusat.tgl_ttb).format('YYMM') + `${quDataPusat.no_ttb.slice(0, 2)}`
                            );

                            const quTTBPusat: any = {};
                            quTTBPusat.kode_ttb = 'oto';
                            quTTBPusat.no_ttb = noTTB;
                            quTTBPusat.tgl_ttb = backtimeData;
                            quTTBPusat.tgl_buku = moment(quTTBPusat.tgl_ttb).format();
                            quTTBPusat.no_reff = quDataPusat.no_reff;
                            quTTBPusat.dokumen = quDataPusat.dokumen;
                            quTTBPusat.kode_gudang = quTempGudang.kode_gudang; // quDataPusat.kode_gudang;
                            quTTBPusat.kode_cust = quDataPusat.kode_cust;
                            quTTBPusat.via = quDataPusat.via;
                            quTTBPusat.pengemudi = quDataPusat.pengemudi;
                            quTTBPusat.nopol = quDataPusat.nopol;
                            quTTBPusat.total_rp = quDataPusat.total_rp;
                            quTTBPusat.total_diskon_rp = quDataPusat.total_diskon_rp;
                            quTTBPusat.total_pajak_rp = quDataPusat.total_pajak_rp;
                            quTTBPusat.netto_rp = quDataPusat.netto_rp;
                            quTTBPusat.keterangan = quDataPusat.keterangan;
                            quTTBPusat.status = quDataPusat.STATUS;
                            quTTBPusat.userid = userid.toUpperCase(); //quDataPusat.userid;
                            quTTBPusat.tgl_update = quDataPusat.tgl_update;
                            quTTBPusat.kode_jual = quDataPusat.kode_jual;
                            quTTBPusat.jenis_ttb = 'utuh';
                            quTTBPusat.alasan = 'Pembatalan Otomatis';

                            const quDTTBPusat: any = [];
                            let TotalHPP = 0;
                            for (let index = 0; index < quDataDetail.length; index++) {
                                let tempQuDTTBPusat: any = {};
                                tempQuDTTBPusat.kode_ttb = 'oto';
                                tempQuDTTBPusat.id_ttb = index + 1; //quDataPusat.FieldByName('SpreadNumber(id_ttb'));
                                tempQuDTTBPusat.kode_sj = quDataDetail[index].kode_sj;
                                tempQuDTTBPusat.id_sj = quDataDetail[index].id_sj;
                                tempQuDTTBPusat.kode_do = quDataDetail[index].kode_do;
                                tempQuDTTBPusat.id_do = quDataDetail[index].id_do;
                                tempQuDTTBPusat.kode_so = quDataDetail[index].kode_so;
                                tempQuDTTBPusat.id_so = quDataDetail[index].id_so;
                                tempQuDTTBPusat.kode_item = quDataDetail[index].kode_item;
                                tempQuDTTBPusat.diskripsi = quDataDetail[index].diskripsi;
                                tempQuDTTBPusat.satuan = quDataDetail[index].satuan;
                                tempQuDTTBPusat.qty = SpreadNumber(quDataDetail[index].qty);
                                tempQuDTTBPusat.sat_std = quDataDetail[index].sat_std;
                                tempQuDTTBPusat.qty_std = SpreadNumber(quDataDetail[index].qty_std);
                                tempQuDTTBPusat.qty_sisa = SpreadNumber(quDataDetail[index].qty_sisa);
                                tempQuDTTBPusat.kode_mu = quDataDetail[index].kode_mu;
                                tempQuDTTBPusat.kurs = SpreadNumber(quDataDetail[index].kurs);
                                tempQuDTTBPusat.kurs_pajak = SpreadNumber(quDataDetail[index].kurs_pajak);
                                tempQuDTTBPusat.harga_mu = SpreadNumber(quDataDetail[index].harga_mu);
                                tempQuDTTBPusat.diskon = quDataDetail[index].diskon;
                                tempQuDTTBPusat.diskon_mu = SpreadNumber(quDataDetail[index].diskon_mu);
                                tempQuDTTBPusat.potongan_mu = SpreadNumber(quDataDetail[index].potongan_mu);
                                tempQuDTTBPusat.kode_pajak = quDataDetail[index].kode_pajak;
                                tempQuDTTBPusat.include = quDataDetail[index].include;
                                tempQuDTTBPusat.pajak_mu = SpreadNumber(quDataDetail[index].pajak_mu);
                                tempQuDTTBPusat.jumlah_mu = SpreadNumber(quDataDetail[index].jumlah_mu);
                                tempQuDTTBPusat.jumlah_rp = SpreadNumber(quDataDetail[index].jumlah_rp);
                                tempQuDTTBPusat.ket = quDataDetail[index].ket;
                                tempQuDTTBPusat.kode_dept = quDataDetail[index].kode_dept;
                                tempQuDTTBPusat.kode_kerja = quDataDetail[index].kode_kerja;
                                tempQuDTTBPusat.hpp = SpreadNumber(quDataDetail[index].hpp);
                                tempQuDTTBPusat.qty_sisa_lkb = SpreadNumber(quDataDetail[index].qty_sisa_lkb);
                                quDTTBPusat.push(tempQuDTTBPusat);
                                tempQuDTTBPusat = {};
                            }

                            for (let index = 0; index < quDTTBPusat.length; index++) {
                                TotalHPP += quDTTBPusat[index].hpp * quDTTBPusat[index].qty_std;
                            ...
                            const quJTTBPusat: any = {};
                            quJTTBPusat.kode_dokumen = 'oto';
                            quJTTBPusat.id_dokumen = 1;
                            quJTTBPusat.dokumen = 'TTB';
                            quJTTBPusat.tgl_dokumen = quTTBPusat.tgl_ttb;
                            quJTTBPusat.kode_akun = responseSettingData.kode_akun_persediaan;
                            quJTTBPusat.kode_subledger = '';
                            quJTTBPusat.kurs = 1;
                            quJTTBPusat.debet_rp = TotalHPP;
                            quJTTBPusat.kredit_rp = 0;
                            quJTTBPusat.jumlah_rp = SpreadNumber(quJTTBPusat.debet_rp);
                            quJTTBPusat.jumlah_mu = SpreadNumber(quJTTBPusat.debet_rp) / SpreadNumber(quJTTBPusat.kurs);
                            quJTTBPusat.catatan = 'Persediaan TTB No: ' + quTTBPusat.no_ttb;
                            quJTTBPusat.no_warkat = '';
                            quJTTBPusat.tgl_valuta = quJTTBPusat.tgl_dokumen;
                            quJTTBPusat.persen = 0;
                            quJTTBPusat.kode_dept = '';
                            quJTTBPusat.kode_kerja = '';
                            quJTTBPusat.approval = 'N';
                            quJTTBPusat.posting = 'N';
                            quJTTBPusat.rekonsiliasi = 'N';
                            quJTTBPusat.tgl_rekonsil = '';
                            quJTTBPusat.userid = userid.toUpperCase(); //quTTBPusat.userid;
                            quJTTBPusat.tgl_update = quTTBPusat.tgl_update;
                            quJTTBPusat.audit = '';
                            quJTTBPusat.kode_kry = '';
                            quJTTBPusat.kode_jual = quTTBPusat.kode_jual;

                            const quJKTTBPusat: any = {};
                            quJKTTBPusat.kode_dokumen = quTTBPusat.kode_ttb;
                            quJKTTBPusat.id_dokumen = 2;
                            quJKTTBPusat.dokumen = 'TTB';
                            quJKTTBPusat.tgl_dokumen = quTTBPusat.tgl_ttb;
                            quJKTTBPusat.kode_akun = responseSettingData.kode_akun_hpp;
                            quJKTTBPusat.kode_subledger = '';
                            quJKTTBPusat.kurs = 1;
                            quJKTTBPusat.debet_rp = 0;
                            quJKTTBPusat.kredit_rp = TotalHPP;
                            quJKTTBPusat.jumlah_rp = SpreadNumber(quJKTTBPusat.kredit_rp) * -1;
                            quJKTTBPusat.jumlah_mu = (SpreadNumber(quJKTTBPusat.kredit_rp) / SpreadNumber(quJKTTBPusat.kurs)) * -1;
                            quJKTTBPusat.catatan = 'Harga Pokok TTB No: ' + quTTBPusat.no_ttb;
                            quJKTTBPusat.no_warkat = '';
                            quJKTTBPusat.tgl_valuta = quJKTTBPusat.tgl_dokumen;
                            quJKTTBPusat.persen = 0;
                            quJKTTBPusat.kode_dept = '';
                            quJKTTBPusat.kode_kerja = '';
                            quJKTTBPusat.approval = 'N';
                            quJKTTBPusat.posting = 'N';
                            quJKTTBPusat.rekonsiliasi = 'N';
                            quJKTTBPusat.tgl_rekonsil = '';
                            quJKTTBPusat.userid = userid.toUpperCase(); //quTTBPusat.userid;
                            quJKTTBPusat.tgl_update = quTTBPusat.tgl_update;
                            quJKTTBPusat.audit = '';
                            quJKTTBPusat.kode_kry = '';
                            quJKTTBPusat.kode_jual = quTTBPusat.kode_jual;
                            ...
                            const auditTTB = {
                                ...
                                diskripsi: `TTB Item = ${quDTTBPusat.length}  nilai transaksi ${SpreadNumber(quTTBPusat.netto_rp).toLocaleString('en-US', {
                            ...
                            const ttb = {
                                ...
                                ...quTTBPusat,
                                detail: quDTTBPusat,
                                jurnal: [quJTTBPusat, quJKTTBPusat],
                                audit: auditTTB,
                            };
                            ...
                            const auditMBLama = {
                                ...
                                kode_audit: null,
                                dokumen: 'MB',
                                kode_dokumen: kode_mb_edit,
                                no_dokumen: mbEditAsli.no_mb,
                                tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                                proses: 'EDIT',
                                diskripsi: `MB Item = ${mbEditAsli.detailMB.length}  nilai transaksi ${SpreadNumber(mbEditAsli.netto_rp).toLocaleString('en-US', {
                                    minimumFractionDigits: 2,
                                    maximumFractionDigits: 2,
                                })}`,
                                userid: userid, // userid login web
                            ...
                            const JSONBARU = {
                                ...
                                ttb: ttb,
                                  token: tokenRedis,
                                mbLama: {
                                    kode_mb_lama: kode_mb_edit,
                                    audit: auditMBLama,
                                },
                            };

                            var responseAPI = await axios.post(`${apiUrl}/erp/batal_mbkl`, JSONBARU);
                            console.log('quTTBPusat', ttb);
                        } else {
                            //kondisi jika tombol ttb di cancle
                            const auditMBLama = {
                                entitas: kode_entitas,
                                kode_audit: null,
                                dokumen: 'MB',
                ...
                if (responseAPI.data.status === true) {
                    ...
                    const auditResponse = await axios.post(`${apiUrl}/erp/simpan_audit`, {
                        ...
                        tanggal: moment().format('YYYY-MM-DD HH:mm:ss'),
                        proses: status_edit == true ? 'EDIT' : 'NEW',
                        diskripsi: `MB item = ${gridMBList.dataSource.length}  nilai transaksi ${totalNettoRP.toLocaleString('en-US', {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                        })}`,
                        userid: userid, // userid login web
                    ...
                    withReactContent(swal)
                        .fire({
                            ...
                            html:
                                status_edit == true
                                    ? '<p style="font-size:12px">Pembatalan Data Mutasi Barang berhasil disimpan</p>'
            ...
            withReactContent(swal).fire({
                title: ``,
                html: status_edit == true ? '<p style="font-size:12px">Pembatalan Data Mutasi Barang gagal disimpan</p>' : '<p style="font-size:12px">Data Mutasi Barang gagal disimpan</p>',
                target: '#dialogMBList',
                icon: 'error',
                width: '350px',
                heightAuto: true,
    ...
    return (
        <DialogComponent
            ...
            header={'DO Pabrik *PEMBATALAN MKL'}
            showCloseIcon={true}
            target="#main-target"
            closeOnEscape={false}
            allowDragging={true}
                                ...
                                <table className={styles.table} style={{ width: '100%' }}>
                                    <thead>
                                        <tr>
                                            ...
                                            <th style={{ width: '12%' }}>Tgl. Pengambilan</th>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <DatePickerComponent
                                                    ...
                                                    showClearButton={false}
                                                    format="dd-MM-yyyy"
                                                    // value={new Date()} // Menggunakan objek Date untuk menetapkan nilai awal
                                        ...
                                        <div style={{ padding: 5 }} className="panel-pager">
                                            ...
                                            <div>
                                                ...
                                                <div className="panel-input" style={{ width: '100%', minHeight: 60, marginTop: 5 }}>
                                                    <textarea value={keterangan} className="h-full w-full text-xs" rows={2} onChange={(e: any) => setKeterangan(e.target.value)} />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div tabIndex={1} style={{ width: '100%', height: '100%', marginTop: '5px', padding: 10 }}>
                                        ...
                                        >
                                            <GridComponent
                                                ...
                                                editSettings={{ allowEditing: false, newRowPosition: 'Bottom' }}
                                                selectionSettings={{ mode: 'Row', type: 'Single' }}
                                                allowResizing={true}
                                                autoFit={true}
                                                rowHeight={22}
...
export default DOPabrikPembatalanMKL;
