...
const DialogNoNamaBarang = ({
    ...
    const [filterDataBarang, setFilterDataBarang] = useState([]);

    const handleSearchNamaItem = async (value: any) => {
        const response = await axios.get(`${apiUrl}/erp/list_barang_filter?`, {
            params: {
                entitas: kode_entitas,
                kode: searchNoItem ? searchNoItem : 'all',
                nama: value ? value : 'all',
                limit: '25',
        ...
        if (result1.status) {
            setFilterDataBarang(result1.data);
        }
    };

    const handleSearchNoItem = async (value: any) => {
        const response = await axios.get(`${apiUrl}/erp/list_barang_filter?`, {
            params: {
                entitas: kode_entitas,
                kode: value ? value : 'all',
        ...
        >
            <div className="h-full w-full">
                ...
                <GridComponent
                    ...
                    rowHeight={22}
                    height={300}
                    dataSource={searchNamaItem === '' && searchNoItem === '' ? listDaftarBarang : filterDataBarang}
                ...
                onBlur={(e: any) => {
                    ...
                    if (e.target.value == 0 || e.target.value === '0') {
                        ...
                        if (penyesuaianNilai == false) {
                            dataSekarang.qty = parseFloat(parseFloat(e.target.value).toFixed(2));
            ...
            >
                <ColumnsDirective>
                    ...
                    <ColumnDirective
                        ...
                        template={(props: any) => {
                            return <span>{props.qty.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}</span>;
                        }}
                    />
                    <ColumnDirective
                        field="jumlah_rp"
