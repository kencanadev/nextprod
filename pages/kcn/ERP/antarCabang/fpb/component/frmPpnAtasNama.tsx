...
interface FrmPpnAtasNamaDlgProps {
    isOpen: boolean;
    onClose: any;
    onBatal: any;
    selectedData: any;
...
const FrmPpnAtasNamaDlg = ({ isOpen, onClose, onBatal, selectedData, target, stateDokumen }: FrmPpnAtasNamaDlgProps) => {
    ...
    const SearchDataNama = (keyword: any, listItemBarang: any[]) => {
        if (keyword === '') {
            ...
            const filteredData = listItemBarang.filter((item) => item.nama_cabang.toLowerCase().includes(keyword.toLowerCase()));
    ...
    const fetchDataDialog = async () => {
        try {
            const response = await axios.get(`${apiUrl}/erp/get_ppn_atas_nama?entitas=${stateDokumen.kode_entitas}`, {
                headers: {
...
export default FrmPpnAtasNamaDlg;
