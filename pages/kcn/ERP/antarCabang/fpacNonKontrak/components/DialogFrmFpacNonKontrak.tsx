...
const DialogFrmFpacNonKontrak: React.FC<DialogFrmFpacNonKontrakProps> = ({ open, onClose, token, kode_entitas, statusPage, kode_dokumen, userid, onRefresh, isApprovalPusat }) => {
    ...
    const { handleSave, setEditDataFromModal, editData, isLoading, setHargaBeliMu, setHargaJualMu, setHargaMu } = useApprovalPusat();
    ...
    const handlePilihFj = async (values: any[]) => {
        // Gunakan item pertama untuk data header/master
        ...
        // Update state untuk data master/header
        ...
        updateState('kodeSo', firstItem.kode_so);
        updateState('kodeSj', firstItem.kode_sj);
        updateState('fakturJual', firstItem.no_fj);
        updateState('kodeGudangJual', firstItem.kode_gudang);
        updateState('namaGudangJual', firstItem.nama_gudang);

        // Hitung total dari semua items
        const totalMu = values.reduce((sum, item) => sum + parseFloat(item.jumlah_mu), 0);
        const totalBerat = values.reduce((sum, item) => sum + parseFloat(item.berat), 0);

        updateState('totalMu', totalMu.toString());
        updateState('berat', totalBerat.toString());

        // Mapping semua items untuk grid
        if (values.length > 0) {
            const mappedBarang = values.map((item: any, index: number) => {
                const { berat, ...restValue } = item;
                return {
                    ...restValue,
                    id_fpac: index + 1,
                    harga_mu: 0,
                    ...
                    berat: 0, // atau bisa tetap menggunakan: parseFloat(item.berat) jika ingin mempertahankan berat per item
                };
            });
    ...
    const handleEdit = async () => {
        ...
        if (detail.length > 0) {
            ...
            setHargaJualMu(parseFloat(mappedBarang[0].harga_jual_mu));
    ...
    const actionCompleteDetailBarang = async (args: any) => {
        switch (args.requestType) {
            case 'save':
                const hargaMu = parseFloat(args.data.harga_mu);
    ...
    const handleApprovalPusat = async (approvalStatus: any) => {
        ...
        const q = parseFloat(res[0].q);
        const jumlah_rp = gridFPACList.dataSource[0].jumlah_rp;
        if (parseFloat(res[0].q) > 0) {
            if (jumlah_rp > q) {
                swal.fire({
                    icon: 'warning',
    ...
    }, [imagePreview, handleMouseMove, handleMouseUp, handleWheel]);
    ...
    return (
        <DialogComponent
            ...
            close={onClose}
            header={
                statusPage === 'EDIT' && !isApprovalPusat
                    ? 'FORM FPAC NON KONTRAK (EDIT)'
                    : statusPage === 'EDIT' && isApprovalPusat
                    ? 'FORM FPAC NON KONTRAK (APPROVAL PUSAT)'
                    : 'FORM FPAC NON KONTRAK (NEW)'
            }
            showCloseIcon={true}
            target="#main-target"
            closeOnEscape={false}
            ...
            <div style={{ minWidth: '70%', overflow: 'auto' }}>
                ...
                <div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <div className="flex" style={{ alignItems: 'center' }}>
                                                ...
                                                <input
                                                    ...
                                                    style={{ fontSize: 11, borderColor: '#bfc9d4', borderRadius: 2 }}
                                                    disabled={true}
                                                    value={masterData.namaGudangBeli}
                                                    readOnly
                                                ></input>
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                    <FontAwesomeIcon
                                        ...
                                        <td>
                                            ...
                                            <DatePickerComponent
                                                ...
                                                change={(args: ChangeEventArgsCalendar) => {
                                                    updateState('tglTrxFpac', moment(args.value));
                                                }}
                                                // value={new Date()}
                                                disabled={true}
                                            >
                                        ...
                                        <td>
                                            ...
                                            <DatePickerComponent
                                                ...
                                                change={(args: ChangeEventArgsCalendar) => {
                                                    updateState('tglFpac', moment(args.value));
                                                }}
                                                disabled={true}
                                            >
                                                <Inject services={[MaskedDateTime]} />
                                        ...
                                        <td style={{ textAlign: 'center' }} colSpan={2}>
                                            ...
                                            <div className="flex" style={{ alignItems: 'center' }}>
                                                <input
                                                    ...
                                                    style={{ fontSize: 11, borderColor: '#bfc9d4', borderRadius: 2 }}
                                                    disabled={true}
                                                    value={masterData.namaGudangJual}
                                                    readOnly
                                                ></input>
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                    <FontAwesomeIcon
                                        ...
                                        <td style={{ textAlign: 'center' }} colSpan={2}>
                                            ...
                                            <div className="flex" style={{ alignItems: 'center' }}>
                                                <input
                                                    ...
                                                    style={{ fontSize: 11, borderColor: '#bfc9d4', borderRadius: 2 }}
                                                    disabled={true}
                                                    value={masterData.namaCustPusat}
                                                    readOnly
                                                ></input>
                                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                                    {/* <FontAwesomeIcon
                    ...
                    <div className="panel-tab" style={{ background: '#f0f0f0', width: '100%', height: '220px', marginTop: 10, borderRadius: 10 }}>
                        <TabComponent
                            ref={(t) => (tabFPACList = t)}
                            selectedItem={statusPage === 'UPDATE-FILE' ? 1 : 0}
                        ...
                        >
                            ...
                            <div className="e-content">
                                ...
                                <div tabIndex={1} style={{ width: '100%', height: '100%', marginTop: '5px', padding: 10 }}>
                                    <div className="flex max-h-[150px] items-start gap-2 overflow-y-scroll">
                                        {/* Table File */}
                                        <table>
                                            <thead>
                                                <tr>
                                        ...
                                        <div className="hidden">
                                            <UploaderComponent
                                                ...
                                                ref={uploaderRefFiles}
                                                multiple={false}
                                                selected={(e) => handleFileSelect(e)}
                                                removing={() => {}}
                                            />
                                        </div>
                                        {/* Action Buttons */}
                ...
                {imagePreview && imageSrc && (
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <span className="e-icons e-close" style={{ fontSize: '20px', fontWeight: 'bold' }} onClick={() => setImagePreview(false)}></span>
                            </ButtonComponent>
                        </div>
                    </div>
                )}
