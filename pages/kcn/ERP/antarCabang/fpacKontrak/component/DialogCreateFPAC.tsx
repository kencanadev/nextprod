...
const DialogFPACList: React.FC<DialogFPACList> = ({
    ...
    const { handleSave, setEditDataFromModal, editData, isLoading, setHargaMu, setHargaBeliMu, setHargaJualMu, setTokenRedis } = useApprovalPusat();
    ...
    const handlePilihEntitas = async () => {
        if (selectedEntitas.kodecabang == '898') {
    ...
    const handleApproval = async (approvalStatus: any) => {
        try {
            ...
            const reqBody = {
                ...
                kontrak: 'Y',
                approval2: approvalStatus,
                tgl_approval2: moment().format('YYYY-MM-DD HH:mm:ss'),
                kirim_langsung_cabang: 'N',
                alamat_kirim_cabang: alamatKirimCabang,
                keterangan_pusat: keteranganPusat,
                kode_gudang_beli: kodeGudangBeli,
            ...
            const response = await axios.patch(`${apiUrl}/erp/update_fpac`, reqBody, {
                headers: { Authorization: `Bearer ${token}` },
            });
    ...
    useEffect(() => {
        if (statusPage === 'EDIT' && isApprovalPusat === true) {
            getTokenKey();
        }
    }, []);
            ...
            <div style={{ minWidth: '70%', overflow: 'auto' }}>
                        ...
                        <div>
                            ...
                            <ButtonComponent
                                ...
                                style={{
                                    ...
                                    marginTop: 1 + 'em',
                                    marginRight: 1 + 'em',
                                    backgroundColor: isLoading || isLoadingApp ? '#A2A5B6' : '#3b3f5c',
                                    cursor: isLoading || isLoadingApp ? 'not-allowed' : 'pointer',
                                }}
                                disabled={isLoading || isLoadingApp}
                                onClick={() => {
                ...
                >
                    ...
                    <GridComponent
                        ...
                        height={'340'}
                        rowSelecting={(args) => {
                            if (args.data.status_warna === 'Aktif' || args.data.status_warna === 'NOO' || args.data.status_warna === 'BlackList-G') {
