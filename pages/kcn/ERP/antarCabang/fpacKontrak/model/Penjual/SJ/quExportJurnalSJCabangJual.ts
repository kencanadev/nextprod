...
export const createJurnalSJPembeliPayload = async (editData: any, kode_entitas: string, token: string) => {
    ...
    const payload1 = {
        ...
        debet_rp: parseFloat(hpp) * editData.detail[0].qty,
        kredit_rp: 0,
        jumlah_rp: parseFloat(hpp) * editData.detail[0].qty,
        jumlah_mu: parseFloat(hpp) * editData.detail[0].qty,
    ...
    const payload2 = {
        ...
        kurs: parseFloat(editData.kurs),
        debet_rp: 0,
        kredit_rp: parseFloat(hpp) * editData.detail[0].qty,
        jumlah_rp: -1 * (parseFloat(hpp) * editData.detail[0].qty),
        jumlah_mu: -1 * (parseFloat(hpp) * editData.detail[0].qty),
