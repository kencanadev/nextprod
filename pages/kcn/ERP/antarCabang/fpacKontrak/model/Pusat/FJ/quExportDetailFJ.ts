...
export const createDetailFJPusatPayload = async (editData: any, hargaBeliMu: any, hargaMu: any, token: string, kode_entitas: string) => {
    const payloads = await Promise.all(
        editData.detail.map(async (detail: any, index: any) => {
            ...
            const payload = {
                ...
                jumlah_mu: harga_mu * parseFloat(detail.qty_std),
                jumlah_rp: harga_mu * parseFloat(detail.qty_std),
                berat: detail.brt,
                hpp: harga_beli_mu,
            };
