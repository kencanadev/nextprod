...
export const createDetailPBPembeliPayload = async (editData: any, kode_entitas: any, token: any, hargaMu: any) => {
    ...
    const payloads = editData.detail.map((detail: any, index: any) => {
        const harga_beli_mu = hargaMu != 0 ? hargaMu : parseFloat(detail.harga_beli_mu);
        const jumlah_mu = detail.qty_std * harga_beli_mu;
        const payload = {
