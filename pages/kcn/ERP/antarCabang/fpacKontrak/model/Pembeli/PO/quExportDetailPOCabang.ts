...
export const createDetailPOPembeliPayload = async (editData: any, kode_entitas: any, token: any, hargaMu: any) => {
    ...
    const payloads = editData.detail.map((detail: any, index: number) => {
        let harga_mu = hargaMu != 0 ? hargaMu : parseFloat(detail.harga_mu);
        const jumlah_mu = detail.qty * harga_mu;
        const payload = {
