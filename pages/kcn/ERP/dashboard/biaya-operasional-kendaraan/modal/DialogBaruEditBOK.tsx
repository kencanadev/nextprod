...
import { generateNU, viewPeriode } from '@/utils/routines';
...
const DialogBaruEditBOK = ({
    ...
    const setGenereteNU = async (periodeTaunBulan: any = null) => {
        if (periodeTaunBulan === null) {
            ...
            const vNoMBUtama = await generateNU(kode_entitas, '', '96', periodeTaunBulan);
    ...
    const validasiForm = () => {
        ...
        const isCekPeriode = cekPeriode(moment(headerDialogState.tgl_bok).format('YYYY-MM-DD'));
    ...
    const savedoc = async () => {
        startProgress();
        let vNoMBUtama = await generateNU(kode_entitas, '', '96', moment(headerDialogState.tgl_bok).format('YYYYMM'));
        ...
        try {
            const cekDataBOK = await cekDataDiDatabase(kode_entitas, 'tb_m_fk', 'no_fk', kirim?.no_fk, token);
            if (cekDataBOK) {
                // jsonData.no_ttb = defaultNoTtb;
                
                const generateCounter = await generateNU(kode_entitas, kirim.no_fk, '96', moment(headerDialogState.tgl_bok).format('YYYYMM'));
                const generateNoDok = await generateNU(kode_entitas, '', '96', moment(headerDialogState.tgl_bok).format('YYYYMM'));
                kirim.no_fk = generateNoDok;
            }
            ...
            if (response.data.status) {
                await generateNU(kode_entitas, vNoMBUtama, '96', moment(headerDialogState.tgl_bok).format('YYYYMM'));
    ...
    const handleTgl = async (date: any, tipe: string) => {
        if (tipe === 'tgl_bok') {
            setHeaderDialogState((oldData: any) => ({
                ...oldData,
                tgl_bok: moment(date).format('YYYY-MM-DD HH:mm:ss'),
            }));
            if (masterState !== 'EDIT') {
                await setGenereteNU(moment(date).format('YYYYMM'));
            }
        }
    };
    return (
