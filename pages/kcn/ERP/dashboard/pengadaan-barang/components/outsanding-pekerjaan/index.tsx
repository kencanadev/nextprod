...
import { mockData, OPTabList, POTabList } from '../../constants';
...
import PembatalanPO from './tabs/poTab/PoPembatalan';
...
const OutstandingPekerjaan = ({ kode_entitas, token, userid, tabIdx1 }: OutstandingPekerjaanProps) => {
    const [activeMainTab, setActiveMainTab] = useState<any>(OPTabList[0].key);
    const [activeMainTab2, setActiveMainTab2] = useState<any>(POTabList[0].key);
    ...
    const [visiblePembatalanPO, setVisiblePembatalanPO] = useState(false);
    const router = useRouter();
    const tabId = typeof sessionStorage !== 'undefined' ? sessionStorage.getItem('tabId') || '' : '';
    const { tabIdx2, tabIdx3, noDokPO } = router.query;
...
// console.log('activeMainTab',activeMainTab);
    ...
    const fetchData = async (tipeGet: any = null) => {
        console.log('fetch data', tipeGet);
        if (tipeGet == 'Y') {
            localStorage.removeItem('dataCustmoerTemp');
        }
        ...
        for (const customerItem of customerData) {
            const props = {
                ...
                kode_dokumen: customerItem.kode_dokumen,
                token,
            };
            const detail = await fetchDetailData(props);
            customerItem.isRed = detail.some((item: any) => item.qty_sisa < 0);
        }
        ...
        const saved: any = localStorage.getItem('dataCustmoerTemp');
        const setingSort: any = localStorage.getItem('SetingFilterCust');

        if (saved) {
            try {
                let tempIndex = 0;
                const mod = JSON.parse(saved).map((item: any, index: number) => {
                    if (item.no_dok === noDokPO) {
                        tempIndex = index;
                        return {
                            ...item,
                            status_batal: 'Y',
                ...
                refGridCustomer.current?.setProperties({ dataSource: mod });
                setCustData(mod);
                refGridCustomer.current?.refresh();
                setTimeout(() => {
                    // refGridCustomer.current?.setProperties({ sortSettings : JSON.parse(setingSort)})
                    refGridCustomer.current?.selectRow(tempIndex, true);
                }, 300);
            } catch (err) {
    ...
    const handleNavigateLink = async (tipe: string) => {
        ...
        if (tipe === 'approval' && header) {
            ...
            router.push({ pathname: `/kcn/ERP/purchase/po/po`, query: { str: result, tipe: 'dashboard', tabIdx1: tabIdx1, tabIdx2: activeMainTab, tabIdx3: activeMainTab2 } });
            ...
            const temp = refGridCustomer.current!.dataSource;

            localStorage.setItem('dataCustmoerTemp', JSON.stringify(temp));
            // const seting = String(refGridCustomer.current?.sortSettings.columns);
            console.log('seting euy ', refGridCustomer.current?.sortSettings);

            // localStorage.setItem("SetingFilterCust", JSON.stringify(refGridCustomer.current?.sortSettings.columns));
            router.push({ pathname: `/kcn/ERP/purchase/po/po`, query: { str: result, tipe: 'dashboard', tabIdx1: tabIdx1, tabIdx2: activeMainTab, tabIdx3: activeMainTab2 } });
        }
    };
    ...
    useEffect(() => {
        if (tabIdx2) {
            setActiveMainTab(tabIdx2 ? tabIdx2 : OPTabList[0].key);
        }
    }, [tabIdx2]);

    useEffect(() => {
        if (tabIdx3) {
            setActiveMainTab2(tabIdx3 ? tabIdx3 : POTabList[0].key);
        }
    }, [tabIdx3]);

    return (
        <div className="mt-1 h-[calc(133vh-200px)] w-[calc(130vw)]" id="main-target">
            {/* Header Tab */}
                ...
                <div className="flex h-full w-full flex-1">
                    {activeMainTab === 'order_pembelian' && (
                        <OrderPembelian
                            ...
                            activeMainTab={activeMainTab2}
                            setActiveMainTab={setActiveMainTab2}
                        />
                    )}
                ...
                <div className="h-fit w-full bg-gray-200 px-5 py-3">
                    ...
                    <div className={`${isShowDetail ? 'block' : 'hidden'}`}>
                        <GridComponent {...gridOptions} enableAdaptiveUI dataSource={detailData}>
                            <ColumnsDirective>
                                ...
                                <ColumnDirective field="qty_sisa" valueAccessor={zeroValueAccessor} headerText="Outstanding" format="N0" headerTextAlign="Center" textAlign="Right" autoFit />
                                <ColumnDirective
                                    field="berat"
                                    headerText="Berat (Kg)"
                                    format="N0"
                    ...
                    <div className="flex items-center gap-1 bg-gray-300 p-2">
                        ...
                        <ButtonComponent
                            ...
                            style={{ ...styleButton, width: 'auto' }}
                            onClick={() => setVisiblePembatalanPO(true)}
                        />
            ...
            <JenisTransaksi
                ...
                tabIdx1={tabIdx1}
                tabIdx2={activeMainTab}
                tabIdx3={activeMainTab2}
            />
            {showDetailSupplier && (
            ...
            {visiblePembatalanPO && <PembatalanPO visible={visiblePembatalanPO} onClose={() => setVisiblePembatalanPO(false)} kode_entitas={kode_entitas} masterData={dataFromSupp} refereshData={() => {}} token={token} userid={userid} gridRefAsli={activeMainTab2 === 'dikirim_gudang' ? refGridGudang.current : refGridCustomer.current} />}
        </div>
    );
};
