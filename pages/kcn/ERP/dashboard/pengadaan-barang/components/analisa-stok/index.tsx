...
import { TemplateSorting } from './template';
...
const AnalisaStok = ({ kode_entitas, token, tabIdx1 }: AnalisaStokProps) => {
    ...
    // context hooks
    const { data, setData, setCurrentEntitas, clearData } = useDataHandlerContext();
    const detailRef = useRef<GridComponent | null>(null);
    ...
    const handleRecordDoubleClick = async (args: any) => {
        if (args.column.field === 'sotl' || args.column.field === 'sol') {
            ...
            if ((args.rowData.sol === 0 || args.rowData.sol === null) && (args.rowData.sotl === 0 || args.rowData.sotl === null)) {
                swal.fire({
                    icon: 'warning',
            ...
            await fetchDetailAktualStok({ params, token }).then(async (res: any) => {
                ...
                await fetchDetailStokGudang({ params, token }).then((res: any) => {
                    ...
                    detailRef.current?.setProperties({ dataSource: res });
    ...
    const getDetailStok = () => {
        const isMb = selectedPabrikData.nama_gudang.toLowerCase().includes('customer') || selectedPabrikData.nama_gudang.toLowerCase().includes('transit');

        const params = {
            entitas: kode_entitas,
            param1: 0,
            param2: moment(selectedPabrikData.tanggal).format('YYYY-MM-DD'),
            param3: selectedPabrikData.kode_gudang,
            param4: selectedPabrikData.kode_item,
            param5: isMb ? selectedPabrikData.nama_gudang : selectedPabrikData.stok,
            param6: isMb ? 'MB' : 'FIFO',
        };
    ...
    useEffect(() => {
        if (kode_entitas) {
            setCurrentEntitas(kode_entitas);
        }
    }, [kode_entitas, setCurrentEntitas]);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const modifiedData1 = res
                .map((item: any) => ({
                    ...item,
                    sol: parseFloat(item.sol),
                    sotl: parseFloat(item.sotl),
                    hariin: item.buffer_in <= 0 ? 1 : item.buffer_in,
                    hariout: item.buffer_out <= 0 ? 1 : item.buffer_out,
                }))
                .filter((item: any) => item.remind <= 1)
                .sort((a: any, b: any) => a.remind - b.remind);

            const modifiedData2 = res
                .map((item: any) => ({
                    ...item,
                    sol: parseFloat(item.sol),
                    sotl: parseFloat(item.sotl),
            ...
            const combinedData = [...modifiedData1, ...modifiedData2];
            setData(combinedData);
            // analisaGridRef.current?.setProperties({ dataSource: combinedData });
    ...
    const setQueryCellInfo = (args: any) => {
        if (
            args.column.field === 'rata_sekarang_out' ||
            args.column.field == 'sotl' ||
            args.column.field === 'sol' ||
            args.column.field === 'qty_real' ||
            args.column.field === 'qty_target' ||
            args.column.field === 'maksimal' ||
            args.column.field === 'maksimal_pab' ||
            args.column.field === 'stok_internal' ||
            args.column.field === 'stok_ekternal' ||
            args.column.field === 'stok_cabang' ||
            args.column.field === 'stok_pabrik' ||
            args.column.field === 'po' ||
            args.column.field === 'jual_tinggi'
        ) {
            if (args.data.remind <= 1) {
        ...
        if (args.column.field === 'nama_item') {
            if (args.data.remind === 0) {
                args.cell.classList.add('bg-reds');
                // args.cell.style.backgroundColor = '#ff0000';
                // args.cell.style.color = '#ffff00';
            } else if (args.data.remind === 1) {
                args.cell.classList.add('bg-yellows');
                // args.cell.style.backgroundColor = '#ffff00';
            } else {
                args.cell.classList.add('bg-whites');
            }
        }

        if (args.column.field === 'minimal') {
            if (args.data.stok_internal < args.data.minimal) {
                console.log('');
        ...
        if (args.column.field === 'minimal_pab') {
            if (args.data.stok_pabrik < args.data.minimal_pab) {
                args.cell.style.backgroundColor = '#00ffff';
                args.cell.style.color = '#000000';
            } else if (args.data.remind <= 1) {
                args.cell.style.color = '#ff0000';
        ...
        if (args.column.field === 'rata_sekarang_in') {
            if (args.data.stok_internal < args.data.rata_sekarang_in * args.data.hariin) {
                args.cell.style.backgroundColor = '#00ffff';
                args.cell.style.color = '#000000';
            } else if (args.data.remind <= 1) {
                args.cell.style.color = '#ff0000';
        ...
        // Tidak digunakan di grid
        // if (args.column.field === 'rata_sekarang_pab') {
        //   args.cell.style.backgroundColor = '#00ffff';
        //   if (args.data.stok_pabrik <= args.data.rata_sekarang_pab * args.data.hariout) {
        //   }
        // }
    };

    const queryDetailInfo = (args: any) => {
        const { column, data } = args;
        if (column.field === 'hari') {
            const isTransit = selectedPabrikData.nama_gudang.toLowerCase().includes('transit');
            if (isTransit) {
                if (data.hari < 5) {
            ...
            } else if (selectedPabrikData.jenis_gudang === 'P') {
                if (data.hari > 35) {
                    args.cell.style.backgroundColor = '#ff0000';
                }
            } else if (selectedPabrikData.jenis_gudang === 'I') {
                if (data.jenis === 'BESI') {
                    if (data.hari > 35) {
                        args.cell.style.backgroundColor = '#ff0000';
                    }
        ...
        if (column.field === 'kuantitas' && dialogType !== 'stok_pabrik') {
            args.cell.style.backgroundColor = '#25F300';
        }
    };

    const zeroValueAccessor = (field: string, data: any, column: any) => {
        return data[field] == '0' ? null : data[field];
    };
    ...
    const minusValueAccessor = (field: string, data: any, column: any) => {
        if (data[field] < 0) {
            ...
            if (field === 'stok_ekternal') {
                return data[field] ? data[field].toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 }) : null;
            }
    ...
    return (
        <div className="Main h-[calc(130vh-280px)] w-[calc(130vw)]">
                                        ...
                                        {kategori.map((item: any) => (
                                            <div key={item.id} className="mb-1 flex items-center">
                                                <input
                                                    ...
                                                    onChange={() => handleCheckboxChange(item.id, item.kode)}
                    ...
                    >
                        <ColumnsDirective>
                            <ColumnDirective
                                ...
                                allowEditing={false}
                            />
                            {/* <ColumnDirective field="remind" headerText="Remind" width="100" autoFit headerTextAlign="Center" textAlign="Left" clipMode="EllipsisWithTooltip" allowEditing={false} /> */}
                            <ColumnDirective
                                ...
                                columns={[
                                    {
                                        ...
                                        headerTemplate: TemplateSorting,
                                    },
                                    {
                        ...
                        <Inject services={[Page, Selection, Edit, Sort, Group, Filter, Resize, Reorder, CommandColumn, Toolbar, Freeze]} />
                    </GridComponent>
                </div>
                {/* style={{ background: '#dedede' }} */}
            </div>
            ...
            {showDetailStok && (
                <Draggable>
                    <div className={`${styles.modalDetailDragable}`} style={{ top: '3%', right: '2%', width: '100%', background: '#dedede' }}>
                        <div className="overflow-auto" style={{ maxHeight: '400px' }}>
                            <div className="flex flex-col gap-1 rounded border-b-2 border-black/80 bg-white p-2" style={{ marginBottom: 21 }}>
                                <span style={{ fontSize: 18, fontWeight: 700 }}>{selectedItem?.nama_item}</span>
                                <span style={{ fontSize: 16, fontWeight: 300 }}>Detail Stok per Gudang</span>
                            </div>
                            <div className="flex">
                                {/* Grid Gudang */}
                                <div className="w-[30%]">
                                ...
                                <div className="w-[70%]">
                                    <GridComponent
                                        dataSource={detailStokGudang}
                                        ref={detailRef}
                                        queryCellInfo={queryDetailInfo}
                                        width={'100%'}
                                        height={200}
                                        rowHeight={30}
                                        gridLines={'Both'}
            ...
            <JenisTransaksi
                ...
                statusDokValue={''}
                tipeForm="dashboard"
                isBelumAccDireksiChecked={false}
                isKirimanLangsungChecked={false}
                isPembatalanOrderChecked={false}
                isPoBarangProduksiChecked={false}
                isPoDenganPajakChecked={false}
                isPoKontrakChecked={false}
                isPoNonKontrakChecked={false}
                isSudahAccDireksiChecked={false}
                onSelectData={(selectedData: any) => {}}
                tabIdx1={tabIdx1}
            />
            {/* Dialog Kartu Stok */}
