...
const InfoGudangTTB = ({ kode_entitas, token }: InfoGudangTtbProps) => {
    ...
    const minusValueAccessor = (field: string, data: any, column: any) => {
        if (data[field] < 0) {
            return `(${Math.abs(data[field]).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })})`;
        } else if (data[field] === 0) {
            return null;
        } else {
            return data[field].toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
        ...
        <div className="Main h-[calc(130vh-240px)] w-[calc(130vw)]">
            <div className="relative flex h-full">
                ...
                <div className="h-full w-full flex-1 overflow-auto bg-white">
                    <GridComponent {...gridOptions} width="55%" queryCellInfo={setQueryCellInfo} rowSelected={handleRowSelected} recordDoubleClick={handleRecordDoubleClick} dataSource={data}>
                        <ColumnsDirective>
                            <ColumnDirective field="nama_item" headerText="Nama Barang" width="220" headerTextAlign="Center" textAlign="Left" clipMode="EllipsisWithTooltip" />
                            <ColumnDirective
                                field="stok_minimal"
                                valueAccessor={zeroValueAccessor}
                                format="N"
                                headerText="Stok Minimal Internal"
                                width="80"
