...
const DataStokOverdue = ({ kode_entitas, token, userid = '' }: DataStokOverdueProps) => {
        ...
        fetchStokOverdue(params, token)
            .then((res) => {
                const modifiedData = res
                    ...
                    .sort((a: any, b: any) => b.hari - a.hari);
    ...
    return (
        <div className="Main h-[calc(130vh-310px)] w-[calc(130vw)]" id="stok-overdue">
                                    ...
                                    {GudangList.map((item) => (
                                        <div key={item.id} className="mt-1 flex items-center">
                                            <input
                                                type="checkbox"
                                                id={`checkbox-${item.id}`}
                                                checked={!!checkedGudang[item.id]}
                                                onChange={() => handleCheckboxChangeGudang(item.id, item.kode)}
                                                className="cursor-pointer"
                                            />
                                            <label htmlFor={`checkbox-${item.id}`} className="m-0 ml-1 cursor-pointer text-xs text-gray-900">
                                                {item.name}
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <input
                                        type="date"
                                        className="rounded border border-gray-300 p-2"
                                        value={updateForm.tgl_followup2}
                                        onChange={(e) => updateFormState('tgl_followup2', e.target.value)}
                                    />
                                </div>
                                <textarea
                                    name="aksi"
