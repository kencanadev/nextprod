...
export const FillFromSQL = async (entitas: string, fld: string, kode_user?: any, token?: any) => {
    ...
    if (fld === 'departemen') {
        ...
        try {
            const response = await axios.get(`${apiUrl}/erp/item_kustom`, {
                params: {
                    entitas,
                    param1: 9,
                },
            ...
            console.error('Error fetching data jenis produk:', error);
            throw error;
        }
    } else if (fld === 'kustom8') {
        try {
        ...
        try {
            const response = await axios.get(`${apiUrl}/erp/akun`, {
                params: {
                    entitas,
                },
                headers: {
...
export const isEntitasPajak = async (entitas: string, userid: string, token: string) => {
    const apiUrl = process.env.NEXT_PUBLIC_LOGIN_API || '';
    try {
        const response = await axios.get(`${apiUrl}/erp/entitas_pajak`, {
            params: {
                entitas,
                param1: userid.toUpperCase(),
            },
            headers: {
                Authorization: `Bearer ${token}`,
            },
        ...
        if (response.data.data.length > 0) {
            const matchingEntitas = response.data.data.find((item: any) => item.kodecabang === entitas && item.pajak === 'Y');

            return !!matchingEntitas;
        }
...
export const cekDataDiDatabase = async (entitas: string, pNamaTabel: string, pNamaField: string, pValueField: string, token: string) => {
    const apiUrl = process.env.NEXT_PUBLIC_LOGIN_API || '';

    try {
        const response = await axios.get(`${apiUrl}/erp/cek_no_dokumen`, {
            params: {
                entitas,
                param1: pNamaTabel,
                param2: pNamaField,
...
export const cekNoDokTerakhir = async (entitas: string, pNamaTabel: string, pJenisDok: string, pKategoriTabel: string, pKodeJual: string, token: string) => {
    const apiUrl = process.env.NEXT_PUBLIC_LOGIN_API || '';

    try {
        const response = await axios.get(`${apiUrl}/erp/cek_last_num`, {
            params: {
                entitas,
                param1: pNamaTabel,
                param2: pJenisDok, // jenis dokumen 'BK', 'BM' UNTUK KATEGORI 0; 'DO', 'SJ', 'FJ' DLL UNTUK KATEGORI 1
                param3: pKategoriTabel, // KATEGORI 0 => tb_m_keuangan_master / tb_m_keuangan; KATEGORI 1 => 'DO', 'SJ', 'FJ', DLL
                param4: pKodeJual, // JIKA KATEGORI (PARAM3 = 1) => 'SA', 'AC','KA' (KODE JUAL)
            },
            headers: {
                Authorization: `Bearer ${token}`,
            },
...
export const usersApp = async (entitas: string, userid: any) => {
    const apiUrl = process.env.NEXT_PUBLIC_LOGIN_API || '';
    try {
        const response = await axios.get(`${apiUrl}/erp/users_app?`, {
            params: {
...
export const overQTYAllForMPB = async (kode_entitas: string, kodeGudang: string, kodeItem: string, tgl: string, kode_dokumen: string, qty: any, jenis: any, jenisWarning: any, target: any = '') => {
    const apiUrl = process.env.NEXT_PUBLIC_LOGIN_API || '';
    let Lskip_persediaan;
    ...
    try {
        ...
        if (Lskip_persediaan === 'N') {
            ...
            if (SpreadNumber(responseData[0]?.stok) < SpreadNumber(qty)) {
                const swalResult = await swal.fire({
                    title: 'Warning',
                    text: `${jenisWarning} (${qty}) melebihi stok yang ada (${SpreadNumber(responseData[0].stok)}).`,
                    icon: 'warning',
                    target,
                    confirmButtonText: 'OK',
