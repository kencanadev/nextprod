import { HandleChangeParamsObject } from '@/utils/fa/konsolidasi-phe/template/HandleChangeParamsObject';
import Swal from 'sweetalert2';
import { GetBankEks, GetListAlokasiPembayaranPhe, GetListKaryawan, GetLoadFileGambarById, GetTbImagesPhe, GetTbImagesPheById, LoadImagesPhe, getListPhe } from '../api/api';
import { HandleChangeParamsObjectDialogPhe } from '@/utils/fa/konsolidasi-phe/template/HandleChangeParamsObjectDialogPhe';
...
import { ListSubledger } from '@/pages/kcn/ERP/fa/ppi/model/apiPpi';
...
const pencarianNoPhe = async (params: HandleChangeParamsObject) => {
    ...
    const filteredDataRpe = pencarianDataNoRpe(searchValue, params.recordsDataRpe);
    ...
    params.setFilteredDataRpe(filteredDataRpe);
...
const pencarianDataNoRpe = (keyword: any, recordsData: any[]) => {
    // Jika keyword kosong, kembalikan semua data
    if (keyword === '') {
        return recordsData;
    } else {
...
const pencarianBayar = async (params: HandleChangeParamsObject) => {
    ...
    const filteredDataRpe = pencarianDataBayar(searchValue, params.recordsDataRpe);
...
const handleNamaEkspedisiChange = async (params: HandleChangeParamsObjectDialogPhe) => {
    ...
    const totalNettoMu = respListAlokasiPembayaranPheFix.reduce((sum: any, item: any) => sum + (item.sisa_hutang || 0), 0);
...
const handleDoubleClick = (args: any, params: HandleChangeParamsObject) => {
    ...
    params.setStateDataParams((prevState: any) => ({
        ...
        kode_dokumen: '',
        filterTab: '',
        kodeEntitasKonsolidasi: args.rowData.entitas,
    }));
};
...
const handleRowSelected = (args: any, params: HandleChangeParamsObject) => {
    if (args.data !== undefined) {
        ...
        params.setStateDataParams((prevState: any) => ({
            ...
            filterTab: '',
            kodeEntitasKonsolidasi: args.data.entitas,
        }));
    }
};

const handleRowSelectedOD = (args: any, tipe: any, params: HandleChangeParamsObject) => {
    if (args.data !== undefined) {
        params.setMasterKodeDokumen(args.data.kode_phe);
        params.setRefreshKey((prevKey: any) => prevKey + 1);
        params.setStateDataParams((prevState: any) => ({
...
const onClickAutoJurnal = async (params: HandleChangeParamsObjectDialogPhe) => {
    ...
    if (params.gridPheListRef && Array.isArray(params.gridPheListRef?.dataSource)) {
        ...
        // Hitung jumlah kemunculan setiap no_reff yang memiliki kode_phe tidak null
        const noReffCount: Record<string, number> = {};
        dataSource.forEach((item) => {
            if (item.kode_phe !== null) {
                noReffCount[item.no_reff] = (noReffCount[item.no_reff] || 0) + 1;
            }
        });

        // Ambil hanya yang muncul lebih dari satu kali
        const noReffList = Object.keys(noReffCount).filter((key) => noReffCount[key] >= 1);
    ...
    const resultListSubledger: any[] = await ListSubledger(params.kode_entitas, params.stateDataHeader?.kodeAkunJurnal2);
    const respListSubledgerFilter = resultListSubledger.filter((item: any) => item.kode_subledger === 'SS0000000647');

    const respListKaryawan = await GetListKaryawan(params.kode_entitas, params.token);
    const respListKaryawanFilter = respListKaryawan.filter((item: any) => item.nama_kry === 'KANTOR');

    console.log('respListKaryawanFilter = ', respListKaryawanFilter);
    const respGetBankEks = await GetBankEks(paramObject);
    const catRek = `${respGetBankEks[0].bank} ${respGetBankEks[0].no_rek} ${respGetBankEks[0].nama_rek}`.trim();

    const selisih = (params.stateDataHeader?.jumlahBayar === '' ? 0 : params.stateDataHeader?.jumlahBayar) - totalPembayaran;
    const apiUrl = process.env.NEXT_PUBLIC_LOGIN_API || '';
    const quSetting : any = await fetchPreferensi(params.kode_entitas, apiUrl);
    const dataJurnalArry = [
        {
            ...
            kode_kry: respListKaryawanFilter[0].kode_kry,
            nama_kry: respListKaryawanFilter[0].nama_kry,
        },
        {
            id: 4,
            ...
            // nama_subledger: '',
            // subledger: '',
            kode_subledger: params.kode_entitas === '300' ? respListSubledgerFilter[0].kode_subledger : '',
            no_subledger: params.kode_entitas === '300' ? respListSubledgerFilter[0].no_subledger : '',
            nama_subledger: params.kode_entitas === '300' ? respListSubledgerFilter[0].nama_subledger : '',
            subledger: params.kode_entitas === '300' ? respListSubledgerFilter[0].subledger : '',
            debet_rp: 0,
...
const clearImagesClose = (params: HandleChangeParamsObjectDialogPhe) => {
...
const loadDataImage = async (kode_phe: any, params: HandleChangeParamsObjectDialogPhe) => {
    const responseDataImage = await GetTbImagesPheById(params.kode_entitas, kode_phe);
    const tempForGrid: any[] = [];

    for (const item of responseDataImage) {
        try {
            const getImage = await GetLoadFileGambarById('898', item.kode_dokumen, item.id_dokumen, params.kode_entitas);
            if (!getImage?.data?.length) continue;

            const imageData = getImage.data[0];
            const mimeType = imageData.decodeBase64_string.split(':')[1]?.split(';')[0] || '';
            const file = new File([imageData.decodeBase64_string], item.filegambar, { type: mimeType });

            tempForGrid.push({
                base64_string: imageData.base64_string,
                decodeBase64_string: imageData.decodeBase64_string,
                dokumen: imageData.dokumen,
                filegambar: imageData.filegambar,
                fileoriginal: imageData.fileoriginal,
                gambar: imageData.gambar,
                id_dokumen: imageData.id_dokumen,
                kode_dokumen: imageData.kode_dokumen,
                st: imageData.st,
            });
        } catch (error) {
    ...
    params.setLoadFilePendukung(tempForGrid);
};
...
const totNettoMuPheBaru = (args: any) => {
    const nettoMu = args.result.reduce((total: number, item: any) => {
        return total + parseFloat(tanpaKoma(String(item.netto_mu)) === '' ? '0' : tanpaKoma(String(item.netto_mu)));
    }, 0);
    return frmNumber(nettoMu);
};

const totBayarMuPheBaru = (args: any) => {
    const bayarMu = args.result.reduce((total: number, item: any) => {
        return total + parseFloat(tanpaKoma(item.bayar_mu) === '' ? '0' : tanpaKoma(item.bayar_mu));
    }, 0);
    return frmNumber(bayarMu);
...
const handleDoubleClickOD = (args: any, tipe: any, params: HandleChangeParamsObject) => {
    params.setMasterDataState('EDIT');
    params.setMasterKodeDokumen(args.rowData.kode_phe);
    params.setDialogInputDataVisible(true);
    params.setRefreshKey((prevKey: any) => prevKey + 1);
